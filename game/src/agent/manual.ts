...
export const EVENT_CATALOGUE: Record<GameEventType, { about: string; fields: string }> = {
  ...
  "item.received": { about: "Items or gold entered the pack. source says how: gather, loot, buy, production. A loot pile drop lists items instead. A kill pays no gold directly: gold is a stack in the kill's pile and reaches the purse when taken.", fields: "itemId?, name?, quantity?, source?, skill?, from?, pileId?, items?" },
  ...
  "item.lost": { about: "Items or gold left the pack: consumed, sold, or dropped on death.", fields: "itemId?, name?, quantity?, reason?, cacheId?, items?" },
...
export const ERROR_CATALOGUE: Record<GameErrorCode, string> = {
  ...
  NOT_ENOUGH_CURRENCY: "Not enough gold.",
...
const TERMINOLOGY = [
  ...
  "gold — the currency. An item (id gold) that never takes a pack slot: picking it up adds to the currency balance.",
