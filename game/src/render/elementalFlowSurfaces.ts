...
import {
  Fn, If, abs, attribute, clamp, cos, float, fract, max,
...
function flowMaterial(shapeId: number): MeshBasicNodeMaterial {
  return acquireEffectMaterial(`elemental-flow:${shapeId}`, () => {
    const emission=uniform(0);
    const time:Node<"float">=clockUniform();
    const material=new MeshBasicNodeMaterial({
      transparent:true,depthWrite:false,side:THREE.DoubleSide,fog:false,alphaTest:.012,
    });
    // These spatial sheets use the pool's named instance matrix attributes.
    const instanceTransform:Node<"mat4">=effectInstanceMatrix();
...
export class ElementalFlowSurfaces {
  private readonly batches = new Map<Shape,EffectInstances<MeshBasicNodeMaterial>>();
  ...
  constructor(parent: THREE.Object3D) {
    for (const [shape,shapeId] of [["band",0],["shell",1],["funnel",2],["plume",3]] as const) {
      ...
      const mesh = new EffectInstances(g, flowMaterial(shapeId), 96);
      mesh.userData["effectClock"] = this.clock;
      mesh.instanceCount=0;mesh.visible=false;mesh.frustumCulled=false;mesh.name=`elemental-flow-${shape}`;
  ...
  begin(seconds:number):void{this.clock.value=seconds;this.dropped=0;for(const m of this.batches.values())m.instanceCount=0;}
  put(shape:Shape,element:SpellElement,x:number,y:number,z:number,sx:number,sy:number,sz:number,alpha:number,seed:number,yaw=0,pitch=0,foam=0,profile:FlowProfile={}):void{
    ...
    const m=this.batches.get(shape)!;if(m.instanceCount>=96){this.dropped++;return;}
    this.pose.position.set(x,y,z);this.pose.rotation.set(pitch,yaw,0);this.pose.scale.set(sx,sy,sz);this.pose.updateMatrix();
    m.setMatrixAt(m.instanceCount,this.pose.matrix);
    (m.geometry.getAttribute("flowProfile") as THREE.InstancedBufferAttribute).setXYZW(m.instanceCount,profile.foot??.09,profile.arc??1,profile.lean??0,profile.twist??1);
    (m.geometry.getAttribute("flowLife") as THREE.InstancedBufferAttribute).setXYZW(m.instanceCount++,kinds[element],alpha,seed,foam);
  }
  get instances():number{return [...this.batches.values()].reduce((n,m)=>n+m.instanceCount,0);}
  end():void{for(const m of this.batches.values()){m.visible=m.instanceCount>0;m.instanceMatrix.needsUpdate=true;m.geometry.getAttribute("flowLife").needsUpdate=true;m.geometry.getAttribute("flowProfile").needsUpdate=true;}}
  dispose():void{for(const m of this.batches.values()){m.removeFromParent();m.geometry.dispose();releaseEffectMaterial(m.material);}}
}
