...
export class BasicElementalVfx {
  ...
  constructor(parent:THREE.Object3D,private readonly ground:(x:number,z:number)=>number,
    ...
    this.flame=new ElementalEnergyBodies(parent,"fire");
    this.flame.mesh.name="elemental-basic-flame";
    this.pebble=new FracturedBoulder(parent,24,"elemental-basic-pebble");
  ...
  get instances():number{return this.currents.instances+this.flow.instances+this.flame.instances+Number(this.pressure.visible)+Number(this.pebble.mesh.visible);}
  get dropped():number{return this.currents.dropped+this.flow.dropped+this.flame.dropped;}
  get solidCount():number{return this.pebble.mesh.visible?this.pebble.pieces:0;}
  begin(seconds:number):void{this.clock.value=seconds;this.currents.begin(seconds);this.flow.begin(seconds);this.flame.begin(seconds);this.pebble.hide();this.pressure.visible=false;}
  update(cast:ElementalCast,now:number,element:SpellElement):void{
    ...
    const palette=ELEMENTAL_ENERGY[cast.spellId],u=clamp((age-100)/(p.at-100));
    ...
    const release:Vec3=[cast.origin[0]+fx*.5+fz*.28,cast.origin[1]+1.17,cast.origin[2]+fz*.5-fx*.28];
    const point=(v:number):Vec3=>[release[0]+(cast.aim[0]-release[0])*v,
      release[1]+(this.ground(cast.aim[0],cast.aim[2])+(element==="earth"?.35:1.1)-release[1])*v+Math.sin(v*Math.PI)*(element==="earth"?.95:.14),
      release[2]+(cast.aim[2]-release[2])*v];
    const head=point(u),tail=point(Math.max(0,u-.12)),grow=.4+smooth(age/140)*.6;
    const fade=local<0?1:1-smooth(local/.88);
    if(element==="earth"){
      this.pebble.pose(head[0],head[1],head[2],.26*grow,Math.min(age,p.at)*.008,local,fade);
    }else if(local<0){
      if(element==="wind"){
        ...
        this.fluids.put("drop",...head,.23*grow,.23*grow,.44*grow,yaw);
        this.flow.put("shell",element,...head,.25*grow,.25*grow,.45*grow,.54,6,yaw,0,0,{arc:.76});
      }else{
        this.flame.curve(tail,[tail[0],tail[1]+.18,tail[2]],[head[0]-fx*.1,head[1]+.1,head[2]-fz*.1],head,.23,0xff9e27,1,7,.7,1);
        this.flow.put("plume",element,...head,.34,.6,.34,.94,7,yaw,.5);
      }
    }
    if(local<0){
      for(let i=0;i<180;i++){
        ...
        this.light.put(pos[0]+Math.cos(angle)*r,pos[1]+Math.sin(angle)*r,pos[2],.014+random(i,3)*.019,
          i%5?palette.edge:palette.core,.7,i,1.2,element==="wind"?1.1:1.7);
      }
    }else{
      const groundY=this.ground(head[0],head[2]),r=.16+(1-Math.exp(-local*7))*.95;
      if(element==="wind"){
        ...
        this.flow.put("plume",element,head[0],groundY+.10,head[2],.52*fade,.9*fade,.46*fade,fade,9,yaw,0,0,{arc:.85,lean:.7});
      }
      for(let i=0;i<320;i++){
        const angle=random(i,4)*Math.PI*2,d=local*(.5+random(i,5)*2),x=head[0]+Math.cos(angle)*d,z=head[2]+Math.sin(angle)*d;
        const y=element==="fire"?groundY+.2+local*(.3+random(i,6)*1.5):Math.max(groundY+.03,head[1]+local*(random(i,6)*2)-local*local*3.8);
        this.light.put(x,y,z,.013+random(i,7)*.020,palette.edge,fade*(1-random(i,8)*.5),i,1.3,element==="wind"?1:1.8);
  ...
  end():void{this.currents.end();this.flow.end();this.flame.end();}
  dispose():void{this.currents.dispose();this.flow.dispose();this.flame.dispose();this.pebble.dispose();this.unregister();this.pressure.removeFromParent();this.pressure.geometry.dispose();this.pressure.material.dispose();}
}
