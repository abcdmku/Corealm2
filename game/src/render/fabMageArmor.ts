...
export function applyRareMageMaterial(
  ...
  const sourceColor = surfaceColorNode(prepared)
  ...
  let panel: Node<'vec3'> = palette.mul(luma.max(0).pow(0.48)).mul(1.9);
  const trim = threadColor.mul(luma.max(0).pow(0.35));
  let thread: Node<'float'> = float(0);
  if (cloth && textile) {
    ...
    panel = mix(panel, weave.mul(tier === 90 ? float(1.25) : luma.max(0).sqrt().mul(0.35).add(1.05)),
      embroidered ? 1 : auroraMantle ? 0.36 : 0.12);
  }
  const color = mix(panel, trim, edge.mul(embroidered ? (tier === 90 ? 0 : 0.08) : 0.88));
  ...
  if (cloth) {
    ...
    result.iridescenceNode = float((result.iridescenceNode ?? sourceMaterialNode<'float'>(result, MaterialNode.IRIDESCENCE)) as Node<'float'>)
