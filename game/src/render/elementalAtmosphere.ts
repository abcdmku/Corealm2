...
export class ElementalAtmosphere {
  ...
  constructor(parent: THREE.Object3D, kind: "fire" | "wind" | "dust" | "smoke") {
    ...
    const material = acquireEffectMaterial("elemental-atmosphere:" + kind, () => {
      ...
      material.fragmentNode = Fn(() => {
        ...
        Loop(steps, ({ i }) => {
          ...
          if (kind === "fire") {
            ...
            const fuel = world.mul(1.35).add(vec3(vSeed.mul(.173), time.mul(-2.8), vSeed.mul(.271))).toVar();
            ...
            const flow = p.mul(vec3(3.8, 2.4, 3.8)).add(vec3(vSeed, time.mul(-1.8), time.mul(.24))).toVar();
            ...
            if (kind === "wind") {
              ...
              const coil = sin(angle.mul(3).sub(u.mul(34)).add(time.mul(5)).add(n.mul(3.5))).mul(.5).add(.5).toVar();
    ...
    this.mesh.userData.effectClock=this.time;
  ...
  dispose():void{this.mesh.removeFromParent();this.mesh.geometry.dispose();releaseEffectMaterial(this.mesh.material);}
}
