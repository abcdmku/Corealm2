...
import { attribute, cos, exp, float, mix, modelNormalMatrix, sin, smoothstep, transformNormal, uv, varying, vec2, vec3, vec4 } from "three/tsl";
...
/** Shared recipes reference each draw's named attributes and object clock, never a pool's arrays. */
function currentMaterial(id: number, pressure: boolean): MeshBasicNodeMaterial {
  const life=attribute("currentLife","vec4" as const),profile=attribute("currentProfile","vec4" as const),time=clockUniform();
  ...
  const coord=uv(),point=currentPoint(coord),u=coord.x,v=coord.y;
  ...
  if (pressure) {
    ...
    const pressureMaterial=createElementalRefractionMaterial({liquid:false,strength:22,
...
export class AirCurrentSheets {
  private readonly batches = new Map<CurrentShape, {
    visible: EffectInstances<MeshBasicNodeMaterial>;
    pressure: EffectInstances<MeshBasicNodeMaterial>;
  }>();
  ...
  constructor(parent: THREE.Object3D, name = "elemental-air") {
    for (const [shape, id] of [["crescent", 0], ["spiral", 1], ["jet", 2], ["helix", 3]] as const) {
      ...
      const material=acquireEffectMaterial(`air-current:${shape}:visible`,()=>currentMaterial(id,false));
      const pressureMaterial=acquireEffectMaterial(`air-current:${shape}:pressure`,()=>currentMaterial(id,true));
      const visible=new EffectInstances(geometry,material,64);
      const pressure=new EffectInstances(geometry,pressureMaterial,64,{matrices:visible.instanceMatrix});
      for (const mesh of [visible, pressure]) {
        mesh.instanceCount = 0; mesh.visible = false; mesh.frustumCulled = false;
        mesh.userData.effectClock = this.clock; parent.add(mesh);
      }
  ...
  get instances(): number { return [...this.batches.values()].reduce((n, b) => n + b.visible.instanceCount * 2, 0); }
  begin(seconds: number): void { this.clock.value = seconds; this.dropped = 0; for (const b of this.batches.values()) b.visible.instanceCount = b.pressure.instanceCount = 0; }
  put(shape: CurrentShape, x: number, y: number, z: number, sx: number, sy: number, sz: number,
    ...
    const b = this.batches.get(shape)!, i = b.visible.instanceCount;
    ...
    b.visible.setMatrixAt(i, this.pose.matrix);
    ...
    b.visible.instanceCount++; b.pressure.instanceCount++;
  }
  end(): void {
    for (const b of this.batches.values()) {
      for (const mesh of [b.visible, b.pressure]) mesh.visible = mesh.instanceCount > 0;
      b.visible.instanceMatrix.needsUpdate = true;
  ...
  dispose(): void {
    ...
    for (const b of this.batches.values()) { b.visible.removeFromParent(); b.pressure.removeFromParent(); b.visible.geometry.dispose(); b.pressure.geometry.dispose(); releaseEffectMaterial(b.visible.material); releaseEffectMaterial(b.pressure.material); }
  }
}
