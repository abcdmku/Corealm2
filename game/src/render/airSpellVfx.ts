...
/** Moving pressure, with sparse directional tracers. Air never emits stone or ground debris. */
export class AirSpellVfx {
  ...
  dropped = 0;
  readonly state={baseDiameter:0,height:0,variant:"",composition:""};
  ...
  get instances(): number { return this.currents.instances+this.flow.instances+this.atmosphere.instances+[...this.batches.values()].reduce((n,m)=>n+m.count,0); }
  begin(seconds:number):void {
    this.clock.value=seconds;this.dropped=0;this.atmosphere.begin(seconds);this.flow.begin(seconds);this.currents.begin(seconds);
    this.state.baseDiameter=0;this.state.height=0;this.state.variant="";this.state.composition="";
  ...
  private needle(cast:ElementalCast,age:number,t:number):void {
    ...
    if(u>=0&&u<1.1){
      const f=1-smooth((u-1)/.1),v=clamp(u),bend=Math.sin(v*Math.PI)*.32,
        x=cast.origin[0]+dx*v+dz/len*bend,z=cast.origin[2]+dz*v-dx/len*bend,y=this.ground(x,z)+1.35;
    ...
    if(local>=0&&local<.82){
      const [x,,z]=p.point,y=this.ground(x,z)+1.3,fade=1-smooth(local/.82),r=.28+local*2.2;
    ...
    for(const [i,p] of cast.pulses.entries()){
      ...
      if(u>=0&&u<1.15){
        const v=clamp(u),f=smooth(u*5)*(1-smooth((u-1)/.15)),side=Math.sin(v*Math.PI)*(i-1)*1.25,
          x=cast.origin[0]+dx*v+dz/len*side,z=cast.origin[2]+dz*v-dx/len*side,y=this.ground(x,z)+1.65+Math.sin(v*Math.PI)*.6;
      ...
      if(local>=0&&local<.9){
        const [x,,z]=p.point,y=this.ground(x,z)+.8,fade=1-smooth(local/.9),r=1+local*2.1;
  ...
  private cannon(cast:ElementalCast,age:number,t:number):void {
    ...
    if(age>280&&end<.28){
      const distance=age<first.at?clamp((age-280)/(first.at-280))*near:near+(age-first.at)/(last.at-first.at)*(far-near),
        fade=smooth((age-280)/200)*(1-smooth(end/.28)),x=cast.origin[0]+fx*distance,z=cast.origin[2]+fz*distance,y=this.ground(x,z)+1.45,
    ...
    for(const [i,p] of cast.pulses.entries()){
      ...
      const art=elementalPulseArt(cast.spellId,i),[x,,z]=p.point,y=this.ground(x,z)+1.35,fade=1-smooth(local/.85),r=.8+local*1.8;
  ...
  private tornado(cast:ElementalCast,age:number,t:number):void {
    const first=cast.pulses[0]!,end=cast.pulses.at(-1)!.at,start=first.at-850,
      fade=smooth((age-start)/420)*(1-smooth((age-end-220)/750));
    if(fade<.009)return;
    const [x,,z]=first.point,y=this.ground(x,z),r=8.4,h=17,foot=.68,
      descent=(1-clamp((age-start)/(first.at-start))**2)*h*.8,touchdown=(age-first.at)/1000;
    ...
    if(warning>.009){
      ...
      this.put("ripple",x,y+.15,z,rad,.32,rad,warning*.52,81);
      for(let i=0;i<900;i++){
    ...
    this.atmosphere.put(x,y+descent+h*.5,z,r,h*.5,r,fade*.16,2,y);
    // Refractive mass supplies the broad wedge. Unequal rotating currents describe its circulation.
    this.put("funnel",x,y+descent,z,r,h,r,fade*.62,2,0,foot);
    this.put("funnel",x+.2,y+descent+.1,z-.2,r*.86,h*.94,r*.86,fade*.42,12,-t*.3,.76);
    ...
    for(const [k,p] of cast.pulses.entries()){
      const local=(age-p.at)/1000;if(local<0||local>1)continue;
      const f=(1-smooth(local)),radius=(k===0?10:p.radius)*(1-Math.exp(-local*5)),art=elementalPulseArt(cast.spellId,k);
      this.put("ripple",x,y+.18,z,radius,.72*(1-local)+.15,radius,f*.8,k);
      this.currents.put("spiral",x,y+.14,z,radius*art.width,1.0,radius*art.depth,f*.85,70+k,t*.7+k*1.4,0,0,
        {turns:.70+k*.08,width:.22,drift:1.2});
      this.flow.put("band","wind",x,y+.1,z,radius,1.2*f,radius,f*.40,70+k,art.yaw,0,0,{arc:.7,lean:art.bend});
      this.spray(x,y+.25,z,p.radius,local,f,70+k,k*2.2,1200);
    }
  }
  private spray(x:number,y:number,z:number,radius:number,age:number,fade:number,seed:number,yaw:number,count:number):void {
    ...
    this.atmosphere.end();
    this.flow.end();this.currents.end();this.dropped+=this.flow.dropped+this.atmosphere.dropped+this.currents.dropped;
