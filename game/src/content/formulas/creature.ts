...
export function calculateCreatureCombat(level: number, profile: CreatureProfile): Omit<EnemyDef, 'id' | 'name' | 'family' | 'lootRolls'> {
  ...
  return {
    ...
    gold: [Math.round(level * profile.goldPerLevel), Math.round(level * profile.goldPerLevel * 2)],
