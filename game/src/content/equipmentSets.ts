...
function defineSet(id: string, name: string, tier: number, style: "melee" | "magic", memberIds: readonly [
    string,
    string,
    string,
    string,
    string
], defence: number, health: number): EquipmentSetDefinition {
    return {
        id, name, tier, style,
        members: { head: memberIds[0], body: memberIds[1], legs: memberIds[2],
            hands: memberIds[3], feet: memberIds[4] },
        thresholds: [
            { pieces: 2, bonuses: bonuses(style === "melee" ? { defence: defence } : { defence: defence }) },
            { pieces: 4, bonuses: bonuses({ health }) },
            { pieces: 5, bonuses: bonuses(style === "melee" ? { defence: defence } : { defence: defence }) },
        ]
    };
}
...
export const EQUIPMENT_SETS: readonly EquipmentSetDefinition[] = [
    ...
    ...WILDERNESS_CRAFTING_TIERS.flatMap(row => [
        defineSet(row.metal, row.metalName, row.tier, 'melee', [`${row.metal}_helm`, `${row.metal}_plate`, `${row.metal}_greaves`, `${row.metal}_gauntlets`, `${row.metal}_boots`], row.tier === 50 ? 10 : 14, row.tier === 50 ? 7 : 10),
        defineSet(row.hide, row.hideName, row.tier, 'magic', [`${row.hide}_hood`, `${row.hide}_robe`, `${row.hide}_leggings`, `${row.hide}_wraps`, `${row.hide}_boots`], row.tier === 50 ? 10 : 14, row.tier === 50 ? 7 : 10),
    ]),
];
