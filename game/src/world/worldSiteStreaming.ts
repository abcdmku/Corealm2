...
export class WorldSiteStreaming {
  ...
  constructor(private readonly scene: WorldScene, private readonly assets: AssetRegistry) {}
  ...
  private selected(area: PlayerAssetArea): SiteRecord[] {
    return [...this.records.values()].filter(({ site, resolved }) =>
      (site.regionId === 'gravelmaw') === (area.regionId === 'gravelmaw') && resolved.placements.some(piece => {
  ...
  async prepare(area: PlayerAssetArea, options: AssetLoadOptions): Promise<void> {
    await Promise.all(this.selected(area).map(record => {
      ...
      record.loading = drawWorldSiteDressing(this.scene, this.assets, record.site, record.resolved, options)
