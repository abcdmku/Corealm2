...
| Production asset synced | Galeskin boss | Six-motion lab check passed; name, ecology, and drop edits are done and content is wired. The GLB is byte-identical, manifest metadata is synced, and the production build passed. Authored-world proof remains; keep `worldIntegrated` false until it passes. |
...
| Motion and palette passed; final review pending | Bramble Tiller creature candidate | Warm layered palette and first five motions passed. Runtime has 2K normal and metallic-roughness maps (metalness 0, roughness 0.90–0.99); emissive is deliberately zero. Final isolated death screenshot and promotion remain. |
