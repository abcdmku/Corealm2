...
| Production integrated and accepted | Galeskin boss | Six-motion lab check passed; name, ecology, and drop edits are done and content is wired. The GLB is byte-identical, manifest metadata is synced, and the production build passed. Fallowmarch world proof passed: the live `creature_boss_galeskin` miniboss rendered at `[-298.64, 144.76]` from a normal camera view with zero errors. |
...
| Production integrated and accepted | Bramble Tiller creature | Full six-motion normal-camera lab check passed, including isolated death at grounded 0.833 m with face and limbs intact; warm woodland PBR palette passed. Production GLB/manifest and name, ecology, and drop edits are complete. Fallowmarch south population proof passed: `creature_briar_harrow` renders as Bramble Tiller with sampled-rig Walk/Idle, drawn bounds, and zero errors from a normal camera view; capture recorded at `test-results/tripo-lab/bramble-authored-south.png`. |
...
| Lab review queued | Grove Brute (Vellenwood T5) | Exact approved 8K source is archived; rig, animation, and PBR candidate files are prepared. Root lab visual/motion acceptance is pending; not production (`worldIntegrated: false`). |
