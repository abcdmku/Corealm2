---
title: "Cobalt Helm"
description: "A closed helm with a stone-cutter's brow ridge."
---

<div class="corealm-item-detail">
<img src="../../assets/items/kaldite_helm.webp" alt="Cobalt Helm" width="256" height="256" />
<p>A closed helm with a stone-cutter's brow ridge.</p>
</div>

<dl class="codex-stats"><div><dt>Tier</dt><dd>10</dd></div><div><dt>Category</dt><dd>Equipment</dd></div><div><dt>Stacks</dt><dd>No</dd></div><div><dt>Buy value</dt><dd>880</dd></div><div><dt>Sell value</dt><dd>528</dd></div><div><dt>Slot</dt><dd>Head</dd></div></dl>

## Equipment stats

<dl class="codex-stats"><div><dt>Melee Accuracy</dt><dd>+2</dd></div><div><dt>Defence</dt><dd>+9</dd></div><div><dt>Health</dt><dd>+2</dd></div></dl>

## Use and requirements

- Requires Melee 10

## Where it comes from

| Source | Name | Details |
| --- | --- | --- |
| Made by | Cobalt Helm | 1 per craft |
| Made by | Pale Dragon Plated Cobalt Helm | 1 per craft |
| Sold by | [Kiln Row Smith](../../spells-and-shops/#kiln-row-smith) | 2 in stock |

[Back to all items](../)
