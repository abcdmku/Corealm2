---
title: "Thick Hide Hood"
description: "Deepwood hide, still faintly thorned along the seam."
---

<div class="corealm-item-detail">
<img src="../../assets/items/bramblehide_hood.webp" alt="Thick Hide Hood" width="256" height="256" />
<p>Deepwood hide, still faintly thorned along the seam.</p>
</div>

<dl class="codex-stats"><div><dt>Tier</dt><dd>5</dd></div><div><dt>Category</dt><dd>Equipment</dd></div><div><dt>Stacks</dt><dd>No</dd></div><div><dt>Buy value</dt><dd>300</dd></div><div><dt>Sell value</dt><dd>180</dd></div><div><dt>Slot</dt><dd>Head</dd></div></dl>

## Equipment stats

<dl class="codex-stats"><div><dt>Magic Accuracy</dt><dd>+2</dd></div><div><dt>Defence</dt><dd>+4</dd></div><div><dt>Health</dt><dd>+2</dd></div><div><dt>Magic Power</dt><dd>+1</dd></div></dl>

## Use and requirements

- Requires Magic 5

## Where it comes from

| Source | Name | Details |
| --- | --- | --- |
| Made by | Thick Hide Hood | 1 per craft |

[Back to all items](../)
