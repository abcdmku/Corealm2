...
Use `--mobile` for a 393 × 852 touch-device context at DPR 2, including the production mobile texture and draw-distance choices. The `viewport` operation accepts widths down to 320 pixels. Resizing a desktop session alone does not enable the touch-device rendering path.
Use `--webgl` to exercise the WebGL2 fallback by withholding WebGPU from the test page. Read `getGraphicsBackendState()` to verify the backend; the default session may also fall back if WebGPU initialization fails.
