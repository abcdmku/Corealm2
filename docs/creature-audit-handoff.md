...
Production review catalog now has 111 entries. All 41 polish verdicts are committed and pushed (f2969ad). All thirteen supplied replacement bases, producing fifteen asset IDs, are integrated and validated. Remaining original replacement verdicts: 35 (30 owned-source reviews and five undelivered manual bases).
...
## User-supplied Beetle Golem, fourth continuation batch

Source preservation commit `f8cb3e6` is pushed. The accepted candidate is `assets/art/tripo/imports/creatures/audit-user-beetle-golem/`, SHA-256 `f4a373ed1867e4c744488f1b001f788f6fea58b83379147ee00cde848b5ce8a2`. The original 5,166-triangle source is preserved under `sources/`, SHA-256 `fb03cd921a5ffed201d577cf4102a182d9826f12664e14d11db38b7a60db5193`, matching Downloads. Source mesh, native 67-joint skin, bind matrices, UVs and detailed PBR maps are preserved. The shell uses masked iridescence/clearcoat; `textures/build-mask.mjs` reproduces the numerical mask from the original embedded map.

Two Sol posture attempts distorted the shoulders and were rejected. Astra medium repaired the pose with unchanged clavicles and joint lengths plus limb IK. Six clips now give a low four-limbed stance, alternating Walk/Run, standing forearm swing, Hit and full-size grounded collapse. Idle is 0.889m high before tier scale; Attack rises to about 2.03m. Attack contact is 560ms in a 1140ms clip, with the existing 3000ms cadence. New body radius is 0.7142747548m. Footprint/provenance/movement are updated and the old stride/pursuit cap is removed.

Root and fresh Luna visual review passed. Lab combat showed Beetle HP 38 -> 37 -> 36, player damage, Run pursuit from 8.42m to 1.42m, then Attack with planted position. Final normal-camera evidence: `user-beetle-final-run-05.png`, `user-beetle-timed-combat-28.png`, `user-beetle-corpse-side-06.png`, plus motion samples in the continuation journal. The combat authority was ignoring authored creature timings; `HeadlessPlayer` now uses them and scales contact with recovery when cadence shortens the clip. Player/missing timing defaults remain 350/900ms. Real published-action regressions cover frog, Beetle and a faster Beetle cadence. Source, material, visual and integration Luna reviews found no remaining blocker.

Rebuild source in order: `node .../textures/build-mask.mjs`, `node .../build-candidate.mjs`, `node .../audit-motion.mjs --correct-floor`, `node .../emit-catalog.mjs`, substituting the directory above for `...`. The audit performs floor correction before catalog emission; hashing only the first builder output is insufficient. Rebuilt catalogs reset acceptance for review. Source-preview GLB/catalog and inspect-source diagnostics are disposable, not production assets. Typecheck, content validation and focused creature/combat tests passed. Production build passed, including navigation, 336 world tiles, server pack, texture optimization and bundle budgets. Authored world reached ready state with no Beetle Golem among 4,781 enemies: its planned `pack_vellenwood_gorge_watch_northeast_roots` belongs to Vellenwood, which `regionalPackActivation.ts` still holds for prior pack-placement/lifecycle issues. No region activation was broadened. Representative Beetle world placement/combat remains pending until that pack is accepted and activated.
...
| Beetle Golem / creature_beetle_golem | audit-user-beetle-golem | PROMOTED user source with limb IK, green iridescence and standing swing. Lab/root/Luna accepted. Rejected audit-mandible-family predecessor remains preserved. |

All nine original staged replacements have a disposition: eight integrated and Beetle Golem rejected, then replaced by the accepted user-supplied body. Continue staged polish and owned-source inspection while waiting for user-supplied models. The queue records missing approved Moonpetal/Scree exports and the unsuitable Cinder Ravager dragon proposal.
...
| Wild Goblin / creature_wild_goblin | audit-lab-missing | Promoted SHA 1cc4cfbcabe3d5d79af0d4db1a80923a05e3d6b9bc2c45dca99f6caae7b2c134. The mocap mid-air leap in Death is replaced by a 0.45 s topple with no hip drift; grounded corpse at 0.006 m, rest by 0.89 s. CMU mocap rig now gets spine/head hit recoil. |
| Cave Roach / creature_cave_roach | audit-lab-missing | Promoted SHA 7461f324c91792ebf7a2ce6208a757ef1bf0e8bb93eb0e6a9dc2b66c4b67470d. Death curls the legs and rolls onto its side (top 0.487 m, 42%). Contact gait 1.48 / 2.63 m/s. Hit recoil on head branch and tail; SpineHigh stays fixed for the planted front claws. |
| Troll Mauler / creature_troll_mauler | audit-lab-missing | Promoted SHA 2c3c1b2e585a86de34e9bc9bda9eb0c18a7d70a803e751f180d170a7372c20d7. Cloth alpha repaired; Death falls onto its back with every part within 2.6 cm of the floor (56% height from belly depth). Contact gait 0.72 / 1.40 m/s. Loincloth is rigid with the pelvis. |
...
| harts_ram | creature_crown_hart, fairy_garden_hart_faeholme, creature_cairn_bighorn | Promoted; final batch production validation passed |
...
| fairy_garden | fairy_garden_frog_faeholme, fairy_garden_snail_faeholme, fairy_garden_snail_gloamgarden | All three promoted; Starcap repaired corpse passed root/Luna review |
| shaman | creature_goblin_shaman | Promoted; root/Luna visuals and focused checks passed; production build passed |
| skeletons | creature_skeleton_archer, creature_skeleton_mage, creature_skeleton_soldier | Promoted; final batch production validation passed |
| earth_golems | creature_cairn_treader, creature_chalk_warden, creature_shale_elemental | All three promoted after native-hand geometry repair and root/Luna views |
...
| reaver | creature_gloamfang_reaver | Promoted; final batch production validation passed |
| rootwood | creature_starroot_guardian, creature_briar_harrow | Promoted; final batch production validation passed |
| votary | creature_cinder_penitent | Promoted; final batch production validation passed |
| drakes | creature_basalt_drake, creature_furnace_grazer | Promoted; final batch production validation passed |
| dragons | creature_amethyst_dragon, creature_purple_wilderness_dragon | Promoted; final batch production validation passed |
| bandits | outfit_female_ranger, outfit_male_ranger | Promoted; final batch production validation passed |
...
Batch four promotes Creek Crab, Cindercrest Salamander, Rimeback Tortoise, Scree Bustard and Reed Strider; earth golems (3), Crown Hart, Starhorn Hart and Starcap Snail. Root reviewed native-map rigs, normal-camera combat/motion and full-size corpses; Luna reviewed revised corpses and earth silhouettes. All 54 focused checks pass after removing retired rig cadence aliases; typecheck and content validation pass. Production build passed, including asset budgets, navigation, all 336 tiles and the server pack. Sources remain preserved. Crown b028 is preserved as sources/reviewed-crown-hart-batch4.glb while its newer material-name-only refinement awaits next batch. New creature corpses with corrected sidefalls are in new-fixed-*. No authored-world placement claim is made.

Final polish batch: dedicated Forest/Highland/Quarry Bandit bodies preserve player outfits; three elite skeleton IDs preserve low tiers; Ashbound Votary replaces only high-tier penitent encounters. Starroot and Votary material names now preserve detailed authored textures against tier tint. Bighorn uses the revised fine-wool map, Crown retains its warm coat, and Reaver/Briar keep reviewed native motion. Four drake/dragon finishes are promoted; Amethyst Dragon is named consistently. Furnace Grazer scale is 1.35, and the existing ten-member east Wilderness herd is spaced within its original 20m placement envelope to avoid overlapping the larger footprint. That spacing is authored-world integration, so its placement proof uses the full world rather than a new lab fixture. Fifty-four focused tests, typecheck and content validation pass; production build passed. The authored-world herd loaded at its intended scale with ten separated members; normal-camera screenshots and player navigation around it passed (polish-world-grazer-loaded and polish-world-grazer-interaction). The first capture preceded asset loading and was rejected.

Final delivered-base integration: independent timing and footprints replace retired rig metadata; Road Bandit uses its dedicated creature ID in world, hunt lab and regional packs, preserving player gear. Amethyst Sovereign has its own detailed crystal/flower variant. The actual Hollow Star world boss now uses the radial source rather than the earlier stone-golem stand-in. The three Hollow Bough populations are spaced within their existing envelopes, and both treant boss court reservations expand to 3m for their broader crowns. These placement changes use the authored-world exception; all bodies were first accepted in the existing creature/combat lab. Eight stale regional source-radius pins were refreshed to the current manifest; the eight existing pack tests pass. Six focused creature/content files (54 tests), typecheck and content validation pass; final production build passed, including navigation, 336 world tiles, server pack and texture/bundle budgets. Representative world checks passed: the treant courts and spaced Hollow Bough grove show coherent scale and clearance; the Thistledown population remains readable at its small fairy scale. World combat records Thistledown 106 -> 101 -> 91 -> 82 HP with Walk/Run/Attack, Sovereign 724 -> 707 with Attack, and Hollow Bough 210 -> 209 -> 196 with Attack. Evidence: new-world-{thistledown,sovereign,bough}-combat and new-world-matriarch-clear. Initial loading-screen/empty captures were rejected. These are representative checks, not an exhaustive audit of every world spawn.

Fresh Luna completion inventory confirms all 41 original polish verdicts are represented by root-accepted outputs, all fifteen assets from the thirteen delivered bases match served SHA/bytes, and all thirteen original exports remain preserved. Skeleton elite contacts now use native Attack phases. Integrated lab Road combat: HP 7 -> 6 -> 4 with Walk/Run/Attack; Sovereign 724 -> 718 -> 708 with Attack; elite soldier 134 -> 128 -> 115 with Run/Attack. Clear Road corpse supersedes the crate-occluded first capture. No renderer/streaming/startup changes were made by this thread.

Requested 41-polish / 13-delivery batch is complete. Sol workers owned separate asset directories; fresh Luna reviewers checked visuals, integration and the complete source/hash inventory. Sixty-two focused tests across seven existing files pass, with typecheck, content validation and the production build. The three pre-existing lab-only missing-model warnings remain. Five requested manual bases are still undelivered (Heron, Moose, Iron Golem, Ivory Castellan, Gorge Mantis); the 30 owned-source verdicts remain a separate queue, with known source gaps listed in creature-generation-queue.md. No model generation or Tripo job was started.
