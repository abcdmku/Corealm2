...
The final loading gate waits for selected actors, structures, vegetation, terrain, equipment, the shaders that frame draws and GPU completion of the first gameplay frame. It does not wait for the spell pools: nobody can cast before they have joined a world, and the driver compiles programs in submission order, so about 80 spell programs submitted during boot would sit in front of the scene programs the frame needs. The authored game submits them after the first frame (`Renderer.finishDeferredEffects`), polls them through `KHR_parallel_shader_compile` and reflects one per task; the span `boot.effects.ready` records when the last one is usable. Until then `SpellVfx` holds a cast's visual rather than show a pool whose program would link inside the frame, and drops it if it waited more than 600 ms, since by then the hit has landed; damage numbers and sound come from events and are not held. `tools/effects-deferral-test.ts` casts the moment the world is joined and requires every frame gap to stay under 250 ms. A feature lab still waits for the pools before it is ready, because its tests cast at once. An asynchronous WebGL fence keeps loading feedback responsive while that frame finishes. Banked items and unused equipment remain deferred. Audio does not hold up reveal.
