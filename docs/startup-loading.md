...
Local play is always worker-hosted. Before the app is imported, the entry fetches `generated/local-world.json` and the client catalog it names and installs it (`boot.catalog.install`); a failure there shows the reason and a Retry button on the boot screen. The `boot.localWorker.prepare` span then settles the local seed from the same manifest, without a second fetch. This thread loads and writes no save, has no simulation to tick, and "Play local" joins the worker's world once the first frame is drawn. The worker starts as soon as local play is the known target, so its cold start (about 4.6 s for the shipped world, nearly all of it building the world and its first entity baseline) overlaps the scene's load instead of following it. If local play cannot be prepared the picker says so on its Local row and still lists servers. See [local play in a worker](./architecture.md#local-play-in-a-worker).
...
The final loading gate waits for selected actors, structures, vegetation, terrain, equipment, all local and remote spell pools, and GPU completion of the first gameplay frame. Batched lighting makes effect programs independent of the joined world and its lights, so each enrolled effect root compiles exactly once. Spell, ambience and overlay pools enrol through `Renderer.compileEffects()`, and `Renderer.prepareEffects()` compiles them in the `boot.effects.ready` span while the world joins and the starting area prepares. Startup completes the required world assets and entities, runs bounded main-scene warmup, then awaits that effect preparation. The completed `boot.effects.ready` span must precede `boot.playable`, so the first cast has its visuals ready. `tools/effects-readiness-test.ts --url http://127.0.0.1:4180 --channel chrome` reuses a running server, casts as soon as the saved local world is joined, requires particles on both the first and later casts, and keeps first-cast frame gaps at or below 250 ms. Banked items and unused equipment remain deferred. Audio does not hold up reveal.

Authored spell PNG downloads start through `Renderer.prepareEffectTextures()` before the world join. Their download and decode must finish before effect compilation. Feature labs await these textures before main warmup because their starting scene can already display an effect. Portal transitions call `Renderer.prepareEffects()` again before uncovering the destination, which compiles only roots enrolled since startup.

Glow, antialiasing, colour grading and player visibility prepare their actual native pipelines during graphics setup. Smoke, spark and dust batches compile while still hidden, before their first emission. Preparation retains the real mesh identities, vertex layouts and target formats. Native glow reuses matching base preparation and the main frame's depth; refraction prepares its own layer against the main target. `tools/magic-glow-compile-test.ts` checks native emitter preparation, a visible halo and unchanged pixels when a wall occludes the emitter.

Native pipeline creation, texture uploads and geometry uploads run in bounded batches. Startup allows eight pipeline creations in flight and fences its uploads once, at the end of each preparation job; gameplay retains one pipeline at a time and a fence per batch. Effect preparation tokens reuse completed work only when the real mesh, material, scene, camera and target still match. Spell pools share shader graphs by recipe, with clocks resolved per draw and private instance attributes for each pool. All local and remote pool capacities remain unchanged, and every pool's buffers and bindings prepare before play. The readiness gate checks graphics validation as well as completion, so a rejected pipeline cannot count as ready. Loading feedback remains responsive while that work finishes. Current evidence belongs to the [native WebGPU measurements](./runtime-performance.md#native-webgpu-migration); the dated measurements below describe earlier releases.

Every eight metres of travel starts preparation 48 metres beyond the working set. Visited sources stay reusable. Portal transitions, and same-map jumps beyond the 64 m actor radius such as a respawn, await complete destination preparation and rendering before uncovering the view. Very slow downloads can still overrun the travel margin.
