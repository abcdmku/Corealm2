...
12. Follow [docs/item-icons.md](./docs/item-icons.md) for item art. Every item icon comes from prompted GPT image generation (GPT Image 2.5), with no exceptions. Never create procedural items or use renders or screenshots of 3D models for inventory icons. Review the icon first; derive a 3D asset from it only when gameplay requires one. Minor jewelry, including rings and earrings, needs no dedicated 3D asset. Consolidate duplicate items before generating replacements.
