...
try {
  ...
  if (scene === "fishing") {
    ...
    await clearPanels();
    for (const [name, key, fraction] of [["01-run-back", "w", 0.2], ["01-run-front", "s", 0.5], ["01-run-sword-side", "d", 0.35], ["01-run-shield-side", "a", 0.7]] as const) {
      await frame(0.2, 0.12, 4.2);
