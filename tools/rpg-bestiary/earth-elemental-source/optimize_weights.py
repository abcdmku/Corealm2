...
p=Path('tools/rpg-bestiary/earth-elemental-source/derived/source.json');s=json.loads(p.read_text());mat=np.array(json.loads(Path('test-results/earth-elemental-source/skin-matrices.json').read_text())).reshape((-1,63,4,4)).transpose(0,1,3,2)
...
s['weightReduction']['method']='Select four of native influences and nonnegative refit across 85 poses of five native runtime clips, minimizing maximum positional error plus 0.25 RMS; no new joints';s['weightReduction']['refitVertices']=changed
