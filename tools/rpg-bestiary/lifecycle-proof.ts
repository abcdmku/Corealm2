...
try{
 ...
 assert(dead.game.skills.melee.xp>report.beforeKill.game.skills.melee.xp,'No kill XP after fixture level setup');assert((await page.evaluate(()=>(window as any).__gameDebug.getEvents(0).events.some((event:any)=>event.type==='item.received'&&Array.isArray(event.data?.items)&&event.data.items.some((stack:any)=>stack.itemId==='gold'&&stack.quantity>0)))),'No gold in the kill loot pile');
