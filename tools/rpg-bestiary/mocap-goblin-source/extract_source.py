...
out=Path('tools/rpg-bestiary/mocap-goblin-source/derived');out.mkdir(parents=True,exist_ok=True)
original=Path('test-results/fantasy-collection-source/goblin-animated-by-motion-capture/Goblin.blend')
bpy.ops.wm.open_mainfile(filepath=str(original.resolve()),load_ui=False,use_scripts=False)
rig=bpy.data.objects['GoblinArmature'];mesh=bpy.data.objects['Goblin'];strips=[{'name':s.name,'action':s.action.name,'start':s.action_frame_start,'end':s.action_frame_end,'timelineStart':s.frame_start,'timelineEnd':s.frame_end} for track in rig.animation_data.nla_tracks for s in track.strips if s.name in ['Idle','Walk','Flee','Attack1','Attack2','Die']];rig.animation_data_clear();bpy.context.scene.frame_set(0)
...
data={'name':mesh.name,'positions':[],'normals':[],'uvs':[],'skinIndices':[],'skinWeights':[],'fullWeights':[],'sourceControlIndices':[],'materialIndices':[],'joints':[boneids[b.name] for b in bones],'inverseBindMatrices':[mat((rig.matrix_world@b.matrix_local).inverted()@mesh.matrix_world) for b in bones]}
...
for tri in mesh.data.loop_triangles:
 data['materialIndices'].append(tri.material_index)
 uv=mesh.data.uv_layers['UVMap-Knife' if tri.material_index==1 else 'UVMap-goblin'].data
...
result['sourceFacts']={'controlVertices':len(mesh.data.vertices),'triangles':len(mesh.data.loop_triangles),'bones':len(bones),'armaturePreserveVolume':mesh.modifiers[0].use_deform_preserve_volume,'uvLayers':[u.name for u in mesh.data.uv_layers],'activeUV':mesh.data.uv_layers.active.name,'materials':[m.name for m in mesh.data.materials],'animationNames':[a.name for a in bpy.data.actions]}
