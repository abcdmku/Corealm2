 ...
 * this checkout's formulas, so it can never lag the code the way a committed artifact can. Loads no
 * content table, so it is safe to call before `installCatalog`.
 */
export async function repoBaseCatalog(): Promise<BaseCatalog> {
  ...
  return { catalog, sources: Object.fromEntries(values) };
}
