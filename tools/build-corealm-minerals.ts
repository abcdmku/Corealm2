...
const GEMS = {
  ...
  amber: { colour: 0xffdba0, roughness: 0.11, transmission: 0.78, ior: 1.54, attenuation: 0xd99531, distance: 0.19 },
  garnet: { colour: 0xe5a1af, roughness: 0.10, transmission: 0.91, ior: 1.79, attenuation: 0x9f1838, distance: 0.17 },
  opal: { colour: 0xf5d6aa, roughness: 0.085, transmission: 0.78, ior: 1.45, attenuation: 0xe7aa78, distance: 0.22 },
} as const;
...
class MineralShape {
  ...
  ore(): void {
    ...
    for (const cell of cells) {
      ...
      this.triangle([vertex(cell[2],false),vertex(cell[1],false),vertex(cell[0],false)], kind === "emberite" ? "rind" : "host");
      for (let i=0;i<3;i++) {
  ...
  opalMatrix(frame: Matrix4): void {
    ...
    const angles=[...Array.from({length:80},(_,i)=>i/80*tau),...outline.map(p=>(Math.atan2(p.y-centre.y,p.x-centre.x)+tau)%tau)]
    ...
    const contour=angles.map(angle=> {
      ...
      const radius=(1+0.022*Math.sin(angle*3+0.4)+0.012*Math.cos(angle*5))/Math.hypot(direction.x/0.57,direction.y/0.66);
    ...
    const vertex=(section:Section,front:boolean):V=> {
      const p=section.p;
      const z=front ? section.inside ? 0.31+0.46*Math.pow(Math.max(0,1-section.t*section.t),0.70)
        : 0.31+(host(p)-0.31)*section.t
...
export async function buildMineralItemAsset(assetId: string): Promise<{ glb: Uint8Array; entry: MineralItemEntry }> {
  ...
  for (const surface of ["host", "mineral", "rind", "gem", "cloud", "inclusion", "film"] as const) {
    ...
    if (surface === "gem" && spec.kind === "opal") {
      material.setExtension("KHR_materials_iridescence", document.createExtension(KHRMaterialsIridescence).createIridescence()
        .setIridescenceFactor(0.83).setIridescenceIOR(1.46).setIridescenceThicknessMinimum(180).setIridescenceThicknessMaximum(425));
    }
    const positions: number[] = [], normals: number[] = [], colours: number[] = [], uv: number[] = [];
