...
/** New deer leg cycles at the existing duration and native stride speed. */
export function authorCervineGait(doc: Document, name: 'Walk' | 'Run', nativeMps: number, floorY: number): BakedGait {
  const rest = storedPose(doc), nodes = doc.getRoot().listNodes();
  const requireNode = (suffix: string): Node => { const found = nodes.filter(node => node.getName() === `Deer_${suffix}SHJnt`); if (found.length !== 1) throw new Error(`Missing cervine ${suffix}`); return found[0]!; };
  const clip = doc.getRoot().listAnimations().find(clip => clip.getName() === name)!;
  const seconds = duration(clip), root = requireNode('ROOT'), rootPosition = worldPosition(root), skin = createSkinReader(doc, 'deer_body');
  const duty = name === 'Walk' ? .6 : .30, crouch = name === 'Walk' ? .12 : .20;
  const legs = ['l_FrontLeg', 'r_FrontLeg', 'l_HindLeg', 'r_HindLeg'].map((prefix, index) => {
    ...
    if (b.getParentNode() !== a || end.getParentNode() !== b || ankle.getParentNode() !== end) throw new Error(`Cervine ${prefix} chain changed`);
    ...
    return { prefix, a, b, end, ankle, vertices, primary, center, offset: pad.clone().sub(distal), distalQ: worldQuaternion(end), ankleQ: worldQuaternion(ankle), pole,
      l1: start.distanceTo(knee), l2: knee.distanceTo(distal), phaseOffset: name === 'Walk' ? [.25, .75, 0, .5][index]! : [0, .06, .5, .56][index]!, minimumReachMargin: Infinity };
  });
  const specs = nodes.filter(node => node.getName() && nodes.filter(other => other.getName() === node.getName()).length === 1).flatMap(node => (['translation', 'rotation', 'scale'] as const).map(path => ({ node, path })));
  ...
  try {
    for (const time of times) {
      ...
      for (const leg of legs) {
        ...
        if (skin.point(leg.primary).distanceTo(target) > 1e-6) throw new Error('Cervine physical pad did not converge');
      }
    ...
    return { name, seconds, nativeMps, tracks, feet: legs.map(leg => ({ name: leg.prefix, vertices: leg.vertices, primaryVertices: [leg.primary], phaseOffset: leg.phaseOffset, duty, clearance: .0005 })),
      notes: ['Four-beat walk and paired fore/hind run. Source geometry, joint lengths, clip duration and measured native speed preserved.', 'Body compression permits full existing stroke without stretching joints. Physical weighted soles are audited after serialization.'],
