...
import { authorBovineGait } from './bovine-gait.js';
...
for (const id of process.argv.slice(2).length ? process.argv.slice(2) : ['animal_cattle', 'animal_aurochs']) {
  ...
  if (sha(source) !== entry.sha256) throw new Error(`Public source hash mismatch ${id}`);
  const doc = await io.readBinary(source), floorY = entry.groundY ?? entry.base.y;
  const gaits = (['Walk', 'Run'] as const).map(name => authorBovineGait(doc, name, name === 'Walk' ? entry.impliedWalkMps : entry.impliedRunMps, floorY));
  const bytes = appendGaitAnimations(source, gaits), candidate = await io.readBinary(bytes);
  const audit = gaits.map(gait => auditGroundGait(candidate, gait, 'Cow_Mesh', floorY, 7680));
