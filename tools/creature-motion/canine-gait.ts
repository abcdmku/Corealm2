/** Offline physical-paw authoring for the shipped wolf and bear skeletons. */
...
export function authorCanineGait(doc: Document, id: string, name: 'Walk' | 'Run', seconds: number, floorY: number, nativeMps: number): BakedGait {
  if (!['animal_coyote', 'animal_bear'].includes(id) || !(seconds > 0) || !(nativeMps > 0) || ![seconds, floorY, nativeMps].every(Number.isFinite)) throw new Error('Invalid canine gait inputs');
  const wolf = id === 'animal_coyote', prefix = wolf ? 'Wolf' : 'Bear', mesh = wolf ? 'Wolf_Mesh' : 'brown_bea4';
  ...
  const root = find(`${prefix}_ROOTSHJnt`), rootPosition = worldPosition(root);
  const duty = name === 'Walk' ? .56 : .22, clearance = .0005, samples = 3840;
  const crouch = wolf ? -.14 : -.20;
  const feet = ['l_FrontLeg', 'r_FrontLeg', 'l_HindLeg', 'r_HindLeg'].map((suffix, index) => {
    ...
    const upper = find(`${base}HipSHJnt`), lower = find(`${base}${wolf && rear ? 'Knee1' : 'Knee'}SHJnt`), end = find(`${base}${wolf && rear ? 'Knee2' : 'Ankle'}SHJnt`), ankle = find(`${base}AnkleSHJnt`);
    ...
    const all = skin.indicesForBranches([ankle.getName()], Infinity), minimum = Math.min(...all.map(i => skin.restPoints[i]!.y));
    const vertices = all.filter(i => skin.restPoints[i]!.y <= minimum + .005), primaryVertices = vertices.filter(i => skin.restPoints[i]!.y <= minimum + .0003);
    if (!vertices.length || !primaryVertices.length) throw new Error('Missing physical paw');
    const center = c.clone(); center.y += floorY + clearance - minimum; center.z = a.z;
  ...
  try {
    for (let i = 0; i <= samples; i++) {
      ...
      setWorldPosition(root, rootPosition.clone().add(new Vector3(0, crouch + .008 * Math.cos(4 * Math.PI * phase), 0)));
      for (const foot of feet) {
        const localPhase = fract(phase - foot.phaseOffset);
        const path = cyclicFootPath(localPhase, duty, seconds, nativeMps, name === 'Walk' ? .045 : .055);
        if (!path.contact) {
