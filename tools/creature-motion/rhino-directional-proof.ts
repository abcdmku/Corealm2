...
try {
  ...
  for (const id of selected) for (const side of ['front', 'left', 'right'] as const) {
    ...
    for (let attempt = 1; attempt <= 2; attempt++) {
      ...
      try {
        ...
        const lateral = side === 'left' ? -standoff : side === 'right' ? standoff : 0, forward = side === 'front' ? standoff : 0;
        const destination = [x + lateral * Math.cos(heading) + forward * Math.sin(heading), y,
          z - lateral * Math.sin(heading) + forward * Math.cos(heading)];
        await call('debug', 'callTool', ['corealm_move_to', { position: destination }]);
        const arrival = performance.now() + 5_000;
        while (performance.now() < arrival) {
          const sample = await snap(entityId); trial.samples.push({ ...sample, stage: 'navigation' });
          if (Math.hypot(sample.player.position.x - destination[0]!, sample.player.position.z - destination[2]!) < .25) break;
          await page.waitForTimeout(50);
        }
        trial.positioned = await snap(entityId);
        if (Math.hypot(trial.positioned.player.position.x - destination[0]!, trial.positioned.player.position.z - destination[2]!) >= .25) throw new Error('Side navigation did not arrive');
        ...
        while (performance.now() < untilHit) {
          ...
          if (sample.motion.motion === 'hit' && sample.entity.combat.health < healthBefore) {
        ...
        trial.status = recorded && trial.hit.motion.clip === expected ? 'natural-clip-observed-awaiting-visual-review' : 'coverage-incomplete';
  ...
  report.coverage = selected.map(id => ({ id, clips: ['Hit', 'HitLeft', 'HitRight'].map(clip => ({ clip,
    observed: report.trials.some((trial: any) => trial.id === id && trial.hit?.motion?.clip === clip) })) }));
