...
import { authorSuidGait } from './suid-gait.js';

const out = 'art/rebuild/candidates/finish-motion/legacy-suids'; await mkdir(out, { recursive: true });
...
const generatorSha256 = Object.fromEntries(await Promise.all(['tools/creature-motion/suid-gait.ts', 'tools/creature-motion/stage-suid-gaits.ts', 'tools/creature-motion/pose.ts', 'tools/lib/ground-gait.ts', 'tools/repair-ground-creature-gaits.ts'].map(async file => [file, await generatorFileSha256(file)])));
for (const id of process.argv.slice(2).length ? process.argv.slice(2) : ['animal_boar', 'animal_hog']) {
  if (!['animal_boar', 'animal_hog'].includes(id)) throw new Error('Suid helper owns only cattle/aurochs');
  ...
  const gaits = (['Walk', 'Run'] as const).filter(name => doc.getRoot().listAnimations().some(clip => clip.getName() === name)).map(name => authorSuidGait(doc, id, name, name === 'Walk' ? entry.impliedWalkMps : entry.impliedRunMps, floorY));
  const bytes = appendGaitAnimations(source, gaits), candidate = await io.readBinary(bytes);
  const audit = gaits.map(gait => auditGroundGait(candidate, gait, id === 'animal_boar' ? 'WildBoar_mesh' : 'miko34', floorY, 7680));
