...
try{await driver.launch();await driver.open(60000,'/index.html?mode=combat&environment=1&atmosphere=1');const page=driver.page!;
 ...
 await page.evaluate(async()=>{await (window as any).__environmentLab.showFoliage('corealm_deadwood_1',{layout:'grid',count:12,span:32,variants:['corealm_deadwood_1','corealm_deadwood_2']});await (window.__gameDebug as any).inspectPose({x:0,y:2,z:25,yaw:.25,pitch:.17,distance:30,detached:true});});
