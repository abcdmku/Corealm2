...
for (const id of ['fairy_rounded_bank_0','fairy_rounded_bank_1']) {
 const doc=await new NodeIO().registerExtensions([EXTTextureWebP]).read(`test-results/fairy-terraces-assets/rounded-banks/models/${id}.glb`);
 ...
 const camera=new PerspectiveCamera(40,W/H,.1,100);camera.position.set(Math.sin(angle+.5)*13,6,Math.cos(angle+.5)*13);camera.lookAt(0,1.3,0);camera.updateMatrixWorld(true);
 ...
 const projected=[];for(let i=0;i<positions.length;i+=3)projected.push(project(positions[i],positions[i+1]-.08,positions[i+2]));
 ...
 const label=`<svg width="${W}" height="${H}"><text x="20" y="35" font-family="sans-serif" font-size="22" fill="white">${id}: rounded native outcrop; CPU material/silhouette preview, not gameplay acceptance</text></svg>`;
 await sharp(pixels,{raw:{width:W,height:H,channels:3}}).composite([{input:Buffer.from(label)}]).png().toFile(`test-results/fairy-terraces-assets/rounded-banks/${id}-${Math.round(angle*180/Math.PI)}.png`);
 }
}
