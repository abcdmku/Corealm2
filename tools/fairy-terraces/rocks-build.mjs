/** Original BK LOD0 boulders and source Unity top-down material baked into their unchanged UVs. */
...
const out = 'test-results/fairy-terraces-assets/rocks';
...
async function readTexture(file) {
  const { data, info } = await sharp(file).resize(2048, 2048, { fit: 'inside', withoutEnlargement: true })
...
for (const number of [4, 0]) {
  const id = `fairy_boulder_${number}`, modelFile = `${base}/Models/Boulders/Boulder_${number}.fbx`;
  const matFile = `${base}/Models/Boulders/Textures/Materials/Boulder_${number}.mat`;
  ...
  const triangles = pos.count / 3, expected = number === 4 ? 3028 : 1390;
  ...
  const sourceBounds = new Box3().setFromBufferAttribute(pos), midpoint = sourceBounds.getCenter(new Vector3()).multiplyScalar(.01);
  const shift = [-midpoint.x, -sourceBounds.min.y * .01, -midpoint.z];
  const positions = Float32Array.from(originalPositions, (x, i) => x + shift[i % 3]);
  ...
  for (let triangle = 0; triangle < triangles; triangle++) {
    ...
    for (let y = minY; y <= maxY; y++) for (let x = minX; x <= maxX; x++) {
      ...
      // Exact source TOPDOWN expression. The unresolved grass normal uses Unity's flat bump default.
      const alpha = Math.min(1, Math.max(0, n[1] + materialParameters._LayerPower)) ** (.001 + materialParameters._LayerThreshold * .999);
      ...
      for (let k = 0; k < 3; k++) albedo[offset + k] = srgb(rock[k] * (1 - alpha) + grass[k] * alpha);
  ...
  const material = document.createMaterial(`BK ${id} original rock and top-down Grass01`)
  ...
  const outputMesh = document.createMesh(`Boulder_${number}_LOD0`).addPrimitive(primitive);
  ...
  if (bytes.length >= 3_000_000) throw Error(`${id}: size budget exceeded ${bytes.length}`);
  ...
  const sources = await Promise.all([modelFile, `${modelFile}.meta`, matFile,
    `${base}/Prefabs/Boulders/Boulder_${number}.prefab`, ...Object.values(textureFiles)].map(async path => ({ path, sha256: hash(await readFile(path)) })));
  const asset = { id, file: `models/fairy/${id}.glb`, pack: pack.id, category: 'rock', is: 'boulder',
    tags: ['rock', 'boulder', 'fairy', 'source-derived', 'top-down-grass'], bytes: bytes.length, sha256: hash(bytes),
    ...
    animations: [], materials: [material.getName()], sourceProvenance: {
      ...
      modifications: ['Full original LOD0. No decimation, deformation, replacement geometry or UV changes.',
        'Centimetres to metres; rigid translation centres XZ and grounds minimum Y.',
        'Source Unity top-down Grass01, rock albedo, rock normal, triplanar detail normal and masks baked at native scale into unchanged UVs.',
        '2048 WebP source-material atlases; source material smoothness and occlusion powers retained.'],
      materialParameters, unresolvedSourceReference: 'Original grass normal GUID f3854bd7648c4b2e825b724bd7450464 is absent from the supplied archive. Unity flat bump default retained.',
  ...
  const audit = { asset, sourceTriangleCount: triangles, outputTriangleCount: triangles,
    ...
    bake: { side, shadedPixels: shaded, overlappingPixels: overlap, mossPixels, mossWeightMean: mossWeight / shaded,
      shaderFormula: 'pow(saturate(normal.y + _LayerPower), .001 + _LayerThreshold * .999)', textures: texturesAudit },
  };
