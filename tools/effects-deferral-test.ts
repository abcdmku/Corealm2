...
/**
 * A spell cast the moment the game is playable, while the spell pools' shader programs may still be compiling.
 *
 *   tsx tools/effects-deferral-test.ts            the dev server
 *   tsx tools/effects-deferral-test.ts --dist     the production build in game/dist
 *
 * Boot leaves the spell programs until after the first frame (`Renderer.finishDeferredEffects`), and `SpellVfx` holds a
 * cast's visual until they are usable. The first visit sets a character up beside a creature and stores it. The second
 * visit, a cold page in the same profile with the GPU shader cache off, casts as soon as the world is joined and watches
 * the frames: no frame may stall on a program link, the span `boot.effects.ready` must end after first playable, and once
 * the programs are ready a cast must draw particles. Ports 4350 and 4351, or `COREALM_TEST_PORT`.
 */
const dist = process.argv.includes("--dist");
const out = path.join(repoRoot, "test-results/effects-deferral"); await mkdir(out, { recursive: true });
const clearDeadline = installTestDeadline("effects deferral", 420_000);
...
const browser = await chromium.launch({ headless: true, args: ["--enable-gpu", "--ignore-gpu-blocklist", "--mute-audio", "--disable-gpu-shader-disk-cache",
...
try {
  ...
  await setup.goto(`${server.url}/?play=local`); await joined(setup);
  ...
  // ---- 2. A cold page: cast as soon as the world is joined, and watch every frame around it.
  ...
  await page.goto(`${server.url}/?play=local`); await joined(page);
  const cast = await page.evaluate(async (entityId) => {
    ...
    let firstParticlesMs: number | null = null, readyMs: number | null = readyAtCast ? 0 : null;
    while (performance.now() - started < 12_000) {
      await new Promise(resolve => requestAnimationFrame(resolve));
      if (readyMs === null && lab.shaders()?.effectsReady) readyMs = performance.now() - started;
      if (firstParticlesMs === null && debug.getMetrics().spellParticles > 0) firstParticlesMs = performance.now() - started;
      if (firstParticlesMs !== null && readyMs !== null && performance.now() - started > 3000) break;
    }
    ...
    return { readyAtCast, playableMs, castAtMs: started, result, firstParticlesMs, readyMs, maxGapMs, frames: gaps.frames.length,
      effectsReadySpan: span ? { endMs: span.endMs, detail: span.detail } : null };
  }, target);
  notes.cast = cast;
  checks.castAccepted = !(cast.result as { error?: string })?.error;
  checks.noFrameStall = cast.maxGapMs < 250;
  checks.effectsReadySpanAfterPlayable = cast.effectsReadySpan !== null && cast.effectsReadySpan.endMs > cast.playableMs;
  checks.programsBecameReady = cast.readyMs !== null;
  // ---- 3. With the programs ready, a cast draws the real effect.
  ...
  notes.later = later;
  checks.readyCastDrawsParticles = later.particles > 0;
...
const passed = errors.length === 0 && Object.keys(checks).length === 5 && Object.values(checks).every(Boolean);
const report = { passed, mode: dist ? "dist" : "dev", checks, notes, errors };
