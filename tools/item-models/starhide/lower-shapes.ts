...
export function ribbon(g: THREE.Group, name: string, fn: Surface, path: (t: number) => [number, number],
  material: THREE.Material, width = .0027, segments = 64, lift = .001) {
  const p: number[] = [], uv: number[] = [], ix: number[] = [];
  const primary = material.name.includes('polished silver') && width >= .0018;
  if (primary) width *= 1.45;
  const cross = primary ? [-1, 0, 1] : [-1, 1], stride = cross.length;
  ...
  for (let i = 0; i <= segments; i++) {
    ...
    for (const s of cross) {
      const q = center.clone().addScaledVector(side, width * s / 2).addScaledVector(n, primary ? .0008 * (1 - s * s) : 0);
