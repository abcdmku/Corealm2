...
function texture(kind: 'metal' | 'leather' | 'normal' | 'rough'): THREE.DataTexture {
  ...
  for (let y = 0; y < n; y++) for (let x = 0; x < n; x++) {
    ...
    const heat = Math.pow(Math.max(0, wave), 5), violet = Math.pow(Math.max(0, Math.sin(x * .032 + y * .012 + 1.4)), 10);
    const scratch = Math.abs(Math.sin(x * .47 + y * .79 + Math.sin(y * .037) * 4)) < .018;
    let c: number[];
    if (kind === 'normal') c = [128 + 4 * Math.sin(x * .22) * Math.cos(y * .18), 128 + 4 * Math.cos(x * .22) * Math.sin(y * .18), 255];
    else if (kind === 'rough') c = [170 + grain * 45 + heat * 18, 170 + grain * 45 + heat * 18, 170 + grain * 45 + heat * 18];
    else if (kind === 'leather') c = [43 + grain * 15, 29 + grain * 11, 22 + grain * 9];
    else { const k = .88 + grain * .2 + (scratch ? .2 : 0); c = [(123 + heat * 46 + violet * 8) * k, (141 - heat * 7 - violet * 22) * k, (158 - heat * 35 + violet * 7) * k]; }
...
export const author: ItemModelAuthor = {
  ...
  build(id) {
    ...
    function mesh(name: string, g: THREE.BufferGeometry, m: THREE.Material | THREE.Material[]) {
      const a = new THREE.Mesh(g, m); a.name = name; a.castShadow = a.receiveShadow = true;
      const materials = Array.isArray(m) ? m : [m], hard = materials.some(v => v === metal || v === trim || v === brass || v === opal);
      const side = /--1(?:-|$)/.test(name) ? 'r' : 'l';
      if (hard) {
        if (id === 'emberite_helm') a.userData.itemModelBone = 'Head';
        if (id === 'emberite_plate') a.userData.itemModelBone = /inner-shoulder-cap/.test(name) ? 'spine_03' : /pauldron|shoulder-[-1]/.test(name) ? `upperarm_${side}` : /fauld/.test(name) ? 'pelvis' : /plackart|side-buckle|side-fastener/.test(name) ? 'spine_01' : 'spine_03';
        if (id === 'emberite_greaves') a.userData.itemModelBone = `${/^thigh/.test(name) ? 'thigh' : 'calf'}_${side}`;
        if (id === 'emberite_boots') a.userData.itemModelBone = `${/cuff/.test(name) ? 'calf' : 'foot'}_${side}`;
        if (id === 'emberite_gauntlets' && /cuff/.test(name)) a.userData.itemModelBone = `lowerarm_${side}`;
        if (id === 'emberite_gauntlets' && /metacarpal|knuckle/.test(name)) a.userData.itemModelBone = `hand_${side}`;
      }
      if (id === 'emberite_plate' && /side-leather-strap/.test(name)) a.userData.itemModelBone = 'spine_01';
      if (id === 'emberite_plate' && /shoulder-bridge-hide/.test(name)) a.userData.itemModelBone = 'spine_03';
      if (id === 'emberite_greaves' && /strap/.test(name)) a.userData.itemModelBone = `${/^thigh/.test(name) ? 'thigh' : 'calf'}_${side}`;
      if (id === 'emberite_boots' && /leather-foot|outsole|heel|welt/.test(name)) a.userData.itemModelBone = `foot_${side}`;
      if (id === 'emberite_boots' && /open-leather-shaft/.test(name)) a.userData.itemModelBone = `calf_${side}`;
      root.add(a); return a;
    }
    ...
    function edgedShell(name: string, s: Surface, nx = 24, ny = 6, mat: THREE.Material = metal, radius = .002) {
      shell(name, s, nx, ny, .004, mat);
      // Silver edge is a raised broad forged band, not only a thin wire outline.
      if (mat === metal) for (const edge of [0, 1]) shell(`${name}-raised-silver-band-${edge}`, (u, v) => {
        const t = edge === 0 ? v * .09 : 1 - v * .09, p = s(u, t);
        const du = s(Math.min(1, u + .001), t).sub(s(Math.max(0, u - .001), t));
        const dv = s(u, Math.min(1, t + .001)).sub(s(u, Math.max(0, t - .001)));
        return p.addScaledVector(du.cross(dv).normalize(), .0018);
      }, nx, 1, .0012, trim);
    ...
    function plate(name: string, boundary: P[], center: P, bevel = .009, mat: THREE.Material = metal) {
      ...
      const inside = vertices.map(p => p.clone().lerp(c, bevel * 1.25 / Math.max(.02, p.distanceTo(c))).add(V(0, 0, .002)));
    ...
    function limb(name: string, x: number, low: number, high: number, r0: number, r1: number, z = -.036) {
      const s: Surface = (u, v) => { const a = -.97 * Math.PI + u * 1.94 * Math.PI, r = r0 + (r1 - r0) * v;
        const calf = name.startsWith('shin'), rear = Math.cos(a) < 0;
        const depth = r * .94 + (calf ? Math.sin(v * Math.PI) * (rear ? .035 : .012) : 0);
        return V(x + Math.sin(a) * r, low + (high - low) * v, (calf ? -.064 : z) + Math.cos(a) * depth);
      };
      edgedShell(`${name}-hollow-wrap`, s, 32, 7);
      // The pad follows the SAME rigid segment as its metal and sits 14mm inside.
      // Extend its wrap over the narrow rear seam without growing its silhouette.
      const backing = shell(`${name}-inset-rigid-leather-backing`, (u, v) => {
        const t = (u * 2 - .03) / 1.94, a = -Math.PI + u * Math.PI * 2;
        return s(t, v).add(V(-Math.sin(a) * .014, 0, -Math.cos(a) * .014));
      }, 32, 10, .003, leather);
      const side = x > 0 ? 'l' : 'r';
      backing.userData.itemModelBone = `${name.startsWith('thigh') ? 'thigh' : 'calf'}_${side}`;
    ...
    if (id === 'emberite_plate') {
      root.userData.itemModel.bodyCoverage = [{ region: 'torso', minY: .905, maxY: 1.385 }];
      // Separate inset pads move rigidly with their plates. Only the narrow waist
      // gusset bends; a full blended torso would swell through the rigid cuirass.
      const chestPad = shell('chest-back-inset-rigid-leather-backing', (u, v) => { const a = u * Math.PI * 2, y = 1.025 + v * .365, t = Math.min(1, (y - 1.025) / .357); return V(Math.sin(a) * (.127 + .045 * Math.sin(t * Math.PI * .68)), y, -.014 + Math.cos(a) * (.1 + .043 * Math.sin(t * Math.PI * .8))); }, 48, 14, .004, leather);
      chestPad.userData.itemModelBone = 'spine_03';
      const skirtPad = shell('fauld-inset-rigid-leather-backing', (u, v) => { const a = u * Math.PI * 2; return V(Math.sin(a) * .14, .9 + v * .15, -.024 + Math.cos(a) * .104); }, 40, 7, .004, leather);
      skirtPad.userData.itemModelBone = 'pelvis';
      shell('recessed-flexible-waist-gusset', (u, v) => { const a = u * Math.PI * 2; return V(Math.sin(a) * .116, .995 + v * .09, -.02 + Math.cos(a) * .084); }, 32, 8, .004, leather);
      for (const side of [-1, 1]) {
        const bridge = shell(`leather-upper-back-shoulder-bridge-${side}`, (u, v) => V(side * (.065 + u * .102), 1.354 + v * .111, -.128 + v * .039), 8, 6, .005, leather); bridge.userData.itemModelBone = 'spine_03';
        for (let j = 0; j < 7; j++) line(`lining-${side}-saddle-stitch-${j}`, [V(side * .141, 1.04 + j * .037, .108), V(side * .141, 1.054 + j * .037, .108)], .0013, stitch);
      }
      ...
      for (const side of [-1, 1]) {
        // Fixed inner cap closes the top of the shoulder while the four lower
        // lames swing on the upper arm. Their 35mm overlap is deliberate.
        edgedShell(`inner-shoulder-cap-${side}`, (u, v) => {
          const a = .02 + (side > 0 ? 1 - u : u) * (Math.PI - .04);
          return V(side * (.133 + v * .127), 1.461 + Math.sin(a) * (.112 - v * .004), -.065 + Math.cos(a) * .106);
        }, 28, 5);
        for (let j = 0; j < 4; j++) {
        ...
        plate(`pauldron-${side}-forged-angular-front-cap`, [[side * .153, 1.477, .035], [side * .177, 1.551, -.011], [side * .211, 1.576, -.027], [side * .274, 1.544, .004], [side * .285, 1.477, .032], [side * .229, 1.449, .059]], [side * .219, 1.517, .069], .009);
        for (const x of [.182, .265]) stud(`pauldron-${side}-cap-rivet-${x}`, [side * x, 1.483, .05], .005);
        // Rear shoulder cap gives the same armored volume in the walking view.
        const rearCap = plate(`pauldron-${side}-rear-cap`, [[side * .157, 1.474, .034], [side * .179, 1.549, -.009], [side * .211, 1.572, -.025], [side * .269, 1.54, .003], [side * .279, 1.475, .035], [side * .224, 1.447, .055]], [side * .22, 1.514, .065], .008); rearCap.scale.z = -1; rearCap.position.z = -.13;
    ...
    if (id === 'emberite_greaves') {
      root.userData.itemModel.bodyCoverage = [{ region: 'legs', minY: .165, maxY: .965 }];
      const hipPad = shell('inset-pelvic-leather-gusset', (u, v) => { const a = u * Math.PI * 2; return V(Math.sin(a) * (.16 + v * .013), .85 + v * .12, -.035 + Math.cos(a) * .098); }, 40, 8, .004, leather); hipPad.userData.itemModelBone = 'pelvis';
      for (const side of [-1, 1]) {
        const x = side * .1143;
        shell(`recessed-flexible-knee-gusset-${side}`, (u, v) => { const a = u * Math.PI * 2; return V(x + Math.sin(a) * .054, .484 + v * .127, -.042 + Math.cos(a) * .057); }, 28, 12, .004, leather);
        ...
        // Rear plates follow the native calf's aft bulge rather than a straight tube.
        const rearCalf = plate(`shin-${side}-rear-anatomical-panel`, [[x - .052, .465, .137], [x, .491, .136], [x + .052, .465, .137], [x + .045, .267, .142], [x + .032, .171, .109], [x - .032, .171, .109], [x - .045, .267, .142]], [x, .342, .168], .006); rearCalf.scale.z = -1;
        const rearThigh = plate(`thigh-${side}-rear-cuisse-panel`, [[x - .069, .901, .091], [x, .925, .111], [x + .069, .901, .091], [x + .06, .657, .105], [x, .614, .111], [x - .06, .657, .105]], [x, .761, .144], .007); rearThigh.scale.z = -1;
