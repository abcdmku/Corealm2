...
function grainTexture(leather = false): THREE.DataTexture {
  ...
  for (let y = 0; y < size; y++) for (let x = 0; x < size; x++) {
    ...
    const k = leather ? .68 + fine * .20 + coarse * .17 : .77 + coarse * .22 + fine * .027 - (scratch ? .07 : 0);
    const color = leather ? [57, 34, 24] : [239, 145, 91];
...
export const author: ItemModelAuthor = {
  ...
  build(itemId) {
    ...
    root.userData.itemModel = {
      ...
      wearable: true,
      bodyCoverage: [{ region: 'torso', minY: .971, maxY: 1.284 }],
    };
    const copperMap = grainTexture();
    const hideMap = grainTexture(true);
    const copper = new THREE.MeshPhysicalMaterial({ name: 'forged-hammered-copper', map: copperMap, metalness: .84, roughness: .32, clearcoat: .06, clearcoatRoughness: .32, normalMap: hammeredNormal() });
    ...
    const mesh = (name: string, geometry: THREE.BufferGeometry, material: THREE.Material | THREE.Material[]) => {
      ...
      const outerMaterial = Array.isArray(material) ? material[0] : material;
      const rigidMetal = outerMaterial === copper || outerMaterial === rim || outerMaterial === brass;
      // The hem is a flange on this breastplate, not an independently hinged skirt.
      // Shoulder bridges connect its front and back above the clavicles. They must
      // follow the plate rather than swing outward with upper-arm animation.
      const mountedBridge = name.startsWith('shoulder-') && (name.includes('-leather-bridge') || name.includes('-stitched-edge-'));
      // Leather bonded beneath a rigid plate must share its transform. Blending
      // this backing between lower spine joints pushes it through the breastplate.
      const plateBacking = name === 'chest-continuous-dark-leather-inner-vest'
        || name === 'chest-leather-waist-bound-edge'
        || name.includes('-continuous-leather-arch-lining');
      if (rigidMetal || mountedBridge || plateBacking) result.userData.itemModelBone = 'spine_03';
    ...
    const radius = (y: number) => .151 + .045 * Math.exp(-sq((y - 1.345) / .112)) + .025 * Math.exp(-sq((y - 1.015) / .050));
    function torso(theta: number, y: number, offset = 0) {
      ...
      const depth = front ? .153 + .025 * chest + .013 * hem : .135 + .014 * chest + .008 * hem;
      const x = (radius(y) + offset) * Math.sin(theta);
      let z = (depth + offset) * Math.cos(theta) - .018;
      if (front) {
        const pec = .027 * Math.exp(-sq((Math.abs(x) - .080) / .067) - sq((y - 1.355) / .062));
        const abs = .011 * Math.exp(-sq((Math.abs(x) - .043) / .036)) * (Math.exp(-sq((y - 1.247) / .035)) + .8 * Math.exp(-sq((y - 1.179) / .036)));
        const sternum = -.005 * Math.exp(-sq(x / .017) - sq((y - 1.34) / .09));
        const ribFold = -.008 * Math.exp(-sq((y - (1.286 + .12 * Math.abs(x))) / .018)) * Math.exp(-sq((Math.abs(x) - .082) / .065));
        z += (pec + abs + sternum + ribFold) * Math.cos(theta);
      }
      // Millimetre-scale tool marks also deform the silhouette and grazing highlights.
      const hammer = .00065 * Math.sin(theta * 79 + Math.sin(y * 135)) * Math.sin(y * 243 + Math.sin(theta * 113));
      z += hammer * Math.cos(theta);
      return V(x, y, z);
    }
    ...
    // A continuous sleeveless leather garment supports every plate and side lace.
    // Its long waist overlaps the leg lining when the spine bends.
    const lining: Surface = (u, v) => {
      const theta = u * Math.PI * 2 - Math.PI;
      const frontFacing = Math.cos(theta) >= 0;
      const a = frontFacing ? Math.abs(theta) : Math.PI - Math.abs(theta);
      const edgeHeight = top(frontFacing ? 1.25 : 1.1, frontFacing) + .004;
      const neckline = a <= 1.1 ? top(a * (frontFacing ? 1.25 / 1.1 : 1), frontFacing) + .004 : THREE.MathUtils.lerp(edgeHeight, 1.30, (a - 1.1) / (Math.PI / 2 - 1.1));
      const y = THREE.MathUtils.lerp(.966, neckline, v);
      return torso(theta, y, -.014);
    };
    shell('chest-continuous-dark-leather-inner-vest', lining, 80, 44, .004, leather);
    tube('chest-leather-waist-bound-edge', Array.from({ length: 81 }, (_, i) => lining(i / 80, 0)), .0025, leather, 80);
    ...
    for (const [name, surface] of [['front', front], ['back', back]] as const) {
      ...
      const hemBand: Surface = (u, v) => surface(u, v * .073).addScaledVector(V(Math.sin((u * 2 - 1) * (name === 'front' ? 1.1 : 1.25) + (name === 'back' ? Math.PI : 0)), 0, Math.cos((u * 2 - 1) * (name === 'front' ? 1.1 : 1.25) + (name === 'back' ? Math.PI : 0))), .004 + .005 * (1 - v));
      shell(`belt-${name}-overlapping-flared-copper-fauld`, hemBand, 48, 5, .003, copper);
      for (const edge of [0, 1]) tube(`belt-${name}-fauld-beaded-edge-${edge}`, Array.from({ length: 61 }, (_, i) => hemBand(i / 60, edge)), .0022, rim, 60);
      for (let i = 0; i < 7; i++) {
        const t = (i / 6 * 2 - 1) * (name === 'front' ? 1.04 : 1.18) + (name === 'back' ? Math.PI : 0);
        rivet(`belt-${name}-rivet-${i}`, torso(t, 1.024 + .014 * Math.abs(Math.sin(t)), .010), V(Math.sin(t), 0, Math.cos(t)), .007);
      }
    }
    ...
    for (const side of [-1, 1]) {
      ...
      const shoulder = (x: number, v: number, lift: number) => {
        ...
        return V(side * x, 1.477 + (.033 + lift) * Math.sin(Math.PI * f), THREE.MathUtils.lerp(a.z, b.z, f));
      };
      const copperShoulder: Surface = (u, v) => shoulder(.084 + u * .080, v, 0);
      const shoulderLining: Surface = (u, v) => shoulder(.080 + u * .089, v, -.006);
      shell(`shoulder-${side}-continuous-leather-arch-lining`, side === 1 ? shoulderLining : (u, v) => shoulderLining(1 - u, v), 10, 32, .004, leather);
      ...
      const strap: Surface = (u, v) => shoulder(.097 + u * .052, .015 + v * .97, .005 + .002 * Math.sin(v * Math.PI));
      ...
      for (const v of [.015, .985]) {
        const p = strap(.5, v), normal = V(side * .5, .15, v < .5 ? .85 : -.85).normalize();
        const loop = mesh(`shoulder-${side}-brass-retaining-loop-${v}`, new THREE.TorusGeometry(.015, .0027, 8, 18), brass);
        loop.position.copy(p).addScaledVector(normal, .002); loop.scale.set(1.5, .65, 1);
        loop.quaternion.setFromUnitVectors(V(0, 0, 1), normal);
      }
    }
    return root;
  },
