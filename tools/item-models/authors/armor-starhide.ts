...
function materials() {
  const n = 256, data = new Uint8Array(n * n * 4), rough = new Uint8Array(n * n * 4), normals = new Uint8Array(n * n * 4);
  let seed = 17321;
  const rnd = () => { seed = (Math.imul(seed, 1664525) + 1013904223) >>> 0; return seed / 4294967296; };
  for (let y = 0; y < n; y++) for (let x = 0; x < n; x++) {
    const k = (y * n + x) * 4;
    const grain = rnd() * 14 + 3 * Math.sin(x * 1.7 + Math.sin(y * .7));
    data[k] = 44 + grain; data[k + 1] = 43 + grain; data[k + 2] = 69 + grain * 1.3; data[k + 3] = 255;
    rough[k] = rough[k + 1] = rough[k + 2] = 130 + rnd() * 38; rough[k + 3] = 255;
    normals[k] = 128 + 5 * Math.sin(x * 1.6 + Math.sin(y * .9)); normals[k + 1] = 128 + 5 * Math.cos(y * 1.9 + Math.sin(x * .8)); normals[k + 2] = 255; normals[k + 3] = 255;
  }
  ...
  const nt = new THREE.DataTexture(normals, n, n); nt.wrapS = nt.wrapT = THREE.RepeatWrapping; nt.needsUpdate = true;
  const leather = new THREE.MeshStandardMaterial({ color: 0xffffff, map: tex, normalMap: nt, normalScale: new THREE.Vector2(.55, .55), roughnessMap: rt, roughness: .98, metalness: .04 }); leather.name = 'Star-speckled indigo leather';
  const lining = new THREE.MeshStandardMaterial({ color: 0x322040, roughness: .87 }); lining.name = 'Deep violet lining';
...
function padded(g: THREE.Group, name: string, f: Surface, m: Mats, direction: V, depth = .014) {
  const surface: Surface = (u, v) => f(u, v).addScaledVector(vec(direction), .004 + depth * Math.sin(Math.PI * u) * Math.sin(Math.PI * v));
  shell(g, name, surface, m, 16, 12, .007);
  for (const edge of [0, 1]) { seam(g, `${name} silver binding`, surface, 'v', edge, m); }
  seam(g, `${name} lower silver binding`, surface, 'u', 0, m);
  seam(g, `${name} inset stitches`, (u, v) => surface(.05 + u * .9, .045 + v * .91), 'u', 0, m, false);
  return surface;
}
...
function hood(g: THREE.Group, m: Mats) {
  ...
  for (const side of [0, 1]) {
    const facing: Surface = (u, v) => { const q = f(side ? 1 - u * .057 : u * .057, v); q.z += .007 + .009 * Math.sin(u * Math.PI); return q; };
    shell(g, 'Broad rolled leather face facing', facing, m, 10, 30, .006); seam(g, 'Face facing violet saddle seam', facing, 'v', .8, m, false);
  }
  ...
  for (let panel = 0; panel < 6; panel++) {
    const center = (panel + .5) / 6;
    const overlay: Surface = (u, v) => { const q = mantle(center + (u - .5) * .157, .04 + v * .9); q.y -= .027 * Math.sin(u * Math.PI) * (1 - v); return q; };
    padded(g, 'Pointed hood shoulder mantle panel', overlay, m, [Math.sin(-TAU * center), .45, Math.cos(TAU * center)], .009);
  }
...
function robe(g: THREE.Group, m: Mats) {
  const lining = { ...m, leather: m.lining };
  shell(g, 'Continuous violet torso lining beneath lapels', profile([[1.01, .15, .109], [1.18, .162, .128], [1.33, .196, .14], [1.43, .188, .119], [1.493, .085, .081]], 0, TAU, 0, .002), lining, 40, 18, .005);
  ...
  for (const side of [-1, 1]) {
    const lapel: Surface = (u, v) => { const y = 1.10 + v * .37, inside = .01 + .086 * v, width = .041 + .035 * Math.sin(v * Math.PI); return new THREE.Vector3(side * (inside + u * width), y - .04 * Math.sin(u * Math.PI) * v, .105 + .031 * Math.sin(v * Math.PI) + .014 * u); };
    padded(g, 'Broad folded pointed front lapel', lapel, m, [0, 0, 1], .018);
    jewel(g, 'Lapel corner star clasp', [side * .126, 1.407, .152], .019, m);
  }
  const skirtBase = profile([[.19, .345, .226], [.38, .302, .214], [.69, .243, .18], [.92, .19, .141], [1.075, .153, .118]], 0, TAU - .13, .13, .004);
  const skirtShape: Surface = (u, v) => { const q = skirtBase(u, v), a = TAU * (1 - u), pleat = .020 * Math.cos(a * 12) * Math.pow(1 - v, .6); q.x += pleat * Math.sin(a); q.z += pleat * Math.cos(a); return q; };
  shell(g, 'Continuous violet gathered skirt lining', profile([[.20, .328, .211], [.45, .278, .191], [.72, .226, .164], [.96, .165, .123], [1.077, .148, .112]], 0, TAU, 0, .012), lining, 48, 24, .004);
  const skirt = shell(g, 'Full split leather robe skirt with folded back', skirtShape, m, 72, 30);
  seam(g, 'Skirt hem', skirt, 'u', 0, m);
  for (const u of [0, 1, .2, .4, .6, .8]) seam(g, 'Skirt tailored length seam', skirt, 'v', u, m, u === 0 || u === 1);
  for (const u of [.055, .22, .40, .60, .78, .945]) {
    const f: Surface = (a, b) => skirt(u + (a - .5) * (.11 + .025 * (1 - b)), .105 + b * .895).add(new THREE.Vector3(-.012 * Math.sin(TAU * u), -.055 * Math.sin(a * Math.PI) * (1 - b), .015 * Math.cos(TAU * u)));
    padded(g, 'Long pointed overlapping leather coat panel', f, m, [-Math.sin(TAU * u), 0, Math.cos(TAU * u)], .009);
  ...
  for (const y of [1.038, 1.079]) band(g, 'Double waist belt raised leather rail', [[y, .168, .136], [y + .018, .168, .136]], m);
  for (const side of [-1, 1]) {
    const tail: Surface = (u, v) => new THREE.Vector3(side * (.055 + .058 * (1 - v)) + (u - .5) * .028, .72 + v * .34 - .02 * Math.sin(u * Math.PI), .178 - v * .052 + .009 * Math.sin(v * 8));
    padded(g, 'Hanging pointed leather belt end', tail, m, [0, 0, 1], .004);
    jewel(g, 'Belt end star stud', [side * .103, .77, .184], .016, m);
  }
  for (const side of [-1, 1]) {
    const sleeve: Surface = (u, v) => {
      const theta = TAU * (side > 0 ? u : 1 - u), r = .075 + .04 * Math.pow(v, 3) + .008 * Math.sin(v * 36 + theta * 2) * Math.sin(v * Math.PI);
    ...
    const cuff: Surface = (u, v) => { const q = sleeve(u, .82 + v * .18), a = TAU * (side > 0 ? u : 1 - u); q.y += .009 * Math.cos(a); q.z += .009 * Math.sin(a); return q; };
    shell(g, 'Broad turned-back bell sleeve cuff', cuff, m, 32, 8, .008); seam(g, 'Bell cuff facing inner binding', cuff, 'u', 0, m);
    for (let layer = 0; layer < 3; layer++) {
      const cape: Surface = (u, v) => {
        const theta = Math.PI * 1.26 * (u - .5), r = .139 - .009 * layer + .012 * Math.sin(v * Math.PI);
        return new THREE.Vector3(side * (.145 + layer * .058 + v * (.13 + .036 * Math.sin(u * Math.PI))), 1.455 + r * Math.cos(theta) - .026 * v, -.066 + r * 1.1 * Math.sin(theta));
      };
      shell(g, 'Deeply overlapping pointed shoulder leather tier', cape, m, 28, 10, .009); seam(g, 'Shoulder cape silver border', cape, 'u', 1, m); seam(g, 'Shoulder cape violet inset seam', cape, 'u', .87, m, false);
    }
  }
}
...
function leggings(g: THREE.Group, m: Mats) {
  ...
  const gusset: Surface = (u, v) => new THREE.Vector3((u - .5) * .079, .846 + .05 * Math.pow(2 * u - 1, 2) + .015 * Math.pow(2 * v - 1, 2), -.03 + (v - .5) * .208);
  shell(g, 'Continuous shaped trouser crotch gusset', gusset, m, 14, 14, .005);
  ...
  for (const side of [-1, 1]) {
    ...
    const thigh: Surface = (u, v) => { const a = side * .65 + (u - .5) * 1.15, r = .101 + .005 * Math.sin(v * Math.PI); return new THREE.Vector3(x + r * Math.sin(a), .646 + v * .225 - .034 * Math.sin(u * Math.PI) * (1 - v), -.023 + .113 * Math.cos(a)); };
    padded(g, 'Pointed padded outer thigh leather panel', thigh, m, [side * .5, 0, .8], .013);
    const shin: Surface = (u, v) => { const a = (u - .5) * 1.05; return new THREE.Vector3(x + .066 * Math.sin(a), .19 + v * .241, -.028 + .078 * Math.cos(a) + .007 * Math.sin(v * 18)); };
    shell(g, 'Sewn folded shin leather reinforcement', shin, m, 12, 18, .005);
    for (const u of [0, 1]) seam(g, 'Shin reinforcement violet seam', shin, 'v', u, m, false);
    ...
    for (let layer = 0; layer < 3; layer++) {
      ...
      padded(g, 'Raised layered outer hip leather scale', hip, m, [side * .75, 0, .65], .016); seam(g, 'Hip scale violet stitching', hip, 'u', .12, m, false);
...
function boots(g: THREE.Group, m: Mats) {
  for (const side of [-1, 1]) {
    ...
    for (let fold = 0; fold < 3; fold++) {
      const crease: Surface = (u, v) => { const a = (u - .5) * 2.7, y = .18 + fold * .027 + v * .023 + .008 * Math.sin(a * 1.4 + fold); return new THREE.Vector3(x + (.073 + .009 * Math.sin(v * Math.PI)) * Math.sin(a), y, -.04 + (.077 + .011 * Math.sin(v * Math.PI)) * Math.cos(a)); };
      shell(g, 'Compressed boot shaft leather fold', crease, m, 20, 6, .004);
    }
    for (let layer = 0; layer < 2; layer++) {
    ...
    for (const s of [-1, 1]) {
      const quarter: Surface = (u, v) => { const a = s * (1.06 + u * 1.45), q = foot((TAU - a) / TAU, .04 + v * .66); q.x += s * .004; q.y += .009 * Math.sin(v * Math.PI); return q; };
      shell(g, 'Overlapping side and heel boot leather quarter', quarter, m, 14, 12, .005); seam(g, 'Boot quarter violet saddle seam', quarter, 'u', 1, m, false);
    }
...
function wraps(g: THREE.Group, m: Mats) {
  for (const side of [-1, 1]) {
    const f: Surface = (u, v) => { const a = TAU * (side > 0 ? u : 1 - u), r = .043 + .007 * Math.sin(v * Math.PI) + .0045 * Math.sin(v * 35 + a * 3); return new THREE.Vector3(side * (.632 + v * .129), 1.4555 + r * Math.cos(a), -.0654 + r * .72 * Math.sin(a)); };
    ...
    for (let layer = 0; layer < 3; layer++) {
      const strip: Surface = (u, v) => { const vv = .035 + layer * .285 + v * .205 + (layer % 2 ? -.10 : .10) * Math.sin(u * TAU); const q = f(u, vv), a = TAU * (side > 0 ? u : 1 - u), lift = .007 + .004 * Math.sin(v * Math.PI); q.y += lift * Math.cos(a); q.z += lift * Math.sin(a); return q; };
      shell(g, 'Broad crossing overlapping stitched wrap band', strip, m, 40, 7, .006); seam(g, 'Wrap band silver edge', strip, 'u', 0, m); seam(g, 'Wrap band upper silver edge', strip, 'u', 1, m); seam(g, 'Wrap band violet stitching', strip, 'u', .83, m, false);
      for (let stitch = 0; stitch < 8; stitch++) {
        const u = .11 + stitch * .021;
        line(g, 'Cross-laced violet wrap seam', strip(u, .18), strip(u + .018, .82), .0015, m.thread);
        line(g, 'Cross-laced violet wrap seam return', strip(u, .82), strip(u + .018, .18), .0015, m.thread);
      }
    }
    ...
    const tab: Surface = (u, v) => new THREE.Vector3(side * (.631 + u * .035), 1.429 + v * .05, -.022 + .01 * Math.sin(v * Math.PI));
    padded(g, 'Rounded wrist fastening leather tab', tab, m, [0, 0, 1], .006);
    const button = mesh(g, 'Circular engraved wrist fastener', new THREE.CylinderGeometry(.009, .009, .004, 16), m.silver); button.rotation.x = Math.PI / 2; button.position.set(side * .646, 1.456, -.003);
    jewel(g, 'Wrist strap silver star button', [side * .646, 1.456, .001], .006, m);
  }
}
