...
function materials() {
  const size=256, color=new Uint8Array(size*size*4), rough=new Uint8Array(size*size*4), normal=new Uint8Array(size*size*4);
  let seed=4217;
  ...
  for(let y=0;y<size;y++)for(let x=0;x<size;x++){
    ...
    color[i]=32+n*10+scratch;color[i+1]=42+n*12+scratch;color[i+2]=64+n*16+vein*5+scratch;color[i+3]=255;
    const r=134+n*24+Math.sin(x*.11+y*.05)*12;rough[i]=r;rough[i+1]=r;rough[i+2]=r;rough[i+3]=255;
    normal[i]=128+Math.sin(x*.2)*3+Math.cos(y*.11)*2;normal[i+1]=128+Math.cos(y*.17+x*.04)*3;normal[i+2]=255;normal[i+3]=255;
  }
  ...
  const glass=new T.MeshPhysicalMaterial({name:'Nightglass polished midnight blue black steel',color:0xffffff,map:tex(color,true),roughnessMap:tex(rough),normalMap:tex(normal),normalScale:new T.Vector2(.3,.3),metalness:.74,roughness:.54,clearcoat:.55,clearcoatRoughness:.18});
...
function panel(g:T.Group,name:string,points:V[],m:M,bulge=.009){
  ...
  for(const v of points){const q=new T.Vector3(...v).lerp(c,.045);p.push(q.x,q.y,q.z);}
  ...
  for(let i=0;i<n;i++){const a=points[i]!,b=points[(i+1)%n]!;line(g,name+' raised silver edge '+i,[a[0],a[1],a[2]+Math.sign(bulge)*.001],[b[0],b[1],b[2]+Math.sign(bulge)*.001],.00115,m.edge);}
  const back=points.map(v=>[v[0],v[1],v[2]-Math.sign(bulge)*.014] as V);const bp=back.flat();const bidx:number[]=[];for(let i=1;i<n-1;i++)bidx.push(0,i+1,i);mesh(g,name+' inner plate',geometry(bp,bidx),m.leather);
}
...
function helm(g:T.Group,m:M){
  ...
  for(const s of [-1,1]){
    ...
    panel(g,'Swept rear crown flute '+s,[[0,1.839,-.032],[s*.066,1.803,-.07],[s*.103,1.743,-.042],[s*.087,1.688,-.066],[s*.029,1.724,-.116],[0,1.79,-.089]],m,-.008);
    panel(g,'Lower cheek branching inset '+s,[[s*.083,1.639,.123],[s*.056,1.623,.145],[s*.016,1.572,.153],[s*.065,1.594,.135]],m,.005);
  }
...
function plate(g:T.Group,m:M){
  shell(g,'Inset pelvis fauld backing',[[.935,.155,.108],[1.055,.137,.112]],m.leather,0,-.018).userData.itemModelBone='pelvis';
  shell(g,'Inset abdominal plate backing',[[1.035,.14,.108],[1.115,.135,.111],[1.21,.162,.126]],m.leather,0,-.018).userData.itemModelBone='spine_01';
  shell(g,'Inset breast and back backing',[[1.19,.151,.125],[1.26,.169,.134],[1.36,.178,.127],[1.415,.157,.111],[1.473,.076,.073]],m.leather,0,-.018).userData.itemModelBone='spine_03';
  shell(g,'Flexible waist gusset seam',[[1.031,.131,.098],[1.06,.13,.099]],m.leather,0,-.018);
  shell(g,'Flexible rib gusset seam',[[1.188,.143,.115],[1.216,.149,.118]],m.leather,0,-.018);
  shell(g,'Back and flanks cuirass',[[1.04,.157,.113],[1.15,.153,.124],[1.3,.197,.142],[1.395,.188,.125]],m.glass,0,-.014,.93,Math.PI*2-.93);
  for(const s of [-1,1]){
    panel(g,'Inner top shoulder cap '+s,[[s*.112,1.487,-.097],[s*.195,1.534,-.108],[s*.279,1.486,-.092],[s*.284,1.482,.067],[s*.197,1.536,.067],[s*.114,1.489,.081]],m,.009);
    panel(g,'Sculpted breast half '+s,[[0,1.397,.173],[s*.158,1.437,.114],[s*.198,1.336,.148],[s*.149,1.245,.151],[s*.126,1.199,.14],[0,1.125,.174]],m,.036);
    panel(g,'Raised swept pectoral ridge '+s,[[s*.012,1.384,.185],[s*.15,1.416,.137],[s*.179,1.353,.162],[s*.122,1.33,.184],[s*.022,1.261,.195]],m,.009);
    strap(g,'Shoulder suspension '+s,[s*.143,1.466,.045],[s*.177,1.346,.131],m,.025);
    for(let i=0;i<3;i++){
      const x=s*(.23+i*.039),y=1.48-i*.034;
      panel(g,'Pointed overlapping shoulder '+s+' '+i,[[x-s*.061,y+.041,-.1],[x+s*.049,y+.028,-.109],[x+s*.084,y-.045,-.032],[x+s*.072,y-.067,.089],[x-s*.049,y+.018,.103]],m,.028);
      panel(g,'Shoulder swept upper bevel '+s+' '+i,[[x-s*.047,y+.033,.107],[x+s*.065,y-.052,.097],[x+s*.075,y-.035,.063],[x-s*.031,y+.047,.056]],m,.008);
    ...
    panel(g,'Rear scapula '+s,[[s*.01,1.463,-.092],[s*.166,1.426,-.118],[s*.186,1.345,-.172],[s*.17,1.251,-.179],[s*.016,1.2,-.19]],m,-.019);
  }
...
function greaves(g:T.Group,m:M){
  shell(g,'Inset pelvic arming backing',[[.90,.184,.12],[.978,.189,.118]],m.leather,0,-.04).userData.itemModelBone='pelvis';
  for(const s of [-1,1]){const x=s*.1143;
    shell(g,'Inset calf plate backing '+s,[[.18,.038,.053],[.30,.052,.071],[.38,.056,.073],[.49,.054,.06],[.562,.057,.06]],m.leather,x,-.036).userData.itemModelBone=`calf_${s>0?'l':'r'}`;
    shell(g,'Inset thigh plate backing '+s,[[.545,.057,.06],[.63,.067,.067],[.76,.08,.076],[.965,.093,.088]],m.leather,x,-.036).userData.itemModelBone=`thigh_${s>0?'l':'r'}`;
    shell(g,'Flexible knee gusset seam '+s,[[.533,.052,.054],[.575,.054,.055]],m.leather,x,-.036);
...
function boots(g:T.Group,m:M){for(const s of [-1,1]){const x=s*.1143;
  shell(g,'Inset boot shaft backing '+s,[[.14,.042,.052],[.20,.045,.054],[.305,.054,.058]],m.leather,x,-.058).userData.itemModelBone=`calf_${s>0?'l':'r'}`;
  shell(g,'Flexible ankle gusset seam '+s,[[.126,.039,.048],[.154,.04,.049]],m.leather,x,-.058);
  ...
  shell(g,'Shaped leather vamp '+s,footRings,m.leather,x,-.02).userData.itemModelBone=`foot_${s>0?'l':'r'}`;
...
function fitAndBind(g:T.Group,itemId:string,m:M){
  g.updateMatrixWorld(true);
  g.traverse(o=>{
    if(!(o instanceof T.Mesh))return;
    const attribute=o.geometry.getAttribute('position');
    if(itemId==='nightglass_greaves'||itemId==='nightglass_boots'){
      const inverse=o.matrixWorld.clone().invert(),p=new T.Vector3();
      for(let i=0;i<attribute.count;i++){
        p.fromBufferAttribute(attribute,i).applyMatrix4(o.matrixWorld);
        // The native calf projects farther behind its joint than an elliptical
        // stick figure. Expand only its rear half; the approved front keel stays put.
        if(itemId==='nightglass_greaves'&&p.z<-.036){const factor=p.y<.59?1.55:1.22;p.z=-.036+(p.z+.036)*factor;}
        if(itemId==='nightglass_boots'&&p.y>.14&&p.z<-.058){const factor=1+Math.min(1,(p.y-.14)/.1)*.42;p.z=-.058+(p.z+.058)*factor;}
        p.applyMatrix4(inverse);attribute.setXYZ(i,p.x,p.y,p.z);
      }
      attribute.needsUpdate=true;o.geometry.computeVertexNormals();o.geometry.computeBoundingBox();o.geometry.computeBoundingSphere();
    }
    const name=o.name,center=new T.Box3().setFromObject(o).getCenter(new T.Vector3()),side=center.x>=0?'l':'r';
    const soft=o.material===m.leather&&!name.includes('inner plate');
    if(soft||o.material===m.seam)return;
    let bone='';
    if(itemId==='nightglass_helm')bone='Head';
    else if(itemId==='nightglass_plate'){
      if(/Inner top shoulder cap/.test(name))bone='spine_03';
      else if(/shoulder/i.test(name)&&!/suspension/i.test(name))bone=`upperarm_${side}`;
      else if(/hip tasset/.test(name))bone='pelvis';
      else if(/abdominal lame|lower articulated spine/.test(name))bone=/(?:abdominal lame|lower articulated spine) [23](?: |$)/.test(name)?'pelvis':'spine_01';
      else bone='spine_03';
    }else if(itemId==='nightglass_greaves'){
      bone=/thigh|cuisse|Thigh/.test(name)?`thigh_${side}`:/hinge/.test(name)&&center.y>.6?`thigh_${side}`:`calf_${side}`;
    }else if(itemId==='nightglass_boots')bone=/shaft|cuff|ankle diagonal/i.test(name)||(/closure/.test(name)&&center.y>.14)?`calf_${side}`:`foot_${side}`;
    else bone=/cuff|Rear wrist/i.test(name)?`lowerarm_${side}`:`hand_${side}`;
...
export const author:ItemModelAuthor={ids,build(itemId){
  ...
  fitAndBind(g,itemId,m);
  const bodyCoverage=itemId==='nightglass_plate'?[{region:'torso',minY:.94,maxY:1.465}]:itemId==='nightglass_greaves'?[{region:'legs',minY:.185,maxY:.96}]:itemId==='nightglass_boots'?[{region:'legs',minY:.145,maxY:.298}]:undefined;
  g.userData.itemModel={itemId,author:'armor-nightglass',reference:`art/item-icons/generated/${itemId}.png`,description:`Nightglass ${itemId.slice(11)} forged for level 70 melee combat. Angular midnight steel armor with cold bevels, articulated plates and riveted leather lining.`,wearable:true,...(bodyCoverage?{bodyCoverage}:{})};return g;
}};
