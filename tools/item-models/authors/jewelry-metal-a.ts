...
function copperPendant(root:THREE.Group) {
  ...
  const plate=dome(outline,.0022,.0012),pos=plate.getAttribute('position'),uv=plate.getAttribute('uv');
  const dents=Array.from({length:87},(_,i)=>{const a=i*2.399963,r=Math.sqrt((i+.5)/87)*.94;return {x:.014*r*Math.cos(a),y:.018*r*Math.sin(a),radius:.00125+.00075*(.5+.5*Math.sin(i*7.31)),depth:.00014+.00016*(.5+.5*Math.sin(i*3.73))};});
  for(let i=0;i<pos.count;i++) {
    const x=pos.getX(i),y=pos.getY(i),z=pos.getZ(i);uv.setXY(i,x/.024+.5,y/.024+.5);
    if(z<=0)continue;
    let relief=0;for(const d of dents){const r2=((x-d.x)**2+(y-d.y)**2)/(d.radius*d.radius);relief-=d.depth*Math.exp(-r2*2.5);}
    const boundary=Math.max(0,1-(x/.014)**2-(y/.018)**2);
    pos.setZ(i,z+relief*Math.min(1,boundary*8));
  }
  plate.computeVertexNormals();add(root,'Hammered convex copper medallion',plate,m);
...
function pendant(root:THREE.Group,cobalt:boolean) {
  ...
  const stone=cobalt?garnetMaterial():new THREE.MeshPhysicalMaterial({name:'Clear golden honey amber pendant',color:0xffd782,roughness:.075,transmission:.94,thickness:.003,ior:1.54,attenuationColor:new THREE.Color(0xe89528),attenuationDistance:.055,clearcoat:.55,clearcoatRoughness:.06});
  add(root,cobalt?'Faceted marquise garnet':'Pear shaped polished amber',cobalt?gem(o,depth):dome(o,depth,.0023),stone);
  ...
  if(cobalt)for(const side of [-1,1])tube(root,'Rear basket strut '+side,[v(side*.008,-.009,-.0011),v(0,0,-.0030),v(-side*.006,.011,-.0011)],.00043,m,false,24,8);
  else for(const side of [-1,1])tube(root,'Short rear amber edge keeper '+side,[v(side*.0090,-.010,-.0010),v(side*.0088,-.008,-.0018),v(side*.0094,-.006,-.0010)],.00020,m,false,16,6);
}
function ring(root:THREE.Group,cobalt:boolean) {
  ...
  if(cobalt)for(const side of [-1,1]) {
    // Sample the exact superellipse cross-section used by shank(), so every
    // engraved border and diamond remains attached to the solid shoulder.
    const p=(a:number,f:number,extra=0)=>{const s=Math.pow(Math.sin(a),5),w=.00225+s*.0037,t=.00165+s*.0031;
      const sin=Math.pow(Math.abs(f),1/.30),c=Math.pow(Math.sqrt(1-sin*sin),.30),r=.01+t*(c+1)/2+extra;
      return v(side*r*Math.cos(a),r*Math.sin(a),f*w);};
    for(const flank of [-1,1])tube(root,'Flush shoulder chevron '+side+' '+flank,Array.from({length:30},(_,i)=>{const t=i/29,a=.38+t*.77;return p(a,flank*(.15+.65*Math.sin(t*Math.PI)),.000025);}),.000045,edge,false,32,6);
    const positions:number[]=[],uv:number[]=[],steps=20;
    for(let i=0;i<steps;i++) {
      const a0=.49+i/steps*.52,a1=.49+(i+1)/steps*.52;
      const f0=.49*Math.sin(i/steps*Math.PI),f1=.49*Math.sin((i+1)/steps*Math.PI);
      for(const flank of [-1,1])for(const q of [p(a0,0,.00011),p(a0,flank*f0,.00004),p(a1,0,.00011),p(a0,flank*f0,.00004),p(a1,flank*f1,.00004),p(a1,0,.00011)]){positions.push(...q.toArray());uv.push(q.x*100,q.y*100);}
    }
    ...
    // Orient both shoulder overlays away from the ring bore.
    const normals=g.getAttribute('normal'),pp=g.getAttribute('position');for(let i=0;i<pp.count;i+=3)if(pp.getX(i)*normals.getX(i)+pp.getY(i)*normals.getY(i)<0){for(let k=0;k<3;k++){const tmp=positions[(i+1)*3+k]!;positions[(i+1)*3+k]=positions[(i+2)*3+k]!;positions[(i+2)*3+k]=tmp;}}
    g.setAttribute('position',new THREE.Float32BufferAttribute(positions,3));g.computeVertexNormals();add(root,'Inset shoulder diamond '+side,g,edge);
  }
