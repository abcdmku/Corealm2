...
function materials(revised = false) {
  ...
  return { fur: make('Cairnpelt desaturated grey brown guard fur', 0x979086, .94, true, true),
    pale: make('Cairnpelt pale grey ivory underfur', 0xc7c4b9, .97, true, true),
...
function pelt(g: THREE.Group, name: string, s: Surface, m: Mats, count: number, pale = false, length = .016, nu = 40, nv = 12, hole?: (u: number,v: number) => boolean) {
  ...
  if(g.userData.itemModel.itemId!=='cairnpelt_hood') {
    hairs(g,name+' dense flowing pelt locks',s,pale?m.pale:m.fur,Math.floor(count*1.05),length*1.75,.0029,hole);
    hairs(g,name+' fine underfur locks',s,pale?m.pale:m.fur,Math.floor(count*.60),length*1.6,.0023,hole);
    return;
  }
...
// Narrow overlapping locks blend into the same-colour connected undercoat.
// Three closed triangular cross-sections retain real volume without bead-like bulges.
function shag(g:THREE.Group,name:string,s:Surface,m:Mats,columns:number,rows:number,width:number,length:number,depth:number,pale=false,hole?:(u:number,v:number)=>boolean){
  const positions:number[]=[],uv:number[]=[],indices:number[]=[];
  columns*=4;width*=.32;depth*=.23;
  for(let row=0;row<rows;row++)for(let col=0;col<columns;col++){
    const u=(col+.5+(row%2)*.43)/columns,v=(row+.78)/rows;
    if(hole?.(u,v))continue;
    const root=s(u,v),n=normal(s,u,v),down=s(u,v-.002).sub(s(u,v+.002)).normalize(),across=n.clone().cross(down).normalize();
    const scale=.84+.24*(.5+.5*Math.sin(col*4.32+row*7.8)),base=positions.length/3;
    for(let j=0;j<3;j++){
      const t=j/2,w=width*scale*(j===0?.85:j===2?.01:1);
      const c=root.clone().addScaledVector(down,length*scale*t).addScaledVector(n,-.0008+depth*Math.sin(t*Math.PI*.85));
      c.addScaledVector(across,Math.sin(t*Math.PI)*width*.35*Math.sin(row+col*.05));
      for(let k=0;k<3;k++){const a=TAU*k/3,p=c.clone().addScaledVector(across,Math.cos(a)*w).addScaledVector(n,Math.sin(a)*depth*.32);positions.push(p.x,p.y,p.z);uv.push(u*3+Math.cos(a)*.008,v*4-t*.08);}
    }
    for(let j=0;j<2;j++)for(let k=0;k<3;k++){const a=base+j*3+k,b=base+j*3+(k+1)%3;indices.push(a,b,a+3,b,b+3,a+3);}
    indices.push(base,base+2,base+1,base+6,base+7,base+8);
  }
  ...
  const mesh=new THREE.Mesh(geo,pale?m.pale:m.fur);mesh.name=name+' connected swept fur layer';g.add(mesh);
}
function raisedJoin(g:THREE.Group,name:string,s:Surface,u:number,m:Mats,count:number,cobalt=false){
  const pts=Array.from({length:24},(_,i)=>s(u,i/23).addScaledVector(normal(s,u,i/23),.025));
  tube(g,name+' deep leather pelt welt',pts,.0042,m.hide);
  for(let i=0;i<count;i++){
    const v=(i+.5)/count,p=s(u,v).addScaledVector(normal(s,u,v),.027),across=s(u+.001,v).sub(s(u-.001,v)).normalize(),up=s(u,v+.001).sub(s(u,v-.001)).normalize();
    line(g,name+' cross stitch',p.clone().addScaledVector(across,-.011).addScaledVector(up,-.004),p.clone().addScaledVector(across,.011).addScaledVector(up,.004),cobalt?.0022:.0016,cobalt?m.cobalt:m.thread);
  }
}
...
function hood(g: THREE.Group, m: Mats) {
  ...
  shag(g,'Bear hood swept winter pelt',s,m,20,8,.016,.043,.009);
  for (const u of [0,1]) {
  ...
  shag(g,'Shoulder cape overlapping fur lobes',mantle,m,24,3,.021,.051,.013,true);
...
function robe(g: THREE.Group, m: Mats) {
  ...
  shag(g,'Long flowing skirt pelts',skirt,m,28,13,.020,.085,.014);
  ...
  shag(g,'Dense winter coat chest',chest,m,18,6,.022,.069,.016);
  // The dark underlap closes the chest slit while leaving the coat fronts separate.
  const underlap:Surface=(u,v)=>V(mix(-.061,.061,u),mix(.97,1.465,v),.112+.012*Math.sin(v*Math.PI)-.01*(2*u-1)**2);
  surface(g,'Continuous suede chest underlap',underlap,m.lining,m.lining,10,12,.007);
  for(const side of [-1,1]) {
    ...
    shag(g,'Overlapping sleeve pelt folds '+side,sleeveOut,m,16,6,.017,.072,.013);
    ...
    shag(g,'Turned cuff heavy fur '+side,cuff,m,20,1,.014,.043,.014,true);
    const lapel:Surface=(u,v)=> { const y=mix(1.09,1.51,v), x=side*(.015+.073*Math.sin(v*Math.PI*.7)+u*(.025+.075*Math.sin(v*Math.PI)));return V(x,y,.126+.043*Math.sin(v*Math.PI)+.036*Math.sin(u*Math.PI)); };
    const lapelOut:Surface=(u,v)=>lapel(side===1?u:1-u,v);
    pelt(g,`Long folded fur lapel ${side}`,lapelOut,m,160,true,.027,10,16);
    shag(g,'Broad turned pelt lapel '+side,lapelOut,m,4,8,.013,.065,.015,true);
    ...
    raisedJoin(g,'Cobalt sewn front pelt '+side,skirt,side===1?.125:.875,m,27,true);
  ...
  for(const u of [.34,.66])raisedJoin(g,'Cobalt sewn rear pelt',skirt,u,m,25,true);
  ...
  band(g,'Leather waist belt',1.079,.181,.159,.052,m);buckle(g,1.079,.144,m,1.18);
  ...
  for(const u of [0,1])tube(g,'Belt tail bound edge',Array.from({length:10},(_,i)=>tail(u,i/9)),.002,m.thread);
}
function leggings(g: THREE.Group, m: Mats) {
  ...
  shag(g,'Pelvis layered winter pelt',hip,m,24,4,.018,.057,.011);
  for(const side of [-1,1]) {
    const cx=side*.1143;
    ...
    shag(g,'Trouser leg flowing fur clumps '+side,leg,m,18,12,.015,.071,.012);
    raisedJoin(g,'Raised trouser pelt seam '+side,leg,side===1?.16:.84,m,25);
    ...
    band(g,'Weighted leather ankle band',.261,.088,.101,.052,m,cx,-.038);
...
function boots(g: THREE.Group, m: Mats) {
  for(const side of [-1,1]) { const cx=side*.1143;
    ...
    shag(g,'Boot shaft thick overlapping winter fur '+side,shaft,m,18,7,.013,.049,.010);
    const cuff=loft([[.30,.083,.092],[.333,.092,.10],[.371,.086,.094]],cx,-.047);pelt(g,'Deep rolled fur boot cuff '+side,cuff,m,200,true,.020,32,6);
    shag(g,'Boot turned cuff heavy fur '+side,cuff,m,22,2,.017,.048,.013,true);
    tube(g,'Thick rolled boot opening '+side,Array.from({length:33},(_,i)=>cuff(i/32,.93)),.009,m.pale);
    ...
    shag(g,'Boot vamp pelt lobes '+side,shoe,m,18,3,.012,.039,.008);
    ...
    for(const y of [.146,.242,.300]) band(g,'Narrow shaft binding',y,.083,.101,.012,m,cx,-.047);
...
function wraps(g: THREE.Group, m: Mats) {
  for(const side of [-1,1]) {
    ...
    const palm:Surface=(u,v)=>{const a=mix(.30,.70,u),b=mix(.48,.99,v);return wrap(a,b).addScaledVector(normal(wrap,a,b),.004+.005*Math.sin(u*Math.PI));};
    surface(g,'Supple continuous leather palm gusset '+side,palm,m.hide,m.lining,18,10,.006);
    ...
    for(let j=0;j<4;j++) {
      const strip:Surface=(u,v)=> { const t=.015+j*.22+.12*u+v*.19;return wrap(u,t).addScaledVector(normal(wrap,u,t),.005+.012*Math.sin(v*Math.PI)); };
      const stripHole=(u:number,v:number)=>thumbHole(u,.015+j*.22+.12*u+v*.19);
      pelt(g,'Spiral pelt binding '+side+' '+j,strip,m,170,true,.014,48,5,stripHole);
      shag(g,'Thick overlapping wrap pelt '+side+' '+j,strip,m,16,2,.010,.025,.007,true,stripHole);
      for(let i=0;i<48;i++){const u=i/48,v=.015+j*.22+.12*u;if(thumbHole(u,v))continue;const un=(i+1)/48,vn=.015+j*.22+.12*un;line(g,'Leather spiral tie '+side,wrap(u,v).addScaledVector(normal(wrap,u,v),.017),wrap(un,vn).addScaledVector(normal(wrap,un,vn),.017),.0035,m.hide);}
    }
    // Native thumb exits the palm on its forward lower edge. The short open cuff is hollow.
    const thumb:Surface=(u,v)=> {const a=(side===1?1-u:u)*TAU;return V(side*(.738+.018*v+.021*Math.sin(a)),1.428-.011*v+.017*Math.cos(a),-.025+v*.044);};
...
export const author: ItemModelAuthor = {
  ...
  build(id) {
    ...
    const g=new THREE.Group(),m=materials(true);g.name=id;
