...
export const RESOURCE_TREE_DESIGNS = [
  { id: 'corealm_teak_lastroot', source: 'corealm_teak_1', species: 'teak', tier: 50, description: 'Veinwood with a tall clear bole and a broad sheltered crown; the bowed bole still supports each surviving leaf spray.' },
  { id: 'corealm_teak_embershelter', source: 'corealm_teak_2', species: 'teak', tier: 50, description: 'A storm-weathered Veinwood with a ragged split crown, charred windward wood and sheltered olive growth.' },
  ...
  { id: 'corealm_magic_moonvein', source: 'corealm_magic_2', species: 'magic', tier: 70, description: 'An upright old magic tree with an unequal swept crown and violet sap following the wood grain.' },
] as const;
...
export function sculptTreePoint(id: ResourceTreeId, [x, y, z]: Point): Point {
  ...
  if (id === 'corealm_teak_lastroot') return [x * (1 + .12 * t) - .7 * t * t, y * .91, z * (1 + .1 * t) + .35 * t * t];
  if (id === 'corealm_teak_embershelter') return [x * (1 + .18 * t) + 1.1 * t * t, y * .94, z * .9 - .65 * t * t];
  const angle = (id === 'corealm_magic_starwood' ? -.14 : .19) * t * t;
  const spread = id === 'corealm_magic_starwood' ? 1 + .12 * root : 1 + .04 * root;
