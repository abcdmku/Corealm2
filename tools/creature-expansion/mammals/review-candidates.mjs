...
for(const id of ids){
 ...
 await page.evaluate(async ()=>{const b=window.__creatureGallery.getBounds(),size=Math.max(...b.max.map((v,i)=>v-b.min[i]));await window.__gameDebug.inspectPose({x:(b.min[0]+b.max[0])/2,y:(b.min[1]+b.max[1])/2,z:(b.min[2]+b.max[2])/2,yaw:Math.PI/2,pitch:.16,distance:Math.max(2.8,size*1.45),detached:true});});
 ...
 if(process.env.CANDIDATE_EXTENDED==='1')for(const [view,yaw,motion]of [['front',0,'idle'],['rear',Math.PI,'idle'],['walk',Math.PI/2,'walk'],['attack',Math.PI/2,'attack'],['hit',Math.PI/2,'hit']]){
  await page.evaluate(async ({yaw,motion})=>{window.__creatureGallery.play(motion);const b=window.__creatureGallery.getBounds(),size=Math.max(...b.max.map((v,i)=>v-b.min[i]));await window.__gameDebug.inspectPose({x:(b.min[0]+b.max[0])/2,y:(b.min[1]+b.max[1])/2,z:(b.min[2]+b.max[2])/2,yaw,pitch:.16,distance:Math.max(2.8,size*1.45),detached:true});},{yaw,motion});
