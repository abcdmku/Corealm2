...
export function coatTextures(id){
  const size=256,height=new Float32Array(size*size),normal=new Uint8Array(size*size*4),rough=new Uint8Array(size*size*4);
  const wool=id==='cairn_bighorn',coarse=id==='marsh_moose';
  ...
  for(let y=0;y<size;y++)for(let x=0;x<size;x++){
    ...
    height[y*size+x]=wool?.40*Math.sin(a*13+1.4*Math.sin(b*11))*.5+.20*Math.sin(b*17+a*7):ridges*broken*.24+(coarse?.14:.065)*Math.sin(a*47+.6*Math.sin(b*2));
  ...
  return {normal:normalMap,roughness:roughnessMap};
}
