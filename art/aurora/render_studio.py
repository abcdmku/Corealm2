...
blender -b SET.blend --disable-autoexec --python art/aurora/render_studio.py
...
Pass --transparent --size 512 for a square RGBA render of the actual model without a floor.
Lighting and exposure stay identical to the opaque studio preview.
"""
...
def main():
    ...
    parser.add_argument("--transparent", action="store_true", help="Render RGBA without the studio floor or visible background.")
    parser.add_argument("--size", type=int, help="Optional square output size in pixels, suitable for inventory icons.")
    ...
    assert args.size is None or 64 <= args.size <= 8192, "Square size must be between 64 and 8192 pixels."
    ...
    scene.render.resolution_x, scene.render.resolution_y = (args.size, args.size) if args.size else (1800, 1600)
    ...
    if args.piece:
        ...
        scene.render.resolution_x, scene.render.resolution_y = ((args.size, args.size) if args.size
            else ((1800, 1600) if width > height else (1600, 1800)))
    ...
    report = {
        ...
        "transparent": args.transparent, "colorMode": scene.render.image_settings.color_mode,
        "floorPresent": not args.transparent,
