...
const pawMode=process.argv.includes('--paw'),inputFile=pawMode?'Fox.paws.glb':'Fox.adapted.glb',outputFile=pawMode?'Fox.paws-motion.glb':'Fox.adapted-motion.glb',reportFile=pawMode?'paw-motion-review.json':'motion-review.json',catalogueFile=pawMode?'paw-motion-catalogue.json':'motion-catalogue.json';
...
function solve(f,target,orientation){
  ...
  const chain=f.hock?f.chain.slice(0,3):f.chain,end=f.hock??f.end,goal=f.hock?target.clone().sub(f.lastSegment):target;
  ...
  if(f.hock)setWorldQ(f.hock,f.hockQ);
...
for(const clip of nativeClips.filter(c=>['Walk','Run'].includes(c.name))){
  ...
  const speed=run?.85:.58,footConfigs=footDefs.map((f,fi)=>{
  ...
  const frames=Math.ceil(clip.duration*240),times=[],values=new Map(joints.map(n=>[n,[]])),hipValues=[],frameReports=[];
  ...
  for(let i=-frames*2;i<=frames;i++){
    ...
    const goals=footDefs.map((f,fi)=>{const c=footConfigs[fi],u=frac(time/clip.duration-c.start),stance=u<c.duty;
      ...
      const z=c.centreZ+c.sweep*(stance?.5-progress:-.5+smooth(progress));
      const y=stance?0:c.lift*Math.sin(Math.PI*progress)**2;
    ...
    let lower=0;
    for(let f=0;f<footDefs.length;f++){const hip=footDefs[f].chain[0].getWorldPosition(vec()),goal=footDefs[f].hock?goals[f].target.clone().sub(footDefs[f].lastSegment):goals[f].target,reach=(footDefs[f].length-(footDefs[f].lastSegment?.length()??0))*.97,xz=Math.hypot(hip.x-goal.x,hip.z-goal.z);const maxY=goal.y+Math.sqrt(Math.max(.0001,reach*reach-xz*xz));lower=Math.max(lower,hip.y-maxY);}
    ...
    for(let fi=0;fi<footDefs.length;fi++){
      ...
      for(let pass=0;pass<5;pass++){
        ...
        if(residual.length()<.00008)break;g.target.add(residual);
      }
...
const report={schema:1,frozenGeometrySha256:frozenHash,motionSha256:hash(bytes),geometryChanged:false,originalGeometryVariantUnchanged:true,
  method:'Native-joint analytic two-bone solves with fixed anatomical bend poles, authored explicit stance paths, physical adapted-sole residual feedback and reach-driven pelvis lowering. Native Survey is unchanged; Walk/Run are separately marked corrected derivatives.',
...
for(const b of baked){
 ...
 let minimum={y:Infinity},maxStanceHeight=0,maxStancePathError=0,maxStanceDrift=0;const previous=new Map(),maxJointStep=new Map();
 ...
 for(let i=0;i<=steps;i++){
  ...
  for(let fi=0;fi<footDefs.length;fi++){const f=footDefs[fi],c=config.feet[fi],p=measureFoot(f),u=frac(time/clip.duration-c.start),stance=u<c.duty,z=c.centreZ+c.sweep*(.5-u/c.duty);if(stance){maxStanceHeight=Math.max(maxStanceHeight,Math.abs(p.minY));maxStancePathError=Math.max(maxStancePathError,Math.hypot(p.centroid.x-c.centreX,p.centroid.z-z));const old=previous.get(f.id);if(old?.stance&&Math.abs(u-old.u)<.1)maxStanceDrift=Math.max(maxStanceDrift,Math.hypot(p.centroid.x-old.p.x,p.centroid.z-old.p.z+config.targetTravelMps*(time-old.time)));}previous.set(f.id,{p:p.centroid,stance,u,time});}
 ...
 report.serializedAudit.push({clip:b.clip.name,samples:steps+1,minimum,maxStanceSoleHeightM:maxStanceHeight,maxStancePathErrorM:maxStancePathError,maxConsecutiveWorldStanceDriftM:maxStanceDrift,maximumQuarterKeyJointAngleRadians:Math.max(...[...maxJointStep.values()].map(x=>x.max)),soleGoal1mmPassed:minimum.y>=-.001&&maxStanceHeight<=.001});
}
...
report.reviewRisks=[`Reach-driven pelvis lowering reaches ${Math.round(configs[0].maxPelvisLowering*1000)} mm Walk and ${Math.round(configs[1].maxPelvisLowering*1000)} mm Run; crouch and hock/limb presentation need hardware visual review.`,'Stance phases are newly regularized from native sole minima. Original durations are preserved, but this is a derivative gait, not untouched native motion.','Native Survey remains unchanged and retains approximately 1.32 mm penetration.','Combat, directional hit and death clips are absent.'];
await writeFile(new URL(reportFile,out),JSON.stringify(report,null,2)+'\n');
