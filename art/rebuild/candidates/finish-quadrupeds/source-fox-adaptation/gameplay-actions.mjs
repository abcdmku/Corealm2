...
await writeFile(new URL('gameplay-draft-review.json',out),JSON.stringify(report,null,2)+'\n');const catalogue=JSON.parse(await readFile(new URL('reach-prototype-catalogue.json',out),'utf8'));catalogue.files.creature_redbrush_fox=outputFile;catalogue.scope='Source-derived Fox gameplay draft, eight distinct clips; hardware review pending';const asset=catalogue.assets[0];asset.sha256=hash(bytes);asset.bytes=bytes.length;asset.animations=serialized.getRoot().listAnimations().map(a=>a.getName());asset.reviewOnly.missingProductionBehaviors=[];asset.gameplayRoleProvenance=roles;asset.motionReview.candidateFile=outputFile;asset.motionReview.report='gameplay-draft-review.json';await writeFile(new URL('gameplay-draft-catalogue.json',out),JSON.stringify(catalogue,null,2)+'\n');console.log(JSON.stringify(report));
