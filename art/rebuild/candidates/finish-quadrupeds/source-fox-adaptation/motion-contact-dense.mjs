...
const pawMode=process.argv.includes('--paw'),inputFile=pawMode?'Fox.paws.glb':'Fox.adapted.glb',outputFile='Fox.contact-dense.glb',reportFile='contact-dense-review.json',catalogueFile='contact-dense-catalogue.json';
...
for(const clip of nativeClips.filter(c=>['Walk','Run'].includes(c.name))){
  ...
  const frames=Math.ceil(clip.duration*(run?3840:240)),times=[],values=new Map(joints.map(n=>[n,[]])),hipValues=[],frameReports=[];
