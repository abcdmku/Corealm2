...
const pawMode=process.argv.includes('--paw'),inputFile=pawMode?'Fox.paws.glb':'Fox.adapted.glb',outputFile='Fox.reach-prototype.glb',reportFile='reach-prototype-review.json',catalogueFile='reach-prototype-catalogue.json';
...
for(const clip of nativeClips.filter(c=>['Walk','Run'].includes(c.name))){
  if(clip.name==='Run'){clip.nativeDuration=clip.duration;clip.duration=.55;}
  ...
  const frames=Math.ceil(clip.duration*(run?480:240)),times=[],values=new Map(joints.map(n=>[n,[]])),hipValues=[],frameReports=[];
