...
const inputFile='Fox.contact-smooth.glb',outputFile='Fox.gameplay-draft.glb';
...
const previousReport=JSON.parse(await readFile(new URL('contact-smooth-review.json',out),'utf8')),evidence=[];
...
await writeFile(new URL('contact-smooth-velocity-review.json',out),JSON.stringify({sourceSha256:frozenHash,basis:'Physical sole centroid differences at every quarter key; adjacent samples both physical sole minima <= floor+0.5mm, without stance-label filtering',evidence},null,2)+'\n');console.log(JSON.stringify(evidence));
