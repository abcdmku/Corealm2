...
const inputFile='Fox.reach-prototype.glb',outputFile='Fox.gameplay-draft.glb';
...
const previousReport=JSON.parse(await readFile(new URL('reach-prototype-review.json',out),'utf8')),evidence=[];
for(const name of ['Walk','Run']){const clip=nativeClips.find(c=>c.name===name),config=previousReport.configs.find(c=>c.clip===name),steps=previousReport.serializedAudit.find(c=>c.clip===name).samples-1,stats=new Map(footDefs.map(f=>[f.id,{maxActualXZVelocityMps:0,minRearwardVelocityMps:Infinity,maxRearwardVelocityMps:-Infinity,maxWorldResidualVelocityMps:0}])),last=new Map();
 for(let i=0;i<=steps;i++){const t=i*clip.duration/steps;nativePose(clip,t);for(let fi=0;fi<footDefs.length;fi++){const f=footDefs[fi],c=config.feet[fi],u=frac(t/clip.duration-c.start),stance=u<c.duty,p=measureFoot(f).centroid,old=last.get(f.id);if(stance&&old?.stance&&Math.abs(u-old.u)<.1){const dt=t-old.t,vx=(p.x-old.p.x)/dt,vz=(p.z-old.p.z)/dt,s=stats.get(f.id);s.maxActualXZVelocityMps=Math.max(s.maxActualXZVelocityMps,Math.hypot(vx,vz));s.minRearwardVelocityMps=Math.min(s.minRearwardVelocityMps,-vz);s.maxRearwardVelocityMps=Math.max(s.maxRearwardVelocityMps,-vz);s.maxWorldResidualVelocityMps=Math.max(s.maxWorldResidualVelocityMps,Math.hypot(vx,vz+config.targetTravelMps));}last.set(f.id,{p,t,u,stance});}}
 evidence.push({clip:name,duration:clip.duration,samples:steps+1,feet:Object.fromEntries(stats)});
}
await writeFile(new URL('stance-velocity-review.json',out),JSON.stringify({sourceSha256:frozenHash,basis:'Physical sole centroid differences at every quarter key; only adjacent samples both classified stance, including stance boundary intervals',evidence},null,2)+'\n');console.log(JSON.stringify(evidence));
