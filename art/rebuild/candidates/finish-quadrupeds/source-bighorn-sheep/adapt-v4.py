...
ROOT=ROOT/'v4';ROOT.mkdir(exist_ok=True)
...
def warp(point):
    ...
    result=Vector((x*.985 - .052*head*smooth(1.13, 1.42, x),
                   y*(.90 - .16*head + .17*neck + .075*chest),
                   z + .065*neck + .045*chest - .026*back + .060*waist
                   - .027*head*smooth(1.15, 1.42, x)))
    # Broaden the source's needle-like toes within the same weighted surface.
    foot=1-smooth(-1.57,-1.40,z)
    cx=(.60 if x>0 else -.61);cy=(.25 if y>0 else -.28)
    result.x+=(x-cx)*.52*foot
    result.y+=(y-cy)*.65*foot
    pastern=gaussian(z,-1.38,.12)
    result.x-=(x-cx)*.15*pastern;result.y-=(y-cy)*.20*pastern
    # Compress the curved underside into a short weight-bearing sole.
    if z<-1.585:result.z=-1.610+(z+1.610)*.30
    return result
...
rest_bones={bone.name:(bone.head.copy(),bone.tail.copy(),bone.roll,bone.use_connect) for bone in rig.data.edit_bones}
for bone in rig.data.edit_bones:bone.use_connect=False
for bone in rig.data.edit_bones:
    head,tail,roll,connected=rest_bones[bone.name]
    bone.head,bone.tail=warp(head),warp(tail);bone.roll=roll
for bone in rig.data.edit_bones:bone.use_connect=rest_bones[bone.name][3]
...
def tint(point):
    x,y,z=point
    base=Vector((.22,.155,.095))
    shoulder=gaussian(x,.45,.36)*smooth(-.78,-.20,z)
    base=base.lerp(Vector((.14,.105,.075)),.45*shoulder)
    back=smooth(-.47,-.12,z)*(1-smooth(.7,1,x));base=base.lerp(Vector((.18,.13,.085)),.35*back)
    pale=max((1-smooth(-.79,-.62,x))*smooth(-1.02,-.71,z),smooth(1.21,1.40,x)*(1-smooth(-.05,.12,z)))
    base=base.lerp(Vector((.81,.76,.64)),pale*.84)
    hoof=(1-smooth(-1.565,-1.46,z));base=base.lerp(Vector((.035,.032,.028)),hoof*.98)
    flank=gaussian(x,-.03,.45)*gaussian(z,-.65,.22);base=base.lerp(Vector((.30,.22,.14)),flank*.20)
    eye=gaussian(x,1.155,.024)*gaussian(abs(y),.104,.045)*gaussian(z,.060,.017)
    nose=smooth(1.30,1.36,x)*(1-smooth(-.15,-.07,z))
    base=base.lerp(Vector((.014,.009,.006)),min(1,eye*1.8))
    base=base.lerp(Vector((.09,.065,.045)),nose*.82)
    return (*base,1)
...
# Local hoof retopology: the continuous source pastern remains above a short
# front cleft. No separate balls, blocks or cuffs are attached to the legs.
deformation_ratios=[(body.data.vertices[a].co-body.data.vertices[b].co).length/(original[a]-original[b]).length for a,b in old_edges if (original[a]-original[b]).length>1e-8]
bpy.context.view_layer.objects.active=body
bpy.ops.object.modifier_apply(modifier=subdivision.name)
hoof_centers=[(.65,.267,'forearm L'),(.63,-.292,'forearm R'),(-.588,.245,'Shin L'),(-.49,-.283,'Shin R')]
for x,y,bone_name in hoof_centers:
    bpy.ops.mesh.primitive_cube_add(size=1)
    cutter=bpy.context.object;cutter.name='Temporary_source_hoof_cleft'
    cutter.matrix_world=rig.matrix_world@Matrix.Translation((x+.071,y,-1.62))@Matrix.Diagonal((.18,.016,.13,1))
    bpy.context.view_layer.objects.active=body
    modifier=body.modifiers.new('Retopologized_hoof_cleft','BOOLEAN');modifier.operation='DIFFERENCE';modifier.solver='EXACT';modifier.object=cutter
    bpy.ops.object.modifier_apply(modifier=modifier.name)
    bpy.data.objects.remove(cutter,do_unlink=True)
    ids=[v.index for v in body.data.vertices if v.co.z<-1.47 and abs(v.co.x-x)<.24 and abs(v.co.y-y)<.12]
    for group in body.vertex_groups:group.remove(ids)
    body.vertex_groups[bone_name].add(ids,1,'REPLACE')
body['source_topology_retained']=False
body['source_base_vertices']=len(original)
body['topology_changes']='Interpolating subdivision followed by four narrow distal hoof clefts; source pastern remains continuous; distal foot weights assigned to original corresponding forearm/shin bone'
for polygon in body.data.polygons:
    polygon.use_smooth=True
    for loop in polygon.loop_indices:body.data.color_attributes['BighornCoat'].data[loop].color=tint(body.data.vertices[body.data.loops[loop].vertex_index].co)
from mathutils.kdtree import KDTree
tree=KDTree(len(original))
for i,p in enumerate(original):tree.insert(warp(p),i)
tree.balance()
mapping=[tree.find(v.co)[1] for v in body.data.vertices]
(ROOT/'source-vertex-nearest-map.json').write_text(json.dumps({'mappingKind':'Nearest original deformed vertex, descriptive correspondence only; subdivision interpolates source UVs and body weights','sourceCount':len(original),'resultCount':len(mapping),'nearestSourceVertex':mapping}))
...
horn_controls=[
    [1.035,.080,.090,.051,.073], [.935,.158,.190,.073,.106],
    [.805,.235,.259,.077,.112], [.649,.281,.211,.070,.107],
    [.548,.306,.080,.062,.092], [.554,.315,-.081,.054,.077],
    [.650,.313,-.206,.044,.061], [.799,.302,-.234,.034,.044],
    [.929,.281,-.180,.024,.030], [1.014,.258,-.085,.013,.018],
    [1.027,.246,-.010,.004,.006],
]
...
horn_shader=horn_material.node_tree.nodes.get('Principled BSDF');horn_shader.inputs['Base Color'].default_value=(.21,.155,.095,1);horn_shader.inputs['Roughness'].default_value=.83
horns=[]
for side in [-1,1]:
...
for name,x,y in [('ForeL',.60,.25),('ForeR',.60,-.29),('HindL',-.64,.25),('HindR',-.65,-.28)]:
    ids=[v.index for v in body.data.vertices if v.co.z<-1.56 and abs(v.co.x-x)<.25 and abs(v.co.y-y)<.13]
    feet[name]=ids
motion=[]
...
bpy.ops.export_scene.gltf(filepath=str(out),export_format='GLB',use_selection=True,export_animations=True,
    ...
    export_apply=True,export_extras=True,export_materials='EXPORT',export_image_format='AUTO')
ratios=deformation_ratios
report={'source':str(SOURCE),'sourceSha256':EXPECTED,'sourceMesh':'Sheep 2','sourceVertices':len(original),
    'retainedBodyVertices':len(body.data.vertices),'retainedBodyPolygons':len(body.data.polygons),'sourceTopologyRetained':False,
    'topologyChanges':body['topology_changes'],'sourceVertexMapping':'source-vertex-nearest-map.json',
