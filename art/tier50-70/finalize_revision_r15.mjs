...
const revision=await json('runs/tier50-70/evidence/revision-r15.json');
assert.equal(revision.passed,true);assert.equal(revision.round,'r15');
const contacts=await json(`${evidence}/contacts-r15.json`);
...
assert.equal(viewer.passed,true);assert.equal(viewer.round,'r15');
const details=await json(`${evidence}/details-r15.json`);
assert.equal(details.passed,true);assert.equal(details.round,'r15');
for(const asset of details.assets){
 const match=new URL(asset.url).pathname.match(/^\/models\/(dragonhide|starhide)\/(\w+)\.glb$/);assert(match);
 assert.equal(await fileHash(`art/item-models/candidates/armor-${match[1]}-reference/models/items/${match[1]}_${match[2]}.glb`),asset.sha256);
}
for(const shot of details.shots){
 assert.equal(await fileHash(shot.file),shot.sha256);
 await copyFile(shot.file,`${evidence}/${shot.file.split('/').at(-1)}`);
}
...
for(const [tier,theme,design,color] of [[50,'dragonhide','starhide','midnight navy and silver'],[70,'starhide','dragonhide','deep maroon and antique gold']]){
 ...
 for(const [kind,suffix] of [['worn','chest'],['pieces','pieces'],['world','world']]){
  const source=kind==='world'?'test-results/item-models/tier50-70-r15-world/report.json':`test-results/item-models/tier${tier}-r15-${suffix}/report.json`,report=await json(source);
 ...
 for(const [suffix,names] of [['chest',['front','side','back','casting','armor-walk-front-50','armor-walk-side-50','armor-walk-rear-50']],['world',[`${theme}-front`,`${theme}-back`,`${theme}-walking`]]]){
  for(const name of names){
   ...
   const sourceDirectory=suffix==='world'?'test-results/item-models/tier50-70-r15-world':`test-results/item-models/tier${tier}-r15-${suffix}`;
 ...
 assert.equal(validation.candidateId,`tier${tier}-${theme}-r15-review`);assert.equal(validation.passed,true);
 ...
 const renders=[];
 for(const part of ['studio','hood','robe','leggings','boots','wraps',...(theme==='starhide'?['robe-back']:[])]){
...
const report={round:'r15',date:new Date().toISOString(),status:'native-male-game-assets-delivered',productionPromoted:true,exactReferenceAccepted:false,
 requestedRevisions:['Complete blue/silver design assigned to T50; complete red/gold design assigned to T70. Equipment IDs and progression retained.',
  'Calmer fabric noise with moderately more celestial embroidery retained from R13.','Normal native-male gameplay loads the authored GLBs.',
  'T50 redundant crossing inner chest bands and tiny trim removed; inner silver V resampled to follow the cloth. Fitted navy underlay and long silver lapels preserved.',
  'T70 two rear flanking cloth panels replaced with dimensional dark iridescent scales, retaining their gold borders and pointed flare.',
  'Previous chest coverage and knee clearance refinements retained.'],
 appearanceReview:{passed:true,critic:'Fresh read-only R15 critic reviewed the T50 chest closeups, T70 rear scale panels and current production lab views. Root inspected final lab, world and Blender views.',
  report:`${evidence}/revision-r15.json`,viewer:`${evidence}/material-viewer.json`,details:`${evidence}/details-r15.json`,
 ...
 validation:{focusedTests:93,focusedFiles:5,typecheck:'passed',productionBuild:'passed',individualLabViews:30,walkingPhaseViews:18,castingFixtures:2,
  kneeFrontClothContactReport:`${evidence}/contacts-r15.json`,sampledJogPhasesPerSet:20,
  normalWorldKits:2,normalPlayerCamera:true,canonicalBlenderScenes:2,renderedPreviews:13},
...
console.log(JSON.stringify({verified:true,round:'r15',sets:sets.length,assets:sets.flatMap(s=>s.assets).length,productionPromoted:true}));
