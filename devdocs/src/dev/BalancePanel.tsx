...
const sourceFiles: Record<string, string> = { gear: gearSource, gearProgression: progressionSource, recipes: recipesSource, sets: setsSource, jewelry: jewelrySource, campfires: campfiresSource, itemFormula: itemFormulaSource, materialFood: materialFoodSource };
...
function SupportedPanel({ collection, moduleName, recordId, supportedKinds }: { collection: string; moduleName: string; recordId?: string; supportedKinds: string[] }) {
  ...
  const example = useMemo(() => {
    ...
    if (tier !== undefined) { if (["jewelry", "jewelryRecipe"].includes(kind)) tag.tier = tier; else if (kind === "gearProgression") tag.ladderTier = tier; else input.tier = tier; }
    ...
    try {
      ...
      return { input, output, changed: !sameValue(Object.fromEntries(Object.keys(output).map(key => [key, selected[key]])), output) };
    } catch (error) { return { error: error instanceof Error ? error.message : "This example could not be calculated." }; }
  }, [selected, params, tier, shape, kind, config.collection]);
  ...
  const shownTier = tier ?? Number(kind === "gearProgression" ? tag.ladderTier : ["jewelry", "jewelryRecipe"].includes(kind) ? tag.tier : selected?.tier ?? 1);
