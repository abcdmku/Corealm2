...
export function CommandPalette({ open, onOpenChange, collections, navigate }: { open: boolean; onOpenChange: (open: boolean) => void; collections: CollectionSummary[]; navigate: AppProps["navigate"] }) {
  ...
  const matches = useMemo(() => needle ? queries.flatMap(q => q.data ? displayRows(q.data, queries.find(query => query.data?.collection.name === "enemies")?.data).map(record => ({ collection: q.data!.collection.name, id: rowId(record, q.data!.collection.idKey), name: rowName(record, q.data!.collection.idKey) })) : []).filter(r => `${r.name} ${r.id}`.toLowerCase().includes(needle)).slice(0, 40) : [], [needle, queries]);
