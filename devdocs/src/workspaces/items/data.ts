...
/*
  ...
  the item page and the drawers all agree on the same records.
*/
...
export interface SetRecord extends ContentRow {
  ...
  thresholds?: { pieces: number; bonuses: Partial<Record<BonusKey, number>> }[];
}
...
export const SLOT_LABELS: Readonly<Record<string, string>> = {
  head: "Head", body: "Body", legs: "Legs", hands: "Hands", feet: "Feet", mainHand: "Main hand", offHand: "Off hand",
  accessory1: "Accessory 1", accessory2: "Accessory 2", ring2: "Ring 2", earring2: "Earring 2",
};
export const EQUIP_SLOT_OPTIONS = ["head", "body", "legs", "feet", "hands", "mainHand", "offHand", "accessory1", "accessory2", "ring2", "earring2"] as const;
export const SKILL_OPTIONS = ["melee", "magic", "mining", "woodcutting", "fishing", "smithing", "crafting", "cooking", "fletching", "agility"] as const;
export const CATEGORY_OPTIONS = ["resource", "bar", "equipment", "food", "tool", "quest", "currency", "component"] as const;
export const ELEMENT_OPTIONS = ["wind", "water", "earth", "fire"] as const;
export const STATION_LABELS: Readonly<Record<string, string>> = { furnace: "furnace", anvil: "anvil", range: "range", campfire: "campfire", crafting_table: "crafting table", fletching_bench: "fletching bench", essence_altar: "essence altar" };

export const titleCase = (value: string): string => value.replace(/[_-]+/g, " ").replace(/([a-z])([A-Z])/g, "$1 $2").replace(/^./, c => c.toUpperCase());
export const stationText = (stations: readonly string[] | null | undefined): string => stations && stations.length ? stations.map(station => STATION_LABELS[station] ?? station).join(" or ") : "anywhere";
export const seconds = (ms: number | undefined): string => ms === undefined ? "—" : `${Number((ms / 1000).toFixed(2))} s`;

/** Short labels for the bonus keys, for cells and threshold text. */
...
export function useItemsData() {
  ...
  return { tiers, families: familyRows, templates: templateRows, authored, compiled, recipes, sets: setRows, item, familyById, templateById, madeBy, loading, error, itemsEditable: Boolean(items.data?.collection.editable) };
}
...
export function targetThresholds(balance: SetBalance | undefined, tier: number | undefined): { pieces: number; bonuses: Partial<Record<BonusKey, number>> }[] | undefined {
