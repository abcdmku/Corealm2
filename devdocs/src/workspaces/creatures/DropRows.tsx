...
export function DropRows({ drops, onChange, readOnly = false, emptyText = "No drops." }: { drops: Drop[]; onChange: (drops: Drop[]) => void; readOnly?: boolean; emptyText?: string }) {
  ...
  return <WeightedList<Drop> items={drops} probabilityKey="chance" onChange={onChange} readOnly={readOnly} emptyText={emptyText} className="drop-rows"
    keyOf={(drop, index) => `${drop.itemId}:${index}`}
    removeLabel={drop => `Remove ${drop.itemId}`}
    addControl={readOnly ? undefined : <RefField kind={ITEM.ref} label="Add drop" value={undefined} onChange={add} className="drop-add" />}
    renderItem={(drop, api) => <>
      <RefField kind={ITEM.ref} label={`${ITEM.label} ${api.index + 1}`} value={drop.itemId} readOnly={readOnly} onChange={next => { if (next) api.update({ ...drop, itemId: next }); }} className="drop-item" />
