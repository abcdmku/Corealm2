...
import { RefField, StackField, TextField, WeightedList, fieldFromSchema } from "../../ui/field/index.js";
...
export function DropRows({ drops, onChange, readOnly = false, emptyText = "No drops." }: { drops: Drop[]; onChange: (drops: Drop[]) => void; readOnly?: boolean; emptyText?: string }) {
  ...
  return <WeightedList<Drop> items={drops} probabilityKey="chance" onChange={onChange} readOnly={readOnly} emptyText={emptyText} className="max-w-[55rem]"
    ...
    renderItem={(drop, api) => <>
      <StackField kind={ITEM.ref} label={`${ITEM.label} ${api.index + 1}`} value={drop.itemId} readOnly={readOnly} onChange={next => { if (next) api.update({ ...drop, itemId: next }); }} className="w-72"
        quantity={drop.quantity} onQuantityChange={next => api.update({ ...drop, quantity: next as [number, number] })} />
