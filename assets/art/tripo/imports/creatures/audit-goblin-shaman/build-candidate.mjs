...
const candidate={schema:'corealm-goblin-shaman-polish/1',id:'creature_goblin_shaman',displayName:'Goblin Shaman',status:'awaiting-root-lab-review',accepted:false,source:{file:source,sha256:sha(sourceBytes),bytes:sourceBytes.length,provenance:'Quaternius CC0 body, outfit and animations; Blink staff Standard Unity Asset Store EULA, per production manifest'},candidate:{file:output,sha256:sha(binary),bytes:binary.length,geometry:'preserved',skin:'preserved 65-joint source rig',maps,attack:{durationSeconds:.5,contactSeconds:.21,overlay:altered},animations:root.listAnimations().map(a=>a.getName()),productionTarget:source},acceptance:{cpuValidated:false,labAccepted:false,worldIntegrated:false}};
await writeFile(`${dir}/catalog.json`,JSON.stringify(candidate,null,2)+'\n');
const lab={schema:'corealm-lab-asset-candidates/1',assets:[{id:candidate.id,file:'models/creature/creature_goblin_shaman.glb',pack:'corealm-goblin-shaman-polish',category:'character',is:candidate.displayName,tags:['creature','goblin','shaman','caster','candidate'],bytes:binary.length,sha256:sha(binary),size:{x:.47444,y:1.3284,z:1.50317},base:{x:-.26779,y:.002,z:-.90358},groundY:.002,triangles:19861,animations:candidate.candidate.animations,materials:materials.map(m=>m.getName()),attackSeconds:.5,contactNormalized:.42,sourceProvenance:candidate.source,acceptance:{assetAudit:true,rigAccepted:false,motionAccepted:false,texturesAccepted:false,labAccepted:false,worldIntegrated:false}}],files:{creature_goblin_shaman:'goblin-shaman-polish-candidate.glb'}};
