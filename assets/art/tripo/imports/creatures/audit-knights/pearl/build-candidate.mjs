...
const baseDir = 'assets/art/tripo/imports/creatures/audit-knights/pearl';
...
const presentationScale = 0.5071496596541646; // game/content/data/creatureDefinitions.json
const renderedHeight = 2.30;
const modelScale = renderedHeight / presentationScale / 0.990234375;
...
if (sourceBounds.min[1] !== 0 || Math.abs(sourceBounds.max[1] - 0.990234375) > 1e-7
  || Math.abs(bounds.max[1] * presentationScale - renderedHeight) > 1e-6) throw new Error('Starred source/runtime scale bounds changed.');
...
addClip('Death', 1.45, [
  ...
  { node: 'mixamorigHips', times: [0, 0.22, 0.65, 1.05, 1.45], values: [quat('z', 0), quat('z', -0.12), quat('z', -0.75), quat('z', -1.35), quat('z', -1.35)] },
...
const guardPoseProof = {
  ...
  nativeScaleMeters: renderedHeight / presentationScale,
...
for (const side of ['left', 'right']) {
  ...
  if (arm.guardMeanAbsX >= arm.sourceMeanAbsX * 0.55 || furthestGuardX > 0.72 * renderedHeight / presentationScale / 2.25) throw new Error(`${side} arm deformation remains too far outside the torso: ${JSON.stringify(arm)}.`);
...
for (const texture of root.listTextures()) {
  ...
  if (texture === baseColorTexture) texture.setImage(await readFile(`${baseDir}/runtime-atlas.jpg`));
  runtimeTextures.push({ name: texture.getName(), width: resized.width, height: resized.height, mime: texture.getMimeType(), bytes: texture.getImage().length, sha256: createHash('sha256').update(texture.getImage()).digest('hex') });
}
...
const motionSummary = {
  ...
  nativeUnits: `meters at ${(renderedHeight / presentationScale).toFixed(3)} m model height`,
  proposedCreatureScale: presentationScale,
...
const resizedMotionScale = (renderedHeight / presentationScale) / 3.3;
requireMotion('Walk', 0.25, 'legs', 0.48 * resizedMotionScale);
requireMotion('Run', 0.25, 'legs', 0.78 * resizedMotionScale);
requireMotion('Attack', 0.36, 'arms', 0.60 * resizedMotionScale);
requireMotion('Hit', 0.13, 'torso', 0.40 * resizedMotionScale);
...
const candidate = {
  ...
  accepted: false,
  sourceComparison: { productionSha256: '710803cf7333cff199899631d5ff765e46096bf02db96646fe793beb20e32ac1', productionTriangles: 29484, candidateDiffers: true, assessment: 'Detailed engraved white/silver plate and tabard are stronger than the generic production gauntlet knight. Missing pearl nacre, shell lamellar, sword and shield; stage as a base pending art and equipment review.' },
  source: {
    file: sourcePath,
    sha256: sourceHash,
  ...
  candidate: {
    ...
    rig: { type: 'Mixamo-named Unity Humanoid glTF skin', joints: bones.map((bone) => ({ name: bone.name, parent: bone.parent, position: bone.p })), influencesPerVertex: 4, distributedVertices, maximumWeightSumError: maxWeightError, method: `Source-specific four-weight distance fields fitted to the original T-pose; a close, slightly bent guard bind tucks the arms and keeps the wrist aligned with the glove. All six clips begin from that pose. Original topology and UVs remain untouched; ${renderedHeight} m rendered height after ${presentationScale} definition.scale.`, offlineDeformationProof: guardPoseProof },
    textures: runtimeTextures,
    textureOverride: { file: `${baseDir}/runtime-atlas.jpg`, sha256: createHash('sha256').update(baseColorTexture.getImage()).digest('hex'), width: 2048, height: 2048, imagegenCoverage: 0.78 },
    renderedHeightMeters: renderedHeight,
    gamePresentationScale: presentationScale,
    metallicChannelRange: metalRange,
    roughnessChannelRange: roughRange,
    pbr: 'Image-generated pearl nacre, silver and ivory base-color details aligned to the original UV atlas; original Tripo packed metallic-roughness and normal maps retained at 2K.',
    animations: clips,
...
const labAsset = {
  ...
  runClipSeconds: 0.72,
  attackSeconds: 0.78,
  contactNormalized: 0.28 / 0.78,
  metadata: { is: 'Nacre-armored Pearl Knight', walkClipSeconds: 1, runClipSeconds: 0.72, attackSeconds: 0.78, contactNormalized: 0.28 / 0.78, contactSeconds: 0.28, contactBasis: 'Authored maximum right-arm slash and forward hip reach at 0.28 s in 0.78 s Attack clip.', renderedHeightMeters: renderedHeight, gamePresentationScale: presentationScale, equipmentNeeded: 'Sword and shield, if available' },
  sourceProvenance: {
    ...
    rigMethod: `22-joint Mixamo-named Unity Humanoid skeleton; four-weight anatomical fields; close guard bind pose and aligned gloves carried through Idle, locomotion and actions; source topology, normals and UV islands preserved at ${renderedHeight} m after ${presentationScale} game definition scale.`,
