...
const geometryHashes = { positions: hashArray(positions), normals: hashArray(normals), uvs: hashArray(uvs), triangleIndices: hashArray(indices) };
...
if (clips.some(clip => clip.channels < 1 || clip.targets.some(target => !target.bone || !target.path || !(target.keys > 1))) ||
    durations.some(clip => !(clip.seconds > 0))) throw new Error('An exported motion clip has no usable channels or timing.');
