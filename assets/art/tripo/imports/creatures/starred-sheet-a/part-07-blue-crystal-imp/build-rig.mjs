...
if(worldMatrixDelta>1e-6||Math.abs(candidateHeight-1)>0.01||Math.abs((candidateWorldBounds.min[0]+candidateWorldBounds.max[0])/2)>.01||Math.abs((candidateWorldBounds.min[2]+candidateWorldBounds.max[2])/2)>.01||Math.abs(candidateWorldBounds.min[1])>.01)throw new Error(`Presentation transform no longer preserves centered grounded 1m source bounds: ${JSON.stringify({sourceWorldBounds,candidateWorldBounds,worldMatrixDelta,candidateHeight})}`);
