...
const sourcePath = path.join(repo, 'assets/art/tripo/imports/creatures/starter-t1-images/exports/ryecrest.glb');
const candidatePath = path.join(here, 'ryecrest-native-rig_candidate.glb');
...
if (!scene) throw new Error('Ryecrest source has no scene');
const sourceArmature = scene.listChildren().find((n) => n.getName() === 'Armature');
const meshNode = sourceArmature?.listChildren().find((n) => n.getMesh());
const mesh = meshNode?.getMesh();
const primitive = mesh?.listPrimitives()[0];
if (!sourceArmature || !meshNode || !mesh || !primitive) throw new Error('Approved source mesh hierarchy was not found');
...
if (triangleCount !== 3964 || vertexCount !== 2780) throw new Error(`Unexpected approved geometry ${vertexCount} vertices / ${triangleCount} triangles`);
...
const rig = [
  ['RyecrestRoot', null, [0, 0, 0]],
  ['Pelvis', 'RyecrestRoot', [0, .43, 0]],
...
const rigContainer = doc.createNode('Ryecrest_native_rig');
scene.addChild(rigContainer);
rigContainer.addChild(meshNode);
const newSkin = doc.createSkin('Ryecrest groundbird skin');
...
newSkin.setSkeleton(nodeByName.get('RyecrestRoot'));
...
const buffer = root.listBuffers()[0] ?? doc.createBuffer('Ryecrest native rig and clips');
...
function poseFor(clip, t) {
  ...
  if (clip.name === 'Idle') {
    ...
    trans('Pelvis', 0, .006 * jab * (1 - recover), .045 * jab);
    rot('Chest', .10 * crouch + .04 * jab, 0, 0);
    rot('Neck', .10 * crouch + .32 * jab, 0, 0);
    rot('Head', .18 * crouch + .48 * jab, 0, 0);
    rot('Beak', .11 * jab, 0, 0);
    ...
    rot('Thigh_R', -.25 * jab, 0, 0); rot('Hock_R', .23 * jab, 0, 0); rot('Foot_R', .10 * jab, 0, 0);
    ...
    trans('Pelvis', 0, -.035 * collapse + .08 * smooth(.28, .60, s), -.025 * collapse);
    rot('Pelvis', 0, 0, .92 * collapse);
    rot('Spine', .06 * collapse, 0, .18 * collapse);
    rot('Chest', .08 * collapse, 0, .14 * collapse);
    rot('Neck', -.22 * flinch - .12 * collapse, .08 * collapse, 0);
    rot('Head', -.20 * flinch - .08 * collapse, -.12 * collapse, 0);
    ...
    rot('Thigh_L', -.24 * collapse, 0, 0); rot('Thigh_R', -.18 * collapse, 0, 0);
...
for (const texture of root.listTextures()) {
  ...
  const output = role === 'metallic-roughness' || role === 'occlusion'
    ? await resized.png({ compressionLevel: 9 }).toBuffer()
    : await resized.jpeg({ quality: role === 'normal' ? 94 : 90, mozjpeg: true }).toBuffer();
  const mimeType = role === 'metallic-roughness' || role === 'occlusion' ? 'image/png' : 'image/jpeg';
  texture.setImage(new Uint8Array(output)).setMimeType(mimeType);
...
function sameArray(a, b) { return a.length === b.length && Buffer.from(a.buffer, a.byteOffset, a.byteLength).equals(Buffer.from(b.buffer, b.byteOffset, b.byteLength)); }
...
if (jointCoverage.some((n, i) => ordered[i] !== 'RyecrestRoot' && n < 4)) throw new Error(`Unused/underused joint influences ${jointCoverage.map((n,i)=>ordered[i] !== 'RyecrestRoot' && n<4?`${ordered[i]}:${n}`:null).filter(Boolean).join(', ')}`);
...
if (jointNames.filter((name) => name !== 'RyecrestRoot' && !animatedBoneNames.has(name)).length) throw new Error(`Unanimated joints: ${jointNames.filter((name) => name !== 'RyecrestRoot' && !animatedBoneNames.has(name)).join(', ')}`);
...
function buildPoseObjects() {
  ...
  const sceneRoot = obj.get('RyecrestRoot'); sceneRoot.updateMatrixWorld(true);
  return obj;
}
...
function skinAt(clip, time) {
  ...
  poseObjects.get('RyecrestRoot').updateMatrixWorld(true);
...
const catalog = {
  ...
  id: 'creature_ryecrest',
  displayName: 'Ryecrest',
  ...
  source: {
    file: 'assets/art/tripo/imports/creatures/starter-t1-images/exports/ryecrest.glb',
    sha256: sourceSha,
    bytes: sourceBytes.length,
    sourceImage: 'assets/art/tripo/imports/creatures/starter-t1-images/ryecrest.png',
    sourceImageId: 'fcdb7bb1-5cc0-4bbc-95bb-7131b299dc9b',
    modelId: '85ab1f1d-a3cc-40ba-b9be-8e730843277b',
    ...
    sourceTextures: { baseColor: '8192x8192 JPEG', metallicRoughness: '4096x4096 PNG', normal: '4096x4096 JPEG' },
  },
  candidate: {
    ...
    notes: [
      'Approved Tripo P2.0 source geometry, topology, UV atlas, layered plumage albedo and PBR source maps are preserved; the broken Tripo bone tree and inverse-bind data are replaced because their rest transforms were absent and vertices were effectively bone_0-only.',
      'Ryecrest is a small Fallowmarch groundbird. Its rig uses a restrained neck/head/beak chain, tail, bilateral folded wings, thighs, hocks, feet, front toes and rear toes.',
...
// Existing-ID override catalog for the persistent production-backed lab. The route path remains
// the Turkey asset path so its existing in-game content can load the candidate without promotion.
const productionManifest = JSON.parse(await readFile(path.join(repo, 'game/public/assets/manifest.json'), 'utf8'));
const currentBird = productionManifest.assets.find((entry) => entry.id === 'creature_marchfield_turkey');
if (!currentBird || currentBird.file !== 'models/creature/creature_marchfield_turkey.glb') throw new Error('Expected Marchfield turkey lab override target/path was not found');
const {
  measuredGait: _retiredTurkeyGait,
  impliedWalkMps: _retiredTurkeyWalkSpeed,
  impliedRunMps: _retiredTurkeyRunSpeed,
  ...entryWithoutStaleGait
} = currentBird;
const labEntry = {
  ...entryWithoutStaleGait,
  id: 'creature_marchfield_turkey',
  file: currentBird.file,
  is: 'Ryecrest',
  tags: ['creature', 'ground', 'bird', 'field', 'ryecrest', 'skinned', 'articulated', 'plumage'],
  bytes: candidateBytes.length,
  sha256: createHash('sha256').update(candidateBytes).digest('hex'),
  ...
  sourceProvenance: {
    ...
    source: 'Approved Tripo Studio Smart Mesh P2.0 groundbird model and GPT Image 2.5 source image; original geometry/UVs preserved.',
    sourceImageId: 'fcdb7bb1-5cc0-4bbc-95bb-7131b299dc9b',
    modelId: '85ab1f1d-a3cc-40ba-b9be-8e730843277b',
    sourceFile: 'assets/art/tripo/imports/creatures/starter-t1-images/exports/ryecrest.glb',
  ...
  metadata: {
    ...
    attackSeconds: clips.find((clip) => clip.name === 'Attack').duration,
    contactNormalized: .46,
    notes: 'Small Fallowmarch field-margin groundbird. Uses a named 22-joint non-humanoid rig, layered 2K base color and PBR maps, and six authored clips. Candidate visuals and gameplay remain unaccepted.',
    boneNames: ordered,
  },
...
if (labCatalog.assets.length !== 1 || labEntry.id !== 'creature_marchfield_turkey' || labEntry.file !== currentBird.file) throw new Error('Lab catalog does not replace only the expected existing bird asset/path');
