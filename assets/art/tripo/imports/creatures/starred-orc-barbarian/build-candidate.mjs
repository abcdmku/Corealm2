...
const baseDir = 'assets/art/tripo/imports/creatures/starred-orc-barbarian';
const sourcePath = 'assets/art/tripo/exports/corealm_stonejaw_marauder_976e7fe5.glb';
const candidatePath = `${baseDir}/stonejaw-marauder-native-rig.glb`;
await mkdir(baseDir, { recursive: true });

const io = new NodeIO().registerExtensions(ALL_EXTENSIONS);
const sourceBytes = await readFile(sourcePath);
const sourceSha256 = createHash('sha256').update(sourceBytes).digest('hex');
if (sourceSha256 !== 'b1a8f9d1c7b90ccc64ae31a1dc7dead8e4ba6d90de8ffd0651c5d3dd5640012c') {
  throw new Error(`Orc source hash mismatch: ${sourceSha256}`);
}
...
if (!scene || !primitive || !meshNode || root.listSkins().length || root.listAnimations().length) {
  throw new Error('Expected the approved static Orc Barbarian GLB with no source skin or clips.');
}
...
if (positions.length / 3 !== 8089 || indices.length / 3 !== 4976) {
...
meshNode.setName('StonejawMarauderMesh');
const rigContainer = doc.createNode('StonejawMarauderArmature').setTranslation([0, 0, 0]).setRotation([0, 0, 0, 1]).setScale([1, 1, 1]);
...
const skin = doc.createSkin('StonejawMarauder_Humanoid').setSkeleton(jointNodes.get('mixamorigHips'));
...
skin.setInverseBindMatrices(doc.createAccessor('StonejawMarauder_InverseBind').setArray(inverseBinds).setType(Accessor.Type.MAT4).setBuffer(buffer));
...
for (let vertex = 0; vertex < positions.length / 3; vertex++) {
  ...
  for (const bone of bones) {
    let gate = 1;
    if (bone.group === 'head') gate = y >= 0.70 ? 1 : 0.015;
    if (bone.group === 'leftArm' || bone.group === 'rightArm') {
      gate = y > 0.235 && y < 0.92 ? 1 : 0.008;
...
primitive.setAttribute('JOINTS_0', doc.createAccessor('StonejawMarauder_Joints0').setArray(jointValues).setType(Accessor.Type.VEC4).setBuffer(buffer));
primitive.setAttribute('WEIGHTS_0', doc.createAccessor('StonejawMarauder_Weights0').setArray(weightValues).setType(Accessor.Type.VEC4).setBuffer(buffer));
...
if (!checkPositions || !checkIndices || !checkJoints || !checkWeights || !checkSkin) throw new Error('Exported candidate is missing required skin attributes.');
...
if (maxPositionDelta !== 0 || indexMismatches !== 0 || verticesWithDistributedWeights < positions.length / 3 * 0.75) {
  throw new Error(`Geometry/weights check failed: position delta ${maxPositionDelta}, index mismatches ${indexMismatches}, distributed ${verticesWithDistributedWeights}.`);
}
...
const candidate = {
  ...
  id: 'creature_stonejaw_marauder',
  displayName: 'Stonejaw Marauder',
  ...
  source: {
    ...
    starredModelId: '976e7fe5-366f-400d-8653-71e3e1c8c6b4',
    starredCardId: '1aafb2b0-2ec1-429e-9997-dda6f53d94c7',
    starredDisplayName: 'orc barbarian 3d model',
    ...
    preliminaryDesignReview: 'Astra-low accepted as a T10-20 fantasy brute; the source is posed with a hammer and needs hand/weapon motion review.',
  ...
  candidate: {
    ...
    productionTarget: 'game/public/assets/models/creature/creature_stonejaw_marauder.glb',
    geometry: { vertices: positions.length / 3, triangles: indices.length / 3, positionsPreserved: true, indicesPreserved: true },
...
const labAsset = {
  id: 'creature_stonejaw_marauder',
  file: 'models/creature/creature_stonejaw_marauder.glb',
  pack: 'corealm-starred-creatures',
  category: 'character',
  is: 'Stonejaw Marauder',
  tags: ['creature', 'humanoid', 'orc', 'martial', 'T10-T20', 'starred', 'tripo', 'candidate'],
  bytes: outputBytes.length,
  sha256: candidateSha256,
  ...
  sourceProvenance: {
    ...
    sourceModelId: '976e7fe5-366f-400d-8653-71e3e1c8c6b4',
    sourceCardId: '1aafb2b0-2ec1-429e-9997-dda6f53d94c7',
    sourceFile: sourcePath,
    sourceSha256,
    candidateFile: candidatePath,
    candidateSha256,
    rigMethod: 'Mixamo-named humanoid skeleton with spatially gated anatomical weights; original mesh indices, positions, UVs, and image-derived maps are retained.',
    textures: runtimeTextureMetrics,
