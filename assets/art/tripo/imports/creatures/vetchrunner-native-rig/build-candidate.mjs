...
const ownerDir = 'assets/art/tripo/imports/creatures/vetchrunner-native-rig';
const srcPath = 'assets/art/tripo/imports/creatures/starter-t1-images/exports/vetchrunner.glb';
const outPath = `${ownerDir}/vetchrunner-native-rig-candidate.glb`;
...
if (sourceVertexCount !== 2926 || sourceTriangleCount !== 4068) {
  throw new Error(`Source topology drifted: ${sourceVertexCount} vertices, ${sourceTriangleCount} triangles.`);
}

// Y is vertical; +Z is the muzzle/front, confirmed from the mesh's head and paw clusters.
const bones = [
  { name: 'VetchRoot', parent: null, p: [0, 0, 0], sigma: 1 },
  { name: 'Pelvis', parent: 'VetchRoot', p: [0, 0.300, -0.070], sigma: 0.125, group: 'body' },
  { name: 'SpineMid', parent: 'Pelvis', p: [0, 0.348, 0.085], sigma: 0.130, group: 'body' },
  { name: 'Chest', parent: 'SpineMid', p: [0, 0.365, 0.260], sigma: 0.120, group: 'body' },
  { name: 'Neck', parent: 'Chest', p: [0, 0.402, 0.365], sigma: 0.105, group: 'body' },
  { name: 'Head', parent: 'Neck', p: [0, 0.432, 0.445], sigma: 0.095, group: 'body' },
  { name: 'TailBase', parent: 'Pelvis', p: [0, 0.270, -0.225], sigma: 0.095, group: 'tail' },
  { name: 'TailMid', parent: 'TailBase', p: [0, 0.252, -0.320], sigma: 0.083, group: 'tail' },
  { name: 'TailEnd', parent: 'TailMid', p: [0, 0.244, -0.412], sigma: 0.073, group: 'tail' },
  { name: 'TailTip', parent: 'TailEnd', p: [0, 0.246, -0.478], sigma: 0.060, group: 'tail' },
  { name: 'ForeUpper_L', parent: 'Chest', p: [-0.060, 0.302, 0.300], sigma: 0.060, group: 'foreL' },
  { name: 'ForeLower_L', parent: 'ForeUpper_L', p: [-0.081, 0.164, 0.350], sigma: 0.050, group: 'foreL' },
  { name: 'ForeWrist_L', parent: 'ForeLower_L', p: [-0.082, 0.064, 0.392], sigma: 0.042, group: 'foreL' },
  { name: 'ForePaw_L', parent: 'ForeWrist_L', p: [-0.083, 0.023, 0.405], sigma: 0.042, group: 'foreL' },
  { name: 'ForeUpper_R', parent: 'Chest', p: [0.060, 0.302, 0.300], sigma: 0.060, group: 'foreR' },
  { name: 'ForeLower_R', parent: 'ForeUpper_R', p: [0.081, 0.164, 0.350], sigma: 0.050, group: 'foreR' },
  { name: 'ForeWrist_R', parent: 'ForeLower_R', p: [0.082, 0.064, 0.392], sigma: 0.042, group: 'foreR' },
  { name: 'ForePaw_R', parent: 'ForeWrist_R', p: [0.083, 0.023, 0.405], sigma: 0.042, group: 'foreR' },
  { name: 'HindUpper_L', parent: 'Pelvis', p: [-0.057, 0.286, 0.015], sigma: 0.066, group: 'hindL' },
  { name: 'HindLower_L', parent: 'HindUpper_L', p: [-0.093, 0.151, -0.052], sigma: 0.052, group: 'hindL' },
  { name: 'HindHock_L', parent: 'HindLower_L', p: [-0.104, 0.071, -0.022], sigma: 0.043, group: 'hindL' },
  { name: 'HindPaw_L', parent: 'HindHock_L', p: [-0.108, 0.023, 0.033], sigma: 0.042, group: 'hindL' },
  { name: 'HindUpper_R', parent: 'Pelvis', p: [0.057, 0.286, 0.015], sigma: 0.066, group: 'hindR' },
  { name: 'HindLower_R', parent: 'HindUpper_R', p: [0.093, 0.151, -0.052], sigma: 0.052, group: 'hindR' },
  { name: 'HindHock_R', parent: 'HindLower_R', p: [0.104, 0.071, -0.022], sigma: 0.043, group: 'hindR' },
  { name: 'HindPaw_R', parent: 'HindHock_R', p: [0.108, 0.023, 0.033], sigma: 0.042, group: 'hindR' },
];
...
meshNode.setName('VetchrunnerMesh');
rigContainer.setName('VetchrunnerRigContainer').setTranslation([0, 0, 0]).setRotation([0, 0, 0, 1]).setScale([1, 1, 1]);
...
const skin = doc.createSkin('Vetchrunner_Quadruped').setSkeleton(nodes.get('VetchRoot'));
...
const bindAccessor = doc.createAccessor('Vetchrunner_InverseBindMatrices').setArray(inverseBind).setType(Accessor.Type.MAT4).setBuffer(buffer);
...
for (let vertex = 0; vertex < sourceVertexCount; vertex++) {
  ...
  if (z < -0.12 && y < 0.40) allowed.add('tail');
  if (z > 0.27 && y > 0.19) allowed.add('body');
  if (y < 0.34) {
    const group = (z > 0.22 ? 'fore' : 'hind') + (x < 0 ? 'L' : 'R');
    const limb = bones.filter(bone => bone.group === group);
    const nearest = Math.min(...limb.map(bone => dist3(point, bone.p)));
    if (nearest < 0.14) allowed.add(group);
  }
  const candidates = [];
...
primitive.setAttribute('JOINTS_0', doc.createAccessor('Vetchrunner_Joints0').setArray(jointValues).setType(Accessor.Type.VEC4).setBuffer(buffer));
primitive.setAttribute('WEIGHTS_0', doc.createAccessor('Vetchrunner_Weights0').setArray(weightValues).setType(Accessor.Type.VEC4).setBuffer(buffer));
...
clip('Attack', 0.82, [0, 0.24, 0.50, 0.82], [
  { node: 'VetchRoot', path: 'translation', type: Accessor.Type.VEC3, values: [[0,0,0], [0,0,0.025], [0,0,0.085], [0,0,0]] },
...
clip('Hit', 0.42, [0, 0.08, 0.19, 0.42], [
  { node: 'VetchRoot', path: 'translation', type: Accessor.Type.VEC3, values: [[0,0,0], [0,0,-0.045], [0,0,-0.015], [0,0,0]] },
...
clip('Death', 1.55, [0, 0.25, 0.70, 1.15, 1.55], [
  { node: 'VetchRoot', path: 'translation', type: Accessor.Type.VEC3, values: [[0,0,0], [0,0.015,0], [0,0.040,0], [0,0.060,0], [0,0.060,0]] },
...
const validation = await validateCandidate(outPath, {sourceTriangleCount, sourceVertexCount, sourcePositions: positions, sourceIndices: indices, bones, boneIndex, influences, clips, minWeights, maxWeights});
const report = {
  schema: 'corealm-creature-native-rig-candidate/1',
  id: 'creature_vetchrunner',
  status: 'awaiting-root-visual-review',
  accepted: false,
  source: { file: '../starter-t1-images/exports/vetchrunner.glb', sha256: sourceSha256, bytes: sourceBytes.length, vertices: sourceVertexCount, triangles: sourceTriangleCount, sourceImageId: 'b589d56f-eaf7-4f04-92d8-cbbf75f82f2f', sourceModelId: 'd45e1741-b2eb-4ae6-b98a-8fdb18bd4fdf', generator: 'Tripo Smart Mesh P2.0', imageGenerator: 'Tripo GPT Image 2.5', approvedGeometryAndMapsPreserved: true, textures: sourceTextureMetrics, existingTripoSkinDiscarded: true },
  candidate: { file: 'vetchrunner-native-rig-candidate.glb', sha256: outputSha256, bytes: outputBytes.length, vertices: sourceVertexCount, triangles: sourceTriangleCount, joints: bones.map((bone, index) => ({ name: bone.name, index, parent: bone.parent, restPosition: bone.p })), maps: textureMetrics, clips: clips.map(clip => ({ name: clip.name, seconds: clip.duration, channels: clip.trackMap.length })) },
  rig: { basis: 'Y-up, +Z toward muzzle, X across shoulders', weightMethod: 'spatially gated anatomical influence fields fitted to source mesh bounds, four normalized influences per vertex; no bone_0 fallback', bind: 'translation-only rest matrices from the authored joint hierarchy; inverse bind matrices computed from those joint rest positions' },
  validation,
};
...
async function validateCandidate(file, ctx) {
  ...
  const deformAt = (animation, time) => {
    ...
    return { time, min, max, maximumDisplacement, displacedVertices };
  };
  ...
  for (const animation of anims) {
    ...
    if (sweptBounds.min[1] < -0.18 || sweptBounds.max[1] > 0.72 || Math.abs(sweptBounds.min[0]) > 0.32 || Math.abs(sweptBounds.max[0]) > 0.32 || Math.abs(sweptBounds.min[2]) > 0.78 || Math.abs(sweptBounds.max[2]) > 0.78) throw new Error(`Animation ${animation.getName()} produces implausible swept bounds: ${JSON.stringify(sweptBounds)}.`);
    clipReport.push({ name: animation.getName(), channels: animation.listChannels().length, sampledPoses: samples.length, maximumVertexDisplacement: Number(maximumDisplacement.toFixed(5)), maximumVerticesMoved: maximumDisplacedVertices, sweptBounds });
  }
  return {
