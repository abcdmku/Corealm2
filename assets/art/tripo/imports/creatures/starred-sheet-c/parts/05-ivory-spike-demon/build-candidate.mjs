...
const baseDir = 'assets/art/tripo/imports/creatures/starred-sheet-c/parts/05-ivory-spike-demon';
const sourcePath = 'assets/art/tripo/imports/creatures/starred-sheet-c/parts/05-ivory-spike-demon/base.glb';
const candidatePath = `${baseDir}/ivory-spike-demon-native-rig.glb`;
await mkdir(baseDir, { recursive: true });

const io = new NodeIO().registerExtensions(ALL_EXTENSIONS);
const sourceBytes = await readFile(sourcePath);
const sourceSha256 = createHash('sha256').update(sourceBytes).digest('hex');
if (sourceSha256 !== 'e77c1ba240227a1132890896c3f9fa78779316a95a80bffb8cbcc16b110fa4ff') {
...
if (positions.length / 3 !== 2864 || indices.length / 3 !== 1525) {
...
const skin = doc.createSkin('IvorySpikeDemon_Humanoid').setSkeleton(jointNodes.get('mixamorigHips'));
...
skin.setInverseBindMatrices(doc.createAccessor('IvorySpikeDemon_InverseBind').setArray(inverseBinds).setType(Accessor.Type.MAT4).setBuffer(buffer));
...
primitive.setAttribute('JOINTS_0', doc.createAccessor('IvorySpikeDemon_Joints0').setArray(jointValues).setType(Accessor.Type.VEC4).setBuffer(buffer));
primitive.setAttribute('WEIGHTS_0', doc.createAccessor('IvorySpikeDemon_Weights0').setArray(weightValues).setType(Accessor.Type.VEC4).setBuffer(buffer));
...
const candidate = {
  ...
  id: 'creature_ivory_spike_demon',
  displayName: 'Ivory Spike Demon',
...
const labAsset = {
  id: 'creature_ivory_spike_demon',
  file: 'models/creature/creature_ivory_spike_demon.glb',
  pack: 'corealm-starred-creatures',
  category: 'character',
  is: 'Ivory Spike Demon',
