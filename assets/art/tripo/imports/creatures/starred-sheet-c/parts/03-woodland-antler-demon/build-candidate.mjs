...
const dir = 'assets/art/tripo/imports/creatures/starred-sheet-c/parts/03-woodland-antler-demon';
const id = 'creature_woodland_antler_demon';
const displayName = 'Woodland Antler Demon';
...
meshNode.setName('WoodlandAntlerDemonMesh');
const armature=doc.createNode('WoodlandAntlerDemonArmature'); scene.addChild(armature); armature.addChild(meshNode);
...
const skin=doc.createSkin('WoodlandAntlerDemonHumanoid').setSkeleton(joints.get('mixamorigHips')); for(const bone of bones)skin.addJoint(joints.get(bone.name));
...
const buffer=root.listBuffers()[0]; skin.setInverseBindMatrices(doc.createAccessor('AntlerDemonInverseBind').setArray(ibm).setType(Accessor.Type.MAT4).setBuffer(buffer)); meshNode.setSkin(skin);
...
primitive.setAttribute('JOINTS_0',doc.createAccessor('AntlerDemonJoints').setArray(ji).setType(Accessor.Type.VEC4).setBuffer(buffer)); primitive.setAttribute('WEIGHTS_0',doc.createAccessor('AntlerDemonWeights').setArray(wt).setType(Accessor.Type.VEC4).setBuffer(buffer));
...
const output=await io.writeBinary(doc);const candidateFile=`${dir}/woodland-antler-demon-rigged.glb`;await writeFile(candidateFile,output);const candidateHash=createHash('sha256').update(output).digest('hex');
