...
const dir = 'assets/art/tripo/imports/creatures/starred-sheet-c/parts/02-pale-lavender-long-ear-demon';
const id = 'creature_pale_lavender_long_ear_demon';
const displayName = 'Pale Lavender Long-Ear Demon';
...
meshNode.setName('PaleLavenderLongEarDemonMesh');
const armature=doc.createNode('PaleLavenderLongEarDemonArmature'); scene.addChild(armature); armature.addChild(meshNode);
...
const skin=doc.createSkin('PaleLavenderLongEarDemonHumanoid').setSkeleton(joints.get('mixamorigHips')); for(const bone of bones)skin.addJoint(joints.get(bone.name));
...
const buffer=root.listBuffers()[0]; skin.setInverseBindMatrices(doc.createAccessor('LavenderDemonInverseBind').setArray(ibm).setType(Accessor.Type.MAT4).setBuffer(buffer)); meshNode.setSkin(skin);
...
primitive.setAttribute('JOINTS_0',doc.createAccessor('LavenderDemonJoints').setArray(ji).setType(Accessor.Type.VEC4).setBuffer(buffer)); primitive.setAttribute('WEIGHTS_0',doc.createAccessor('LavenderDemonWeights').setArray(wt).setType(Accessor.Type.VEC4).setBuffer(buffer));
...
const output=await io.writeBinary(doc);const candidateFile=`${dir}/pale-lavender-long-ear-demon-rigged.glb`;await writeFile(candidateFile,output);const candidateHash=createHash('sha256').update(output).digest('hex');
