...
const baseDir = 'assets/art/tripo/imports/creatures/starred-sheet-c/parts/07-ice-spike-demon';
const slug = 'ice-spike-demon';
const displayName = 'Ice Spike Demon';
const tier = 'T30-T40';
const part = JSON.parse(await readFile(`${baseDir}/part.json`, 'utf8'));
const sourcePath = `${baseDir}/base.glb`;
const candidatePath = `${baseDir}/ice-spike-demon-native-rig.glb`;
...
meshNode.setName('IceSpikeDemonMesh').setTranslation([0,0,0]).setRotation([0,0,0,1]).setScale([1,1,1]);
const armature = doc.createNode('IceSpikeDemonArmature').setScale([presentationScale,presentationScale,presentationScale]); scene.addChild(armature); armature.addChild(meshNode);
...
const skin = doc.createSkin('IceSpikeDemon_Humanoid').setSkeleton(jointNodes.get('mixamorigHips'));
...
const buffer=root.listBuffers()[0]; skin.setInverseBindMatrices(doc.createAccessor('IceSpikeDemon_InverseBind').setArray(ibm).setType(Accessor.Type.MAT4).setBuffer(buffer)); meshNode.setSkin(skin);
...
primitive.setAttribute('JOINTS_0',doc.createAccessor('IceSpikeDemon_Joints0').setArray(joints).setType(Accessor.Type.VEC4).setBuffer(buffer));
primitive.setAttribute('WEIGHTS_0',doc.createAccessor('IceSpikeDemon_Weights0').setArray(weights).setType(Accessor.Type.VEC4).setBuffer(buffer));
