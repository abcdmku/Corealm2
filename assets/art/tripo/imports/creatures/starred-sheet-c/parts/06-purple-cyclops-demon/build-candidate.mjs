...
const baseDir = 'assets/art/tripo/imports/creatures/starred-sheet-c/parts/06-purple-cyclops-demon';
const sourcePath = 'assets/art/tripo/imports/creatures/starred-sheet-c/parts/06-purple-cyclops-demon/base.glb';
const candidatePath = `${baseDir}/purple-cyclops-demon-native-rig.glb`;
await mkdir(baseDir, { recursive: true });

const io = new NodeIO().registerExtensions(ALL_EXTENSIONS);
const sourceBytes = await readFile(sourcePath);
const sourceSha256 = createHash('sha256').update(sourceBytes).digest('hex');
if (sourceSha256 !== '61c616190f018f20a19edf0f8d6f3a9304440c1790fc812614f91868c9e3488f') {
...
if (positions.length / 3 !== 3130 || indices.length / 3 !== 1667) {
...
meshNode.setName('PurpleCyclopsDemonMesh');
const rigContainer = doc.createNode('PurpleCyclopsDemonArmature').setTranslation([0, 0, 0]).setRotation([0, 0, 0, 1]).setScale([presentationScale, presentationScale, presentationScale]);
...
const skin = doc.createSkin('PurpleCyclopsDemon_Humanoid').setSkeleton(jointNodes.get('mixamorigHips'));
...
skin.setInverseBindMatrices(doc.createAccessor('PurpleCyclopsDemon_InverseBind').setArray(inverseBinds).setType(Accessor.Type.MAT4).setBuffer(buffer));
...
primitive.setAttribute('JOINTS_0', doc.createAccessor('PurpleCyclopsDemon_Joints0').setArray(jointValues).setType(Accessor.Type.VEC4).setBuffer(buffer));
primitive.setAttribute('WEIGHTS_0', doc.createAccessor('PurpleCyclopsDemon_Weights0').setArray(weightValues).setType(Accessor.Type.VEC4).setBuffer(buffer));
...
const checkContainer = checkRoot.listNodes().find((node) => node.getName() === 'PurpleCyclopsDemonArmature');
...
const candidate = {
  ...
  id: 'creature_purple_cyclops_demon',
  displayName: 'Purple Cyclops Demon',
  ...
  candidate: {
    ...
    productionTarget: 'game/public/assets/models/creature/creature_purple_cyclops_demon.glb',
...
const labAsset = {
  id: 'creature_purple_cyclops_demon',
  file: 'models/creature/creature_purple_cyclops_demon.glb',
  pack: 'corealm-starred-creatures',
  category: 'character',
  is: 'Purple Cyclops Demon',
