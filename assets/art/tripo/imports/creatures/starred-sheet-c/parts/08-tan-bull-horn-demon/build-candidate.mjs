...
const baseDir = 'assets/art/tripo/imports/creatures/starred-sheet-c/parts/08-tan-bull-horn-demon';
const slug = 'tan-bull-horn-demon';
const displayName = 'Tan Bull-Horn Demon';
const tier = 'T35-T45';
const part = JSON.parse(await readFile(`${baseDir}/part.json`, 'utf8'));
const sourcePath = `${baseDir}/base.glb`;
const candidatePath = `${baseDir}/tan-bull-horn-demon-native-rig.glb`;
...
meshNode.setName('TanBullHornDemonMesh').setTranslation([0,0,0]).setRotation([0,0,0,1]).setScale([1,1,1]);
const armature = doc.createNode('TanBullHornDemonArmature'); scene.addChild(armature); armature.addChild(meshNode);
...
const skin = doc.createSkin('TanBullHornDemon_Humanoid').setSkeleton(jointNodes.get('mixamorigHips'));
...
const buffer=root.listBuffers()[0]; skin.setInverseBindMatrices(doc.createAccessor('TanBullHornDemon_InverseBind').setArray(ibm).setType(Accessor.Type.MAT4).setBuffer(buffer)); meshNode.setSkin(skin);
...
primitive.setAttribute('JOINTS_0',doc.createAccessor('TanBullHornDemon_Joints0').setArray(joints).setType(Accessor.Type.VEC4).setBuffer(buffer));
primitive.setAttribute('WEIGHTS_0',doc.createAccessor('TanBullHornDemon_Weights0').setArray(weights).setType(Accessor.Type.VEC4).setBuffer(buffer));
