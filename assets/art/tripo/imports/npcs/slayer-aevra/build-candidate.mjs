...
const restPointFor = (name) => {
  const named = {
    ...
    Left_Shoulder: [-0.07, 0.71, 0.15], Left_UpperArm: [-0.105, 0.66, 0.15], Left_LowerArm: [-0.15, 0.57, 0.15], Left_Hand: [-0.18, 0.49, 0.15],
    Right_Shoulder: [0.07, 0.71, 0.15], Right_UpperArm: [0.105, 0.66, 0.15], Right_LowerArm: [0.15, 0.57, 0.15], Right_Hand: [0.18, 0.49, 0.15],
  ...
  if (fingerMatch) {
    ...
    return new Vector3(sign * (0.18 + digitIndex * 0.008), 0.49 - Math.max(0, phalanxIndex - 1) * 0.003, 0.15 + phalanxIndex * 0.025);
  }
...
const bodyScore = (point, sourceName, node) => {
  ...
  let gate = 1;
  if (group === 'hips') gate = 1 - smoothstep(0.39, 0.58, point.y);
  if (group === 'spine') gate = smoothstep(0.43, 0.50, point.y) * (1 - smoothstep(0.60, 0.67, point.y));
  if (group === 'chest') gate = smoothstep(0.52, 0.58, point.y) * (1 - smoothstep(0.68, 0.73, point.y));
  if (group === 'upperChest') gate = smoothstep(0.61, 0.67, point.y) * (1 - smoothstep(0.76, 0.82, point.y));
  ...
  if (group === 'leftArm' || group === 'rightArm') {
    ...
    gate *= smoothstep(0.015, 0.11, side * point.x);
...
for (let vertex = 0; vertex < positions.length / 3; vertex++) {
  ...
  const wingStrength = wingBand * smoothstep(0.10, 0.20, lateralDistance);
...
const wingRotationTrack = (classKey, axis, amplitude, times = sampleTimes, phase = 0) => {
  const data = wingData.get(classKey);
  const rootNode = jointNodes.get(data.rootName);
  const tipNode = jointNodes.get(data.tipName);
  const angles = times.map((_, index) => Math.sin((sampleTimes[index] + phase) * Math.PI * 2) * amplitude);
  return [
...
const allWingCycle = (amplitude, phase = 0, times = sampleTimes) => [
  ...wingRotationTrack('LUpper', 'z', amplitude, times, phase),
  ...wingRotationTrack('RUpper', 'z', -amplitude, times, phase),
  ...wingRotationTrack('LLower', 'z', amplitude * 0.70, times, phase + 0.18),
  ...wingRotationTrack('RLower', 'z', -amplitude * 0.70, times, phase + 0.18),
];
...
addClip('Idle', 2.4, [
  ...
  ...allWingCycle(0.10, 0.05, idleTimes),
]);
const walkTimes = sampleTimes.map((t) => t * 1.16);
addClip('Walk', 1.16, [
  { nodeName: 'mixamorigHips', node: body('Hips'), path: 'translation', times: walkTimes, values: walkTimes.map((_, i) => { const p = body('Hips').getTranslation(); return [p[0], p[1] + [0, 0.012, 0, -0.012, 0][i], p[2]]; }) },
  ...
  cycle('Left_UpperArm', 'x', 0.11, 0.5, walkTimes), cycle('Right_UpperArm', 'x', 0.11, 0, walkTimes),
  ...allWingCycle(0.22, 0, walkTimes),
]);
const runTimes = sampleTimes.map((t) => t * 0.74);
addClip('Run', 0.74, [
  { nodeName: 'mixamorigHips', node: body('Hips'), path: 'translation', times: runTimes, values: runTimes.map((_, i) => { const p = body('Hips').getTranslation(); return [p[0], p[1] + [0, 0.022, 0, -0.022, 0][i], p[2]]; }) },
  ...
  cycle('Left_UpperArm', 'x', 0.25, 0.5, runTimes), cycle('Right_UpperArm', 'x', 0.25, 0, runTimes),
  ...allWingCycle(0.39, 0, runTimes),
]);
const attackTimes = [0, 0.18, 0.44, 0.72, 0.96];
addClip('Attack', 0.96, [
  pose('UpperChest', 'y', attackTimes, [0, -0.12, 0.16, 0.08, 0]),
  pose('Right_UpperArm', 'x', attackTimes, [0, -0.48, -0.14, 0.18, 0]),
  pose('Right_LowerArm', 'x', attackTimes, [0, 0.22, -0.62, -0.20, 0]),
  pose('Left_UpperArm', 'x', attackTimes, [0, -0.16, -0.26, 0.04, 0]),
...
addClip('Hit', 0.48, [
  ...
  pose('Right_UpperArm', 'x', hitTimes, [0, -0.26, 0.08, 0]),
...
addClip('Death', 1.45, [
  ...
  pose('Left_UpperArm', 'x', deathTimes, [0, 0.10, 0.50, 0.58, 0.58]),
  pose('Right_UpperArm', 'x', deathTimes, [0, -0.10, -0.48, -0.55, -0.55]),
  ...allWingCycle(0.44, 0.12, deathTimes),
]);
const talkTimes = [0, 0.35, 0.72, 1.05, 1.42];
addClip('Talk', 1.42, [
  pose('Head', 'y', talkTimes, [0, -0.08, 0.06, -0.04, 0]),
  pose('Right_UpperArm', 'x', talkTimes, [0, -0.36, -0.22, -0.31, 0]),
  pose('Right_LowerArm', 'x', talkTimes, [0, -0.15, -0.48, -0.18, 0]),
  pose('Left_UpperArm', 'x', talkTimes, [0, 0.05, -0.18, -0.04, 0]),
...
const candidate = {
  ...
  candidate: {
    ...
    rig: { type: 'Mixamo-named humanoid plus eight articulated wing joints', joints: checkSkin.listJoints().map((node) => node.getName()), wingInfluencedVertices: wingVertexCounts, verticesWithDistributedWeights: checkDistributedVertices, maximumWeightSumError: maxWeightSumError, method: 'Rest transforms recovered from Tripo inverse-bind matrices; source root-only weights replaced by four-influence, anatomy-gated weights. Four upper/lower wing chains receive vertex weights from the existing mesh silhouette. No retopology.' },
...
const labAsset = {
  ...
  sourceProvenance: {
    ...
    candidateSha256,
    rigMethod: 'Mixamo-named body bones recovered from source inverse binds, with four upper/lower wing chains weighted on the unchanged source topology.',
    textures: runtimeTextureMetrics,
