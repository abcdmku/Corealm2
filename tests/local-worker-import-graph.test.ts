...
it("evaluates no content before the catalog is installed", async () => {
  ...
  // `semver.ts` checks the manifest's base version and imports nothing.
  expect([...before.keys()].sort()).toEqual(["game/src/content/catalogInstall.ts", "game/src/contracts.ts", "game/src/featureLab/labSpec.ts", "game/src/multiplayer/semver.ts",
    "game/src/worker/labProtocol.ts", "game/src/worker/localHost.ts", "game/src/worker/localHostProtocol.ts"]);
}, 120_000);
