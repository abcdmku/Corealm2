...
const root='art/rebuild/candidates/finish-structures/closed-boulder04';
const sha=(bytes:Uint8Array)=>createHash('sha256').update(bytes).digest('hex');
describe('staged single-body Boulder 04 shortcuts',()=>{
 it('retains every native source channel, index and embedded map without additional geometry',async()=>{
  ...
  for(const asset of catalogue.assets){
   ...
   expect(asset.triangles).toBe(59066);
  }
 });
 it('keeps exact legacy pivots/bounds under positive transforms and reports the unmodified-ground limitation',async()=>{
  ...
  for(const asset of catalogue.assets){
   ...
   expect(asset.transform.pitchDegrees).toBe(0);
   if(asset.id==='corealm_scree_slide')expect(asset.rearToFrontQuarterDrop).toBeGreaterThanOrEqual(0);
  }
 });
});
