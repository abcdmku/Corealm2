...
`);
    const server = { watcher, httpServer, ws: { send: vi.fn() }, ssrLoadModule: vi.fn(async () => ({ acceptedValue: JSON.parse(await readFile(catalog, 'utf8')).tables.value })) } as unknown as ViteDevServer;
    ...
    await expect(services.compiler()).rejects.toThrow('type checking');
    await vi.waitFor(async () => expect((await services.getBuildStatus!()).state).toBe('valid'), { timeout: 30_000, interval: 50 });
