...
function makeMesh(
  ...
  const mesh = new SceneryInstances(geometry, material, capacity, { colors });
  matrices.forEach((matrix, index) => mesh.setMatrixAt(index, matrix));
  mesh.instanceCount = matrices.length;
  mesh.updateMatrixWorld(true);
  meshes.push(mesh);
  return mesh;
}

function drawnMatrices(mesh: SceneryInstances): number[][] {
  return Array.from({ length: mesh.instanceCount }, (_, index) => {
...
afterEach(() => {
  for (const mesh of meshes.splice(0)) {
    mesh.dispose();
    mesh.sourceGeometry.dispose();
  }
  for (const material of sourceMaterials.splice(0)) material.dispose();
});
...
describe("scatter visibility", () => {
  it("keeps intersecting geometry in source order and recovers after turning away", () => {
    ...
    const version = mesh.instanceTransforms.version;

    camera.lookAt(0, 0, 1);
    camera.updateMatrixWorld(true);
    visibility.prepare(camera);
    expect(mesh.instanceCount).toBe(0);
    expect(mesh.instanceTransforms.version).toBe(version);
    expectResidentBounds();
  ...
  it("uploads changed members when the count is equal and skips unchanged matrices", () => {
    ...
    const firstVersion = mesh.instanceTransforms.version;

    visibility.prepare(camera);
    expect(mesh.instanceTransforms.version).toBe(firstVersion);
    camera.lookAt(0, 0, 1);
    camera.updateMatrixWorld(true);
    visibility.prepare(camera);
    expect(drawnMatrices(mesh)).toEqual([original[2]]);
    expect(mesh.instanceTransforms.version).toBeGreaterThan(firstVersion);
    const secondVersion = mesh.instanceTransforms.version;
    visibility.prepare(camera);
    expect(mesh.instanceTransforms.version).toBe(secondVersion);
  });
  ...
  it("pads wind after the instance transform and before the parent transform", () => {
    ...
    expect(still.instanceCount).toBe(0);
  ...
  it("reports registered sources and effective draw counts across fog, frustum and hidden meshes", () => {
    ...
    const versions = registered.map((mesh) => mesh.instanceTransforms.version);
    const visibility = new ScatterVisibility();
    ...
    expect(registered.map((mesh, index) => mesh.instanceTransforms.version - versions[index]!))
      .toEqual([0, 1, 0, 0, 0, 0, 0]);
    expect(hidden.instanceCount).toBe(1);
    expect(hiddenByParent.instanceCount).toBe(2);
  ...
  it("counts only matrix writes during fog compaction, zero-count changes and full restoration", () => {
    ...
    visibility.add(mesh, 0);
    visibility.prepare(camera, 8);
    const compactedVersion = mesh.instanceTransforms.version;
    ...
    expect(visibility.getStats().matrixUploads).toBe(0);
    expect(mesh.instanceTransforms.version).toBe(compactedVersion);
    ...
    expect(mesh.instanceTransforms.version).toBe(compactedVersion + 1);
    visibility.prepare(camera);
    expect(visibility.getStats().matrixUploads).toBe(0);
  });
  ...
  it("restores the registered population immediately on disable, remove and clear", () => {
    ...
    const original = drawnMatrices(mesh);
    const originalBuffer = Array.from(mesh.instanceTransforms.array);
    ...
    visibility.setEnabled(false);
    expect(drawnMatrices(mesh)).toEqual(original);
    expect(Array.from(mesh.instanceTransforms.array)).toEqual(originalBuffer);
    ...
    visibility.prepare(camera);
    expect(drawnMatrices(mesh)).toEqual(original);
    expect(other.instanceCount).toBe(1);
    visibility.clear();
    expect(drawnMatrices(other)).toEqual(otherOriginal);
    visibility.prepare(camera);
    expect(drawnMatrices(other)).toEqual(otherOriginal);
  ...
  it("rejects registrations that would lose shadow or per-instance data", () => {
    ...
    const colored = makeMesh([translation(0)], undefined, 1, true);
    expect(() => visibility.add(colored, 0)).toThrow();
    ...
    mesh.geometry.setAttribute("variation", new THREE.InterleavedBufferAttribute(mesh.instanceTransforms, 1, 0));
    expect(() => visibility.add(mesh, 0)).toThrow();
    mesh.geometry.deleteAttribute("variation");
    const matrixColumn = mesh.geometry.getAttribute("sceneryMatrix0");
    mesh.geometry.deleteAttribute("sceneryMatrix0");
    expect(() => visibility.add(mesh, 0)).toThrow();
    mesh.geometry.setAttribute("sceneryMatrix0", new THREE.BufferAttribute(new Float32Array(4), 4));
    expect(() => visibility.add(mesh, 0)).toThrow();
    mesh.geometry.setAttribute("sceneryMatrix0", new THREE.InterleavedBufferAttribute(mesh.instanceTransforms, 4, 4));
    expect(() => visibility.add(mesh, 0)).toThrow();
    mesh.geometry.setAttribute("sceneryMatrix0", new THREE.InterleavedBufferAttribute(
      new THREE.InstancedInterleavedBuffer(new Float32Array(16), 16), 4, 0,
    ));
    expect(() => visibility.add(mesh, 0)).toThrow();
    mesh.geometry.setAttribute("sceneryMatrix0", matrixColumn);
