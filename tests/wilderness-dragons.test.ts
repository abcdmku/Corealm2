...
async function assetFor(id:string){
 ...
 let asset=manifest.assets.find((a:any)=>a.id===id),file=asset?`game/public/assets/${asset.file}`:'';
...
describe('Wilderness winged dragon production assets',()=>{
 it('keeps three juvenile and three adult identities at native authored metres',()=>{
  expect(WILDERNESS_DRAGONS).toHaveLength(6);expect(new Set(WILDERNESS_DRAGONS.map(s=>s.id)).size).toBe(6);
  ...
  expect(WILDERNESS_DRAGONS.map(s=>enemyCombatLevel(s.stats))).toEqual([50,53,56,72,75,78]);
 ...
 it('ships three actual winged rigs, native UVs, normalized skin and distinct juvenile geometry',async()=>{
  ...
  for(const s of WILDERNESS_DRAGONS){
   ...
   const molten=s.id==='baby_lava_dragon'||s.id==='purple_wilderness_dragon';
   expect(root.listMaterials().some(m=>!!m.getEmissiveTexture())).toBe(molten);
   if(molten){const fissures=asset.metadata.provenance.textures.fissures;expect(fissures.litFraction).toBeGreaterThan(.002);expect(fissures.litFraction).toBeLessThan(.025);}
