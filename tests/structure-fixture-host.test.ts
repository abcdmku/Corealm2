...
describe("authored structure fixture fidelity", () => {
  ...
  it("places the real Coldbrace vault tower behind its frame with matching solid and regional material", () => {
    ...
    const coldbrace = region.settlements.find(town => town.id === "coldbrace")!;
    const tower = coldbrace.buildings.find(building=>building.id === "coldbrace_vault")!;
    ...
    const native = buildPrefab(tower.prefab,tower.footprint,variantSeed(tower.id),coldbrace.kit);
